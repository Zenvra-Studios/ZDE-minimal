...
void SelectionAnimationModel::clear() noexcept { m_targets.clear(); }
...
bool SelectionAnimationModel::tick() noexcept {
  ...
  if (dt <= 0.0F)
    return m_animated.size() > m_targets.size();

  // Exp decay interpolation formula: val += (target - val) * (1 - exp(-speed *
  // dt)) We approximate it with lerp for small dt, or just use the exact
  // formula:
...
const std::vector<UI::Rect> &
SelectionAnimationModel::get_animated_rects() const noexcept {
  return m_animated;
}
