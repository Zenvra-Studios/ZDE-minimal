...
constexpr auto types = std::to_array<std::string_view>({
    ...
    "string_view", "Display", "Runtime", "Window", "X11Context", "X11Window",
});
...
StudioEditorLayoutResult StudioEditorLayout::calculate(
    ...
    bool terminal_maximized,
    bool tool_sidebar_visible,
    float tool_sidebar_width) const noexcept
{
    ...
    const float available_after_activity = std::max(safe_width - activity_width, 0.0F);
    const float minimum_editor_region = std::min(240.0F * safe_scale, available_after_activity);
    const float maximum_sidebar_width = std::max(
        available_after_activity - minimum_editor_region,
        0.0F);
    const float sidebar_width = tool_sidebar_visible
        ? std::clamp(tool_sidebar_width * safe_scale, 0.0F, maximum_sidebar_width)
        : 0.0F;
    ...
    const Utility::FlexLayoutResult root = Utility::Column::calculate(
        root_bounds,
        root_items);
    ...
    const Utility::FlexLayoutResult content = Utility::Row::calculate(
        content_bounds,
        content_items);
    ...
    const float requested_terminal_height = terminal_visible
        ? (terminal_maximized
            ...
            : std::clamp(terminal_height * safe_scale, 0.0F, workspace_body.height))
        : 0.0F;
    const std::array body_items{
        Utility::FlexItem::flexible(),
        Utility::FlexItem::fixed(requested_terminal_height),
    };
    const Utility::FlexLayoutResult body = Utility::Column::calculate(
        workspace_body,
        body_items);
    ...
    const Utility::FlexLayoutResult editor = Utility::Row::calculate(
        editor_row_bounds,
        editor_items);

    const float terminal_header_height = std::min(28.0F * safe_scale, terminal_bounds.height);
    const std::array terminal_items{
        Utility::FlexItem::fixed(terminal_header_height),
        Utility::FlexItem::flexible(),
    };
    const Utility::FlexLayoutResult terminal = Utility::Column::calculate(
        terminal_bounds,
        terminal_items);
    ...
    const float minimap_width = std::min(
        112.0F * safe_scale,
        std::max(result.editor_bounds.width * 0.17F, 0.0F));
    result.minimap_bounds = {
        std::max(safe_width - scrollbar_width - minimap_width, result.editor_bounds.x),
        result.editor_bounds.y,
        std::min(minimap_width,
            std::max(safe_width - scrollbar_width - result.editor_bounds.x, 0.0F)),
...
std::optional<std::size_t> hit_test_studio_sidebar(
    const StudioEditorLayoutResult& layout,
    float point_x,
    float point_y) noexcept
{
    ...
    for (std::size_t index = 0; index < items.size(); ++index)
    {
        if (calculate_studio_sidebar_item_bounds(layout, index).contains(point_x, point_y))
        {
            return index;
        }
    }
...
float calculate_editor_tab_width(float text_width, float dpi_scale) noexcept
{
    ...
    return std::clamp(
        std::max(text_width, 0.0F) + StudioEditorMetrics::editor_tab_chrome_width * scale,
    ...
    std::array<EditorToken, maximum_editor_tokens>& output) noexcept
{
    ...
    while (cursor < line.size() && token_count < output.size())
    {
        ...
        if (std::isspace(static_cast<unsigned char>(character)) != 0)
        {
            ...
            append(line.substr(token_start, cursor - token_start), EditorTokenKind::Plain);
            continue;
        }
        ...
        if (character == '/' && cursor + 1 < line.size() && line[cursor + 1] == '*')
        {
            ...
            append(line.substr(token_start, cursor - token_start), EditorTokenKind::Comment);
            continue;
        }
        if (character == '"' || character == '\'')
        {
            ...
            append(line.substr(token_start, cursor - token_start), EditorTokenKind::String);
            continue;
        }
             ...
             std::isdigit(static_cast<unsigned char>(line[cursor + 1])) != 0))
        {
            ...
            append(line.substr(token_start, cursor - token_start), EditorTokenKind::Number);
            continue;
        }
        if (is_identifier_start(character))
        {
            ...
            {
                ++cursor;
                append(line.substr(token_start, cursor - token_start), EditorTokenKind::Label);
                continue;
            }
...
bool supports_editor_syntax_highlighting(std::string_view file_name) noexcept
{
    ...
    const std::string_view extension{normalized_name.data() + dot, normalized_name.size() - dot};
    constexpr std::array<std::string_view, 14> highlighted_extensions{
        ".c", ".cc", ".cpp", ".cxx", ".h", ".hh", ".hpp", ".hxx",
        ".inl", ".m", ".mm", ".vert", ".frag", ".glsl",
    };
