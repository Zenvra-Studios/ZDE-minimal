...
void AddNewItemDialog::refresh_fonts() {
    if (m_regular_font) DeleteObject(m_regular_font);
    if (m_semibold_font) DeleteObject(m_semibold_font);
    if (m_small_font) DeleteObject(m_small_font);

    const int font_size_9pt = -MulDiv(9, static_cast<int>(m_dpi), 72);
    const int font_size_8pt = -MulDiv(8, static_cast<int>(m_dpi), 72);

    m_regular_font = CreateFontW(
        font_size_9pt, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
        DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
        CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_DONTCARE, L"Segoe UI");

    m_semibold_font = CreateFontW(
        font_size_9pt, 0, 0, 0, FW_SEMIBOLD, FALSE, FALSE, FALSE,
        DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
    ...
    m_small_font = CreateFontW(
        font_size_8pt, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
        DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
        CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_DONTCARE, L"Segoe UI");
}
...
static std::filesystem::path resolve_asset_path(const std::string& rel_path) {
    ...
    if (!ec && std::filesystem::exists(p1, ec)) return p1;
    ...
    if (len > 0) {
        ...
        for (int i = 0; i < 6 && !exe_dir.empty(); ++i) {
            ...
            if (std::filesystem::exists(candidate, ec)) return candidate;
            if (!exe_dir.has_parent_path() || exe_dir == exe_dir.parent_path()) break;
...
void AddNewItemDialog::draw_icon(HDC dc, const std::string& icon_rel_path, int x, int y, int size) const {
    if (icon_rel_path.empty() || size <= 0) return;
    ...
    if (it != m_icon_cache.end() && it->second.bitmap != nullptr) {
        ...
        auto bitmap = doc->renderToBitmap(static_cast<std::uint32_t>(size), static_cast<std::uint32_t>(size));
        ...
        icon_bm = CreateDIBSection(temp_dc, &bmi, DIB_RGB_COLORS, &bits, nullptr, 0);
        DeleteDC(temp_dc);

        if (!icon_bm || !bits) return;

        const auto* src_pixels = reinterpret_cast<const std::uint8_t*>(bitmap.data());
...
void AddNewItemDialog::init_default_templates() {
    ...
    // 1. Visual C++ Category
    TemplateCategory cpp_cat{"cpp", "Visual C++", "Assets/icons/material-icon-theme/cpp.svg", {
        {
            "cpp_file",
            "C++ File (.cpp)",
            "Source.cpp",
    ...
    // 2. Rust Category
    TemplateCategory rust_cat{"rust", "Rust", "Assets/icons/material-icon-theme/rust.svg", {
        {
            "rs_main",
            "Rust Binary (main.rs)",
            "main.rs",
    ...
    TemplateCategory shader_cat{"shader", "Shaders & Graphics", "Assets/icons/material-icon-theme/shader.svg", {
        {
            "glsl_frag",
    ...
    TemplateCategory build_cat{"build", "Build & Config", "Assets/icons/material-icon-theme/cmake.svg", {
        {
            "cmakelists",
    ...
    // 5. General Category
    TemplateCategory gen_cat{"general", "General", "Assets/icons/material-icon-theme/document.svg", {
        {
            "txt_file",
            "Text Document (.txt)",
            "Document.txt",
...
void AddNewItemDialog::open(HWND parent_hwnd, const std::filesystem::path& target_folder, const std::string& project_name, CreateCallback callback) {
...
void AddNewItemDialog::submit() {
    if (m_filename_input.empty()) return;
    std::string content;
    if (m_selected_category_index < m_categories.size()) {
        const auto& templates = m_categories[m_selected_category_index].templates;
...
AddNewItemDialog::LayoutResult AddNewItemDialog::calculate_layout(const UI::Rect& viewport, float dpi_scale) const {
    return calculate_layout(viewport.width, viewport.height, dpi_scale);
}

AddNewItemDialog::LayoutResult AddNewItemDialog::calculate_layout(float width, float height, float dpi_scale) const {
    LayoutResult layout;
    ...
    layout.close_button_bounds = {width - close_btn_w, 0.0F, close_btn_w, title_h};
    ...
    layout.name_label_bounds = {pad_x, row1_y + 2.0F * dpi_scale, name_label_w, input_h};
    
    layout.cancel_button_bounds = {width - pad_x - btn_w, row1_y, btn_w, btn_h};
    layout.add_button_bounds = {layout.cancel_button_bounds.x - btn_w - 8.0F * dpi_scale, row1_y, btn_w, btn_h};
    
    const float name_input_w = layout.add_button_bounds.x - 14.0F * dpi_scale - (layout.name_label_bounds.right());
    layout.name_input_bounds = {layout.name_label_bounds.right(), row1_y, std::max(60.0F * dpi_scale, name_input_w), input_h};

    // Row 2 of Footer: Location label & path
    const float row2_y = row1_y + input_h + 8.0F * dpi_scale;
    layout.location_label_bounds = {pad_x, row2_y + 2.0F * dpi_scale, name_label_w, input_h};
    layout.location_value_bounds = {layout.location_label_bounds.right(), row2_y + 2.0F * dpi_scale, width - pad_x * 2.0F - name_label_w, input_h};
    ...
    layout.details_pane_bounds = {cat_w + template_w, middle_y, details_w, middle_h};
    ...
    for (std::size_t i = 0; i < m_categories.size(); ++i) {
        layout.category_item_bounds.push_back({0.0F, curr_cat_y, cat_w, cat_item_h});
    ...
    if (m_selected_category_index < m_categories.size()) {
        ...
        for (std::size_t i = 0; i < templates.size(); ++i) {
            layout.template_item_bounds.push_back({cat_w, curr_tpl_y, template_w, tpl_item_h});
...
void AddNewItemDialog::render(HDC dc, const LayoutResult& layout, const UI::Theme::StudioTheme& theme, float dpi_scale) const {
    ...
    // 1. Clean Dark Background & Frame (Matching Image 2)
    const COLORREF bg_col = RGB(28, 28, 30);
    const COLORREF border_col = RGB(55, 55, 60);
    const COLORREF section_border_col = RGB(45, 45, 48);
    ...
    Rectangle(dc, native_dlg.left, native_dlg.top, native_dlg.right, native_dlg.bottom);
    ...
    // 2. Titlebar
    const COLORREF titlebar_col = RGB(24, 24, 26);
    ...
    HPEN sep_pen = CreatePen(PS_SOLID, 1, RGB(55, 55, 62));
    ...
    const int title_icon_y = static_cast<int>((layout.titlebar_bounds.height - title_icon_size) * 0.5F);
    draw_icon(dc, "Assets/icons/new-file.svg", title_icon_x, title_icon_y, title_icon_size);

    SetBkMode(dc, TRANSPARENT);
    SetTextColor(dc, RGB(225, 225, 225));
    RECT title_text_r = native_title;
    title_text_r.left += static_cast<LONG>(32.0F * dpi_scale);
    const std::wstring title_str = L"Add New Item - " + Utility::utf8_to_wide(m_project_name).value_or(L"Project");
    DrawTextW(dc, title_str.c_str(), -1, &title_text_r, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
    ...
    SetTextColor(dc, m_close_hovered ? RGB(255, 255, 255) : RGB(160, 160, 160));
    DrawTextW(dc, L"\u2715", -1, &close_text_r, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
    ...
    const COLORREF select_blue = to_color_ref(theme.accent); // Visual Studio Blue RGB(0, 122, 204)
    const int icon_size_16 = static_cast<int>(16.0F * dpi_scale);

    for (std::size_t i = 0; i < m_categories.size() && i < layout.category_item_bounds.size(); ++i) {
        ...
        const bool hovered = (m_hovered_category_index && *m_hovered_category_index == i);
        ...
        if (selected) {
            ...
            HBRUSH it_br = CreateSolidBrush(RGB(40, 40, 44));
            FillRect(dc, &ir, it_br);
            DeleteObject(it_br);
        }
        ...
        const int cat_ic_y = ir.top + static_cast<int>((item_b.height - icon_size_16) * 0.5F);
        draw_icon(dc, cat.icon_path, cat_ic_x, cat_ic_y, icon_size_16);

        // Draw Category Text
        RECT tr = ir;
        tr.left += static_cast<LONG>(32.0F * dpi_scale);
        SetTextColor(dc, selected ? RGB(255, 255, 255) : RGB(205, 205, 210));
        const std::wstring label_w = Utility::utf8_to_wide(cat.name).value_or(L"");
        DrawTextW(dc, label_w.c_str(), -1, &tr, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
    }

    // 5. Middle Template Items Pane (With SVG Icons & Antialiased text)
    if (m_selected_category_index < m_categories.size()) {
        const auto& templates = m_categories[m_selected_category_index].templates;
        for (std::size_t i = 0; i < templates.size() && i < layout.template_item_bounds.size(); ++i) {
            const auto& tpl = templates[i];
            const auto& item_b = layout.template_item_bounds[i];
            const bool selected = (i == m_selected_template_index);
            const bool hovered = (m_hovered_template_index && *m_hovered_template_index == i);
            ...
            const int tpl_ic_y = ir.top + static_cast<int>((item_b.height - icon_size_16) * 0.5F);
            draw_icon(dc, tpl.icon_path, tpl_ic_x, tpl_ic_y, icon_size_16);

            // Draw Template Name
            RECT tr = ir;
            tr.left += static_cast<LONG>(32.0F * dpi_scale);
            SetTextColor(dc, selected ? RGB(255, 255, 255) : RGB(220, 220, 225));
            const std::wstring name_w = Utility::utf8_to_wide(tpl.name).value_or(L"");
            DrawTextW(dc, name_w.c_str(), -1, &tr, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);

            // Draw Category Tag (with small font)
            SelectObject(dc, m_small_font);
            RECT tag_r = ir;
            tag_r.right -= static_cast<LONG>(14.0F * dpi_scale);
            SetTextColor(dc, selected ? RGB(240, 240, 240) : RGB(135, 135, 140));
            const std::wstring cat_w = Utility::utf8_to_wide(tpl.category).value_or(L"");
            DrawTextW(dc, cat_w.c_str(), -1, &tag_r, DT_RIGHT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
    ...
    if (m_selected_category_index < m_categories.size()) {
        ...
        if (m_selected_template_index < templates.size()) {
            ...
            const std::wstring type_str = L"Type: " + Utility::utf8_to_wide(tpl.category).value_or(L"");
            DrawTextW(dc, type_str.c_str(), -1, &type_r, DT_LEFT | DT_TOP | DT_SINGLELINE | DT_NOPREFIX);
            SelectObject(dc, m_regular_font);
            ...
            const std::wstring desc_str = Utility::utf8_to_wide(tpl.description).value_or(L"");
            DrawTextW(dc, desc_str.c_str(), -1, &desc_r, DT_LEFT | DT_TOP | DT_WORDBREAK | DT_NOPREFIX);
        }
    }
    ...
    DrawTextW(dc, L"Name:", -1, &name_lbl, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);

    // Location Label (full "Location:")
    RECT loc_lbl = to_native_rect(layout.location_label_bounds);
    DrawTextW(dc, L"Location:", -1, &loc_lbl, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
    ...
    DrawTextW(dc, loc_w.c_str(), -1, &loc_val, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX | DT_PATH_ELLIPSIS);

    // Name Input Box
    const COLORREF input_bg = RGB(22, 22, 24);
    const COLORREF input_border = m_name_input_focused ? select_blue : RGB(60, 60, 65);
    ...
    DrawTextW(dc, fn_w.c_str(), -1, &in_text_r, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);

    // Blinking Caret
    if (m_name_input_focused && m_caret_visible) {
        SIZE text_size{};
        const std::wstring pre_caret = fn_w.substr(0, std::min(m_caret_position, fn_w.size()));
        GetTextExtentPoint32W(dc, pre_caret.c_str(), static_cast<int>(pre_caret.size()), &text_size);
    ...
    // Add Button (Solid Blue Flat Button matching Image 2)
    const COLORREF add_bg = m_add_hovered ? RGB(0, 140, 230) : select_blue;
    ...
    SelectObject(dc, m_regular_font);
    SetTextColor(dc, RGB(255, 255, 255));
    DrawTextW(dc, L"Add", -1, &add_r, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);

    // Cancel Button (Dark Gray Flat Button matching Image 2)
    const COLORREF cancel_bg = m_cancel_hovered ? RGB(55, 55, 60) : RGB(42, 42, 45);
    HBRUSH can_br = CreateSolidBrush(cancel_bg);
    HPEN can_pen = CreatePen(PS_SOLID, 1, RGB(55, 55, 60));
    ...
    DrawTextW(dc, L"Cancel", -1, &can_r, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);

    // 8. 1px Outer Window Frame Border (drawn last to outline the entire dialog window)
    HPEN frame_pen = CreatePen(PS_SOLID, 1, RGB(65, 65, 75));
...
LRESULT CALLBACK AddNewItemDialog::dialog_proc(HWND hwnd, UINT message, WPARAM w_param, LPARAM l_param) {
    ...
    if (message == WM_NCCREATE) {
        ...
        self = reinterpret_cast<AddNewItemDialog*>(GetWindowLongPtrW(hwnd, GWLP_USERDATA));
    }
...
LRESULT AddNewItemDialog::handle_message(HWND hwnd, UINT message, WPARAM w_param, LPARAM l_param) {
    switch (message) {
    case WM_PAINT: {
        PAINTSTRUCT ps;
        ...
        HBITMAP mem_bm = CreateCompatibleBitmap(hdc, rc.right - rc.left, rc.bottom - rc.top);
    ...
    case WM_NCHITTEST: {
        ...
        if (layout.close_button_bounds.contains(static_cast<float>(pt.x), static_cast<float>(pt.y))) {
            return HTCLIENT;
        }
        // Titlebar draggable
        if (layout.titlebar_bounds.contains(static_cast<float>(pt.x), static_cast<float>(pt.y))) {
            return HTCAPTION;
        }
        return HTCLIENT;
    }
    ...
    case WM_MOUSEMOVE: {
        ...
        GetClientRect(hwnd, &rc);
        const float dpi_scale = static_cast<float>(m_dpi) / 96.0F;
        const auto layout = calculate_layout(static_cast<float>(rc.right - rc.left), static_cast<float>(rc.bottom - rc.top), dpi_scale);
...
bool AddNewItemDialog::handle_pointer_move(float x, float y, const LayoutResult& layout) {
    ...
    return old_close != m_close_hovered || old_add != m_add_hovered ||
           old_cancel != m_cancel_hovered || old_cat != m_hovered_category_index ||
           old_tpl != m_hovered_template_index;
}

bool AddNewItemDialog::handle_pointer_press(float x, float y, const LayoutResult& layout) {
    if (layout.close_button_bounds.contains(x, y) || layout.cancel_button_bounds.contains(x, y)) {
        close();
        return true;
    }
...
bool AddNewItemDialog::handle_double_click(float x, float y, const LayoutResult& layout) {
...
bool AddNewItemDialog::handle_text_input(std::string_view text) {
    if (!m_name_input_focused || text.empty()) return false;
    for (char ch : text) {
        if (ch >= 32 && ch != 127) {
...
bool AddNewItemDialog::is_interactive_point(float x, float y, const LayoutResult& layout) const {
