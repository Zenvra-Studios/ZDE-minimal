...
public:
  ...
  void set_backdrop_config(const ModalBackdropConfig &config) noexcept {
    m_backdrop_config = config;
  }
  [[nodiscard]] const ModalBackdropConfig &
  get_backdrop_config() const noexcept {
    return m_backdrop_config;
  }
  ...
  [[nodiscard]] const std::string &get_edition() const noexcept {
    return m_edition;
  }
  [[nodiscard]] const std::string &get_platform() const noexcept {
    return m_platform;
  }
  [[nodiscard]] const std::string &get_studio_name() const noexcept {
    return m_studio_name;
  }
  [[nodiscard]] const std::string &get_created_by() const noexcept {
    return m_created_by;
  }
  [[nodiscard]] const std::vector<AboutSpecItem> &get_specs() const noexcept {
    return m_specs;
  }

  /// Formats a complete multi-line specifications text for copying to the
  /// clipboard.
  [[nodiscard]] std::string get_clipboard_text() const;

  /// Calculates responsive layout metrics for the 2-column hero modal dialog.
  [[nodiscard]] AboutModalLayoutResult
  calculate_layout(const Rect &viewport_bounds,
                   float dpi_scale = 1.0F) const noexcept;

  // Pointer events
  [[nodiscard]] bool
  handle_pointer_press(float x, float y,
                       const AboutModalLayoutResult &layout) noexcept;
  [[nodiscard]] bool
  handle_pointer_move(float x, float y,
                      const AboutModalLayoutResult &layout) noexcept;
  [[nodiscard]] bool handle_pointer_release(
      float x, float y, const AboutModalLayoutResult &layout,
      const std::function<void(const std::string &)> &copy_callback =
          nullptr) noexcept;
  ...
  [[nodiscard]] bool is_close_hovered() const noexcept {
    return m_close_hovered;
  }
  ...
  [[nodiscard]] bool is_close_pressed() const noexcept {
    return m_close_pressed;
  }

#ifndef ZDE_APP_NAME
#define ZDE_APP_NAME "Zenvra Development Studio"
#endif

#ifndef ZDE_EDITION_NAME
#define ZDE_EDITION_NAME "Community Edition"
#endif

#ifndef ZDE_VERSION_STRING
#define ZDE_VERSION_STRING "0.1.0"
#endif

#if defined(_WIN32)
  #if defined(_M_ARM64)
    #define ZDE_DEFAULT_PLATFORM "Windows - ARM64 (64-Bit)"
  #elif defined(_M_IX86)
    #define ZDE_DEFAULT_PLATFORM "Windows - x86 (32-Bit)"
  #else
    #define ZDE_DEFAULT_PLATFORM "Windows - x64 (64-Bit)"
  #endif
#elif defined(__APPLE__)
  #if defined(__aarch64__) || defined(__arm64__)
    #define ZDE_DEFAULT_PLATFORM "macOS - Apple Silicon (ARM64)"
  #else
    #define ZDE_DEFAULT_PLATFORM "macOS - Intel (x64)"
  #endif
#elif defined(__linux__)
  #if defined(__aarch64__)
    #define ZDE_DEFAULT_PLATFORM "Linux - ARM64 (64-Bit)"
  #elif defined(__arm__)
    #define ZDE_DEFAULT_PLATFORM "Linux - ARM32 (32-Bit)"
  #elif defined(__i386__)
    #define ZDE_DEFAULT_PLATFORM "Linux - x86 (32-Bit)"
  #else
    #define ZDE_DEFAULT_PLATFORM "Linux - x64 (64-Bit)"
  #endif
#else
  #define ZDE_DEFAULT_PLATFORM "Unix - 64-Bit"
#endif

  std::string m_app_name = ZDE_APP_NAME;
  std::string m_edition = std::string(ZDE_EDITION_NAME) + " v" + ZDE_VERSION_STRING;
  std::string m_platform = ZDE_DEFAULT_PLATFORM;
  std::string m_studio_name = "Zenvra Studios";
  std::string m_created_by = "Zenvra Developer Team (Zenvra Studios)";
