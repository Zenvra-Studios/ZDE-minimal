...
struct ItemTemplate {
    ...
    std::string category;          // e.g. "Visual C++"
...
public:
    using CreateCallback = std::function<void(const std::string& filename, const std::string& initial_content)>;
    ...
    void open(HWND parent_hwnd, const std::filesystem::path& target_folder, const std::string& project_name, CreateCallback callback);
    void close();
    [[nodiscard]] bool is_visible() const noexcept { return m_hwnd != nullptr && IsWindow(m_hwnd); }
    ...
    [[nodiscard]] LayoutResult calculate_layout(const UI::Rect& viewport, float dpi_scale) const;
    [[nodiscard]] LayoutResult calculate_layout(float width, float height, float dpi_scale) const;

    void render(HDC dc, const LayoutResult& layout, const UI::Theme::StudioTheme& theme, float dpi_scale) const;
    ...
    [[nodiscard]] bool is_interactive_point(float x, float y, const LayoutResult& layout) const;
...
private:
    ...
    void submit();
    void draw_icon(HDC dc, const std::string& icon_rel_path, int x, int y, int size) const;
    void refresh_fonts();
