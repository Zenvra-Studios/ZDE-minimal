...
static bool has_argument(int argument_count, char **argument_values,
                         std::string_view expected_argument) {
  return std::any_of(argument_values + 1, argument_values + argument_count,
                     [expected_argument](const char *argument) {
                       return argument == expected_argument;
                     });
}
...
int main(int argument_count, char **argument_values) {
  ...
  for (int i = 1; i < argument_count; ++i) {
    if (std::string(argument_values[i]) == "--simulate-missing-dependency" &&
        i + 1 < argument_count) {
  ...
  if (std::filesystem::exists("/proc/self/exe", ec_proc)) {
    exe_path =
        std::filesystem::canonical("/proc/self/exe", ec_proc).parent_path();
  } else {
  ...
  if (manifest_content.empty()) {
    ShowNativeErrorDialog("ZDE Startup Error",
                          "Failed to read runtime manifest at:\n" +
  ...
  ValidationResult val_result =
      ValidateDependencies(runtime_dir.string(), manifest.dependencies);
  ...
  const bool safe_ui =
      has_argument(argument_count, argument_values, "--safe-ui");
  const bool native_titlebar =
      has_argument(argument_count, argument_values, "--native-titlebar");
  const bool smoke_test =
      has_argument(argument_count, argument_values, "--smoke-test");
  ...
  for (int i = 1; i < argument_count; ++i) {
    ...
    if (!specification.initial_path) {
      ...
      std::filesystem::path p =
          std::filesystem::absolute(argument_values[i], ec);
