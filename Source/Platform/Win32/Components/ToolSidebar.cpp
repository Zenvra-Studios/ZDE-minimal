    ...
    int maximum_width)
{
    ...
    while (!text.empty() &&
           font.getTextWidth(device_context, text + std::string{suffix}) > maximum_width)
    {
        text.pop_back();
        while (!text.empty() &&
    ...
    const UI::Editor::StudioEditorLayoutResult& layout) const
{
    ...
    const float scale = layout.dpi_scale;
    surface.fill_rectangle(device_context, panel, surface.m_palette.sidebar_background);
    ...
    if (m_model.get_active_icon() != UI::Editor::SidebarIcon::Project)
    {
        ...
        surface.draw_text(device_context, *surface.m_ui_font, m_model.get_content_heading(),
            panel.x + 14.0F * scale, message_y, surface.m_palette.text_primary);
        const std::string detail = ellipsize(device_context, *surface.m_small_font,
    ...
    for (std::size_t item_index = first; item_index < end; ++item_index)
    {
        ...
        if (m_hovered_row && *m_hovered_row == visible_row)
        {
            surface.fill_rectangle(
                device_context, row_bounds, surface.m_palette.tab_active_background);
        }
        ...
        else
        {
            ...
            surface.draw_svg_icon(device_context, icon_asset,
                icon_x, icon_y, std::max(round_to_int(14.0F * scale), 11),
                surface.m_palette.text_muted, row_background);
        }
