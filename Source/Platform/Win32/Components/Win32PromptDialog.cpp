...
std::wstring utf8_to_wstring(std::string_view str) {
  if (str.empty())
    return {};
  const int len = MultiByteToWideChar(CP_UTF8, 0, str.data(),
...
bool Win32PromptDialog::open_new_folder(
    HWND parent, const std::filesystem::path &target_dir,
  ...
  m_hwnd =
      CreateWindowExW(WS_EX_APPWINDOW, prompt_dialog_class_name, L"New Folder",
...
bool Win32PromptDialog::open_new_file(
  ...
  m_hwnd =
      CreateWindowExW(WS_EX_APPWINDOW, prompt_dialog_class_name, L"New File",
                      WS_POPUP | WS_CLIPCHILDREN | WS_THICKFRAME, pos_x, pos_y,
                      w, h, parent, nullptr, instance, this);
...
bool Win32PromptDialog::open_rename(
    HWND parent, const std::filesystem::path &item_path,
    std::function<void(const std::string &)> on_confirm) {
  close();
  ...
  m_hwnd = CreateWindowExW(WS_EX_APPWINDOW, prompt_dialog_class_name, L"Rename",
                           WS_POPUP | WS_CLIPCHILDREN | WS_THICKFRAME, pos_x,
                           pos_y, w, h, parent, nullptr, instance, this);
...
bool Win32PromptDialog::open_delete(HWND parent,
  ...
  m_title = "Delete Item";
  m_target_path = item_path;
  m_subtitle = "Are you sure you want to permanently delete '" +
  ...
  m_hwnd =
      CreateWindowExW(WS_EX_APPWINDOW, prompt_dialog_class_name, L"Delete Item",
                      WS_POPUP | WS_CLIPCHILDREN | WS_THICKFRAME, pos_x, pos_y,
                      w, h, parent, nullptr, instance, this);
...
LRESULT CALLBACK Win32PromptDialog::dialog_proc(HWND hwnd, UINT message,
                                                WPARAM w_param,
                                                LPARAM l_param) {
  ...
  if (message == WM_NCCREATE) {
    ...
    self = reinterpret_cast<Win32PromptDialog *>(
...
LRESULT Win32PromptDialog::handle_message(HWND hwnd, UINT message,
  ...
  case WM_PAINT: {
    ...
    SIZE title_sz{};
    GetTextExtentPoint32W(mem_dc, title_w.c_str(),
                          static_cast<int>(title_w.size()), &title_sz);
    const int title_x = (w - title_sz.cx) / 2;
    const int title_y = static_cast<int>(14.0F * scale);
    TextOutW(mem_dc, title_x, title_y, title_w.c_str(),
             static_cast<int>(title_w.size()));
    ...
    if (m_mode == PromptDialogMode::ConfirmDelete) {
      ...
      SIZE sub_sz{};
      GetTextExtentPoint32W(mem_dc, sub_w.c_str(),
                            static_cast<int>(sub_w.size()), &sub_sz);
      const int sub_x = (w - sub_sz.cx) / 2;
      const int sub_y = title_y + title_sz.cy + static_cast<int>(4.0F * scale);
      TextOutW(mem_dc, sub_x, sub_y, sub_w.c_str(),
               static_cast<int>(sub_w.size()));
      ...
      fill_rounded_rect(mem_dc, cancel_rc,
                        m_cancel_hovered ? RGB(58, 61, 68) : RGB(45, 47, 52),
                        static_cast<int>(6.0F * scale));
      SelectObject(mem_dc, m_ui_font);
      SetTextColor(mem_dc, RGB(204, 204, 204));
      DrawTextW(mem_dc, L"Cancel", -1, &cancel_rc,
                DT_CENTER | DT_VCENTER | DT_SINGLELINE);

      RECT ok_rc{ok_x, btn_y, ok_x + btn_w, btn_y + btn_h};
      fill_rounded_rect(mem_dc, ok_rc,
                        m_ok_hovered ? RGB(235, 65, 70) : RGB(218, 45, 50),
                        static_cast<int>(6.0F * scale));
      SetTextColor(mem_dc, RGB(255, 255, 255));
      DrawTextW(mem_dc, L"Delete", -1, &ok_rc,
                DT_CENTER | DT_VCENTER | DT_SINGLELINE);
    } else {
      ...
      if (m_text_value.empty()) {
        ...
        TextOutW(mem_dc, input_x + 4, input_y + 6, ph_w.c_str(),
                 static_cast<int>(ph_w.size()));
      } else {
        SetTextColor(mem_dc, RGB(240, 242, 245));
        const std::wstring val_w = utf8_to_wstring(m_text_value);
        TextOutW(mem_dc, input_x + 4, input_y + 6, val_w.c_str(),
                 static_cast<int>(val_w.size()));
      }
      ...
      if (m_caret_visible) {
        ...
        SIZE pref_sz{};
        GetTextExtentPoint32W(mem_dc, pref_w.c_str(),
                              static_cast<int>(pref_w.size()), &pref_sz);
  ...
  case WM_MOUSEMOVE: {
    ...
    if (m_mode == PromptDialogMode::ConfirmDelete) {
      ...
      const int btn_h = static_cast<int>(26.0F * scale);
      const int btn_y = h - static_cast<int>(14.0F * scale) - btn_h;
      RECT cancel_rc{w / 2 - btn_w - static_cast<int>(6.0F * scale), btn_y,
                     w / 2 - static_cast<int>(6.0F * scale), btn_y + btn_h};
      RECT ok_rc{w / 2 + static_cast<int>(6.0F * scale), btn_y,
                 w / 2 + static_cast<int>(6.0F * scale) + btn_w, btn_y + btn_h};
  ...
  case WM_CHAR: {
    ...
    if (ch >= 32) {
      char utf8_buf[8] = {};
      WideCharToMultiByte(CP_UTF8, 0, &ch, 1, utf8_buf, sizeof(utf8_buf),
                          nullptr, nullptr);
      ...
      if (w_param == 'V' && (GetKeyState(VK_CONTROL) & 0x8000)) {
        if (OpenClipboard(hwnd)) {
          ...
          if (hData) {
            ...
            if (pszText) {
              char utf8_buf[1024] = {};
              WideCharToMultiByte(CP_UTF8, 0, pszText, -1, utf8_buf,
                                  sizeof(utf8_buf), nullptr, nullptr);
              m_caret_pos = std::min(m_caret_pos, m_text_value.size());
              m_text_value.insert(m_caret_pos, utf8_buf);
              m_caret_pos += strlen(utf8_buf);
              GlobalUnlock(hData);
