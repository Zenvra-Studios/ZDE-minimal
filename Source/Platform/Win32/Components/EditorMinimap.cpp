    ...
    std::size_t visible_lines) const
{
    ...
    surface.fill_rectangle(device_context, bounds, surface.m_palette.editor_background);
    surface.draw_line(
        device_context,
    ...
    for (std::size_t sample = 0; sample < sample_count; ++sample)
    {
        ...
        for (std::size_t token_index = 0; token_index < token_count; ++token_index)
        {
            ...
            const int width = surface.get_text_width(
                device_context, *surface.m_minimap_font, text);
    ...
    surface.draw_rectangle(device_context, viewport,
        m_pointer_dragging ? surface.m_palette.accent : surface.m_palette.text_muted);
}
