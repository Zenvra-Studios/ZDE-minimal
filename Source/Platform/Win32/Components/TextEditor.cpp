  ...
  for (const std::size_t start : folding.get_collapsed()) {
    if (const auto *r = folding.get_range_at(start)) {
      if (r->end_line >= r->start_line && r->start_line < total_lines) {
        const std::size_t clamped_end =
            std::min(r->end_line, total_lines > 0 ? total_lines - 1 : 0);
...
fold_start_line_at_point(const UI::Components::EditorFoldingModel &folding,
  ...
  const float actual_line_height =
      line_height > 0.0F ? line_height : (20.0F * dpi_scale);
  const std::size_t clicked_row = static_cast<std::size_t>(std::max(
      static_cast<int>((point_y - layout.editor_bounds.y) / actual_line_height),
      0));
...
std::tuple<std::optional<UI::Editor::TextPosition>,
...
find_enclosing_braces(const UI::Editor::TextDocumentModel &document) {
  ...
  if (start_line >= document.get_line_count()) {
    return {std::nullopt, std::nullopt, std::nullopt};
  }

  std::string_view line = document.get_line(start_line);

  auto is_delimiter = [](char c) noexcept {
    return c == '{' || c == '}' || c == '(' || c == ')' || c == '[' || c == ']';
  };

  auto is_open_delimiter = [](char c) noexcept {
    return c == '{' || c == '(' || c == '[';
  };

  auto get_matching_char = [](char c) noexcept -> char {
    switch (c) {
    case '{':
      return '}';
    case '}':
  ...
  // Locate delimiter adjacent to the cursor:
  // Priority: character immediately before cursor (typing or moving right:
  // `{|`, `(|`, `[|`, `)|`, `}|`, `]|`)
  std::optional<std::size_t> target_col;
  char target_ch = '\0';

  if (start_col > 0 && start_col - 1 < line.size() &&
      is_delimiter(line[start_col - 1])) {
    target_col = start_col - 1;
    target_ch = line[start_col - 1];
  } else if (start_col < line.size() && is_delimiter(line[start_col])) {
    target_col = start_col;
    target_ch = line[start_col];
  }
  ...
  const UI::Editor::TextPosition target_pos{start_line, *target_col};
  const UI::Editor::TextPosition pulsing_pos = target_pos;

  std::optional<UI::Editor::TextPosition> open_pos;
  std::optional<UI::Editor::TextPosition> close_pos;

  if (is_open_delimiter(target_ch)) {
    open_pos = target_pos;
    const char open_ch = target_ch;
    const char close_ch = get_matching_char(target_ch);

    int depth = 0;
    const std::size_t doc_lines = document.get_line_count();
    const std::size_t forward_limit = std::min(doc_lines, start_line + 2500);

    for (std::size_t line_idx = start_line; line_idx < forward_limit;
         ++line_idx) {
      std::string_view l = document.get_line(line_idx);
      std::size_t search_start = (line_idx == start_line) ? *target_col : 0;

      for (std::size_t i = search_start; i < l.size(); ++i) {
        if (l[i] == open_ch) {
          ++depth;
        } else if (l[i] == close_ch) {
          --depth;
          if (depth == 0) {
            close_pos = UI::Editor::TextPosition{line_idx, i};
            break;
          }
        }
      }
      if (depth == 0 && close_pos.has_value()) {
        break;
      }
    }
  } else {
    close_pos = target_pos;
    const char close_ch = target_ch;
    const char open_ch = get_matching_char(target_ch);

    int depth = 0;
    const int search_limit = std::max(0, static_cast<int>(start_line) - 2500);

    for (int line_idx = static_cast<int>(start_line); line_idx >= search_limit;
         --line_idx) {
      std::string_view l =
          document.get_line(static_cast<std::size_t>(line_idx));
      int search_end = (line_idx == static_cast<int>(start_line))
                           ? static_cast<int>(*target_col) + 1
                           : static_cast<int>(l.size());
      search_end = std::min(search_end, static_cast<int>(l.size()));
      ...
      for (int i = search_end - 1; i >= 0; --i) {
        if (l[i] == close_ch) {
          ...
          if (depth == 0) {
            open_pos =
                UI::Editor::TextPosition{static_cast<std::size_t>(line_idx),
                                         static_cast<std::size_t>(i)};
            break;
          }
        }
      }
      if (depth == 0 && open_pos.has_value()) {
        break;
      }
    }
  }

  return {open_pos, close_pos, pulsing_pos};
}
...
void TextEditor::sync_lsp_active_document() {
  if (const auto *doc = m_controller.get_active_document(); doc != nullptr) {
    ...
    Language::LanguageServerManager::instance().on_document_opened(uri, fname,
                                                                   1, content);
  }
}
...
bool TextEditor::is_media_active() const noexcept {
  if (const auto *active_p = m_controller.get_active_path();
      active_p && !active_p->empty()) {
...
bool TextEditor::is_media_fullscreen() const noexcept {
  return m_media_preview_mode && m_media_player_view.is_open() &&
         m_media_player_view.is_fullscreen();
}

bool TextEditor::is_media_point(
    const UI::Editor::StudioEditorLayoutResult &layout, float point_x,
    float point_y) const noexcept {
  if (!m_media_preview_mode)
    return false;
  ...
  const bool left_is_media =
      left_p ? UI::Editor::MediaPlayerView::is_media_file(*left_p)
             : (left_doc && UI::Editor::MediaPlayerView::is_media_file(
                                std::string(left_doc->get_file_name())));

  const auto *right_doc =
      is_split_active ? m_controller.get_document(*m_split_document_index)
                      : nullptr;
  const bool right_is_media =
      right_doc && UI::Editor::MediaPlayerView::is_media_file(
                       std::string(right_doc->get_file_name()));

  const float pane_left_x = (left_is_media && m_media_preview_mode)
                                ? layout.gutter_bounds.x
                                : layout.editor_bounds.x;
  const float full_editor_w =
      (left_is_media && m_media_preview_mode)
          ? (layout.gutter_bounds.width + layout.editor_bounds.width)
          : layout.editor_bounds.width;
  const float half_w = is_split_active
                           ? ((full_editor_w - 2.0F * scale) * m_split_ratio)
                           : full_editor_w;
  const float splitter_x = pane_left_x + half_w;

  const UI::Rect left_pane =
      is_split_active ? UI::Rect{pane_left_x, layout.editor_bounds.y, half_w,
                                 layout.editor_bounds.height}
                      : UI::Rect{pane_left_x, layout.editor_bounds.y,
                                 full_editor_w, layout.editor_bounds.height};
  const UI::Rect right_pane{
      splitter_x + 2.0F * scale, layout.editor_bounds.y,
      std::max(layout.editor_bounds.right() - (splitter_x + 2.0F * scale),
               0.0F),
      layout.editor_bounds.height};

  if (left_is_media && left_pane.contains(point_x, point_y))
    return true;
  if (right_is_media && is_split_active &&
      right_pane.contains(point_x, point_y))
    return true;
...
bool TextEditor::is_media_interactive_point(
    const UI::Editor::StudioEditorLayoutResult &layout, float point_x,
    float point_y) const noexcept {
  if (!is_media_point(layout, point_x, point_y))
    return false;
...
bool TextEditor::open_file_at_location(const std::filesystem::path &path,
  ...
  if (auto *doc = const_cast<UI::Editor::TextDocumentModel *>(
...
bool TextEditor::go_to_definition() {
  ...
  const Language::Protocol::Position pos{.line = caret_line,
                                         .character = caret_col};
  ...
  Language::LanguageServerManager::instance().request_definition(
      ...
      [this](std::vector<Language::Protocol::Location> locations) {
        ...
        if (std::filesystem::exists(target_path, ec)) {
          static_cast<void>(
              this->open_file_at_location(target_path, loc.range.start.line + 1,
...
float TextEditor::get_line_height(const StudioWorkspaceRenderer &surface,
...
bool TextEditor::close_file(const std::filesystem::path &path) {
  ...
  for (std::size_t i = 0; i < docs.size(); ++i) {
    if (docs[i].path == path || docs[i].text.get_file_name() == path.string() ||
        docs[i].path.filename() == path.filename()) {
...
void TextEditor::show_tab_action_menu(
  ...
  for (const auto &item : m_tab_action_menu.items) {
    ...
    total_h += row_height;
    float item_width =
        static_cast<float>(item.label.size()) * 7.0F * scale + 44.0F * scale;
...
void TextEditor::draw_editor_header(
  ...
  if (m_is_split && m_split_document_index.has_value() &&
    ...
    IntersectClipRect(device_context, round_to_int(left_header.x),
                      round_to_int(left_header.y),
                      round_to_int(left_header.right() - 4.0F * scale),
                      round_to_int(left_header.bottom()));
    ...
    surface.draw_svg_icon(device_context, "Assets/icons/" + left_icon,
    ...
    surface.draw_text(device_context, *surface.m_small_font, left_filename,
    ...
    m_split_close_btn_bounds =
        UI::Rect{right_header.right() - button_w - 4.0F * scale, button_y,
                 button_w, button_h};
    if (m_hovered_split_close) {
      surface.fill_rounded_rectangle(device_context, m_split_close_btn_bounds,
                                     UI::Theme::Color{255, 255, 255, 25},
                                     4.0F * scale);
    }
    const UI::Theme::Color cross_col =
        m_hovered_split_close ? UI::Theme::Color{255, 255, 255, 255}
                              : surface.m_palette.text_muted;
    const int cross_cx = round_to_int(m_split_close_btn_bounds.x +
                                      m_split_close_btn_bounds.width * 0.5F);
    const int cross_cy = round_to_int(m_split_close_btn_bounds.y +
                                      m_split_close_btn_bounds.height * 0.5F);
    const int cross_icon_sz = std::max(round_to_int(12.0F * scale), 10);
    surface.draw_svg_icon(device_context, "Assets/icons/diagnostic-error.svg",
                          cross_cx, cross_cy, cross_icon_sz, cross_col,
                          surface.m_palette.editor_background);
    ...
    m_tab_action_bounds[3] =
        UI::Rect{actions_right - 1.0F * button_w, button_y, button_w, button_h};
    m_tab_action_bounds[2] =
        UI::Rect{actions_right - 2.0F * button_w - 2.0F * scale, button_y,
                 button_w, button_h};
    m_tab_action_bounds[1] =
        UI::Rect{actions_right - 3.0F * button_w - 4.0F * scale, button_y,
                 button_w, button_h};
    m_tab_action_bounds[0] =
        UI::Rect{actions_right - 4.0F * button_w - 6.0F * scale, button_y,
                 button_w, button_h};

    // Right Header File Title with Icon (Clipped before buttons)
    SaveDC(device_context);
    IntersectClipRect(device_context, round_to_int(right_header.x),
                      round_to_int(right_header.y),
                      round_to_int(m_tab_action_bounds[0].x - 4.0F * scale),
                      round_to_int(right_header.bottom()));
    ...
    if (split_doc != nullptr) {
      ...
      surface.draw_svg_icon(device_context, "Assets/icons/" + right_icon,
                            round_to_int(right_icon_cx), round_to_int(center_y),
                            right_icon_sz, surface.m_palette.text_muted,
                            surface.m_palette.editor_background, true);

      surface.draw_text(device_context, *surface.m_small_font, right_filename,
    ...
    // Right side: Action Buttons aligned to the right of header_bounds
    const float actions_right = header_bounds.right() - 4.0F * scale;
    const bool has_media = is_media_active();

    if (has_media) {
      m_tab_action_bounds[4] =
          UI::Rect{actions_right - 1.0F * button_w, button_y, button_w,
                   button_h}; // ellipsis
      m_tab_action_bounds[3] =
          UI::Rect{actions_right - 2.0F * button_w - 2.0F * scale, button_y,
                   button_w, button_h}; // arrow-right
      m_tab_action_bounds[2] =
          UI::Rect{actions_right - 3.0F * button_w - 4.0F * scale, button_y,
                   button_w, button_h}; // arrow-left
      m_tab_action_bounds[1] =
          UI::Rect{actions_right - 4.0F * button_w - 6.0F * scale, button_y,
                   button_w, button_h}; // play.svg (media preview toggle)
      m_tab_action_bounds[0] =
          UI::Rect{actions_right - 5.0F * button_w - 8.0F * scale, button_y,
                   button_w, button_h}; // split-right
    } else {
      m_tab_action_bounds[4] =
          UI::Rect{actions_right - 1.0F * button_w, button_y, button_w,
                   button_h}; // ellipsis
    ...
    const float leftmost_btn_x =
        has_media ? m_tab_action_bounds[0].x : m_tab_action_bounds[0].x;

    // Left side: File title with Icon (Clipped before buttons)
    SaveDC(device_context);
    IntersectClipRect(device_context, round_to_int(header_bounds.x),
                      round_to_int(header_bounds.y),
                      round_to_int(leftmost_btn_x - 4.0F * scale),
                      round_to_int(header_bounds.bottom()));
    ...
    surface.draw_svg_icon(device_context, "Assets/icons/" + icon_asset,
                          round_to_int(icon_cx), round_to_int(center_y),
                          icon_sz, surface.m_palette.text_muted,
                          surface.m_palette.editor_background, true);

    surface.draw_text(device_context, *surface.m_small_font, filename,
                      header_bounds.x + 12.0F * scale + icon_sz + 6.0F * scale,
                      center_y, surface.m_palette.text_primary);
    RestoreDC(device_context, -1);
  }

  const std::string media_icon =
      (is_media_active() && m_media_preview_mode) ? "file.svg" : "play.svg";
  const std::string icon_0 = "split-right.svg";
  const std::string icon_2 = "arrow-left.svg";
  const std::string icon_3 = "arrow-right.svg";
  const std::string icon_4 = "ellipsis.svg";
  const std::string icons[] = {icon_0, media_icon, icon_2, icon_3, icon_4};
  const int icon_sizes[] = {std::max(round_to_int(13.0F * scale), 11),
  ...
  for (std::size_t i = 0; i < 5; ++i) {
    ...
    const bool is_active_menu =
        (i == 4 && m_tab_action_menu.visible) || (i == 0 && m_is_split) ||
        (i == 1 && is_media_active() && m_media_preview_mode);

    if (is_active_menu || is_hovered) {
      surface.fill_rounded_rectangle(device_context, btn,
                                     is_active_menu
                                         ? UI::Theme::Color{255, 255, 255, 36}
    ...
    UI::Theme::Color icon_col = (is_hovered || is_active_menu)
                                    ? UI::Theme::Color{255, 255, 255, 255}
                                    : surface.m_palette.text_muted;

    if (i == 1 && is_media_active() && m_media_preview_mode) {
      icon_col = surface.m_palette.accent;
    }

    surface.draw_svg_icon(device_context, icons[i], cx, cy, icon_sizes[i],
  ...
  surface.draw_line(device_context, round_to_int(header_bounds.x), y_bot,
...
void TextEditor::draw_tab_action_menu(
  ...
  const RECT native_bounds{round_to_int(bounds.x), round_to_int(bounds.y),
  ...
  // 4. Draw items
  for (std::size_t i = 0; i < m_tab_action_menu.items.size() &&
                          i < m_tab_action_menu.item_bounds.size();
       ++i) {
    ...
    if (item.is_separator) {
      surface.fill_rectangle(device_context,
                             UI::Rect{item_bounds.x + 10.0F * scale,
                                      item_bounds.y + item_bounds.height * 0.5F,
                                      item_bounds.width - 20.0F * scale, 1.0F},
                             surface.m_palette.border);
      continue;
    }

    const bool hovered = (m_tab_action_menu.hovered_index &&
                          *m_tab_action_menu.hovered_index == i);
    if (hovered) {
      ...
      surface.fill_rounded_rectangle(device_context, hover_bounds,
    ...
    if (item.has_checkbox && item.is_checked) {
      ...
      surface.draw_svg_icon(device_context, "check.svg", check_x, check_y,
    ...
    surface.draw_text(device_context, *surface.m_small_font, item.label, text_x,
                      text_y,
                      hovered ? UI::Theme::Color{255, 255, 255, 255}
                              : surface.m_palette.text_primary);

    if (!item.shortcut.empty()) {
      const int sc_w = surface.get_text_width(
          device_context, *surface.m_small_font, item.shortcut);
      const float sc_x =
          item_bounds.right() - static_cast<float>(sc_w) - 12.0F * scale;
      surface.draw_text(device_context, *surface.m_small_font, item.shortcut,
                        sc_x, text_y,
                        hovered ? UI::Theme::Color{255, 255, 255, 255}
                                : surface.m_palette.text_muted);
    }
...
void TextEditor::draw_split_drop_overlay(
  ...
  if (m_active_drop_zone != SplitDropZone::None) {
    ...
    const float full_h =
        layout.editor_bounds.height + layout.editor_header_bounds.height;
    ...
    if (m_active_drop_zone == SplitDropZone::Right) {
      zone_rect =
          UI::Rect{full_x + full_w * 0.5F, full_y, full_w * 0.5F, full_h};
      ...
      zone_rect =
          UI::Rect{full_x, full_y + full_h * 0.5F, full_w, full_h * 0.5F};
    ...
    surface.fill_rounded_rectangle(device_context, zone_rect,
                                   UI::Theme::Color{53, 132, 228, 55},
                                   6.0F * scale);

    // Glowing border
    const RECT native_zone{round_to_int(zone_rect.x), round_to_int(zone_rect.y),
    ...
    const UI::Rect badge_rect{zone_rect.x + (zone_rect.width - badge_w) * 0.5F,
    ...
    surface.draw_svg_icon(device_context, "split-right.svg", cx, cy,
                          std::max(round_to_int(14.0F * scale), 12),
                          UI::Theme::Color{53, 132, 228, 255},
                          UI::Theme::Color{20, 24, 34, 255});

    surface.draw_text(device_context, *surface.m_small_font, zone_label,
  ...
  if (m_drag_cursor_y > layout.tab_bar_bounds.bottom()) {
    const std::string_view fname_view =
        documents[dragged_idx].text.get_file_name();
    const std::string fname{fname_view};
    const int text_w =
        surface.get_text_width(device_context, *surface.m_small_font, fname);
    const float pill_w = static_cast<float>(text_w) + 40.0F * scale;
    const float pill_h = 24.0F * scale;
    const UI::Rect pill_rect{m_drag_cursor_x + 12.0F * scale,
                             m_drag_cursor_y + 12.0F * scale, pill_w, pill_h};

    // Shadow
    surface.fill_rounded_rectangle(
        device_context,
        UI::Rect{pill_rect.x + 2.0F, pill_rect.y + 3.0F, pill_rect.width,
                 pill_rect.height},
        UI::Theme::Color{0, 0, 0, 100}, 4.0F * scale);

    // Background
    surface.fill_rounded_rectangle(device_context, pill_rect,
                                   UI::Theme::Color{30, 32, 42, 240},
                                   4.0F * scale);

    // Border
    const RECT native_pill{round_to_int(pill_rect.x), round_to_int(pill_rect.y),
    ...
    surface.draw_svg_icon(device_context, "file.svg", px, py,
                          std::max(round_to_int(12.0F * scale), 10),
                          UI::Theme::Color{53, 132, 228, 255},
                          UI::Theme::Color{30, 32, 42, 255});

    surface.draw_text(device_context, *surface.m_small_font, fname,
                      pill_rect.x + 24.0F * scale, pill_rect.y + pill_h * 0.5F,
                      UI::Theme::Color{255, 255, 255, 255});
  }
}
...
bool TextEditor::is_split_resize_handle_point(
  ...
  // Check if mouse is on left scrollbar or minimap - if so, it's NOT a splitter
  // resize point!
  ...
  const float left_minimap_w =
      (left_bounds.width >= MIN_PANE_WIDTH_FOR_MINIMAP * scale)
          ...
          : 0.0F;
  const UI::Rect left_scrollbar{left_bounds.right() - scrollbar_w, scroll_top_y,
                                scrollbar_w, scroll_total_h};
  ...
  if (left_scrollbar.contains(point_x, point_y) ||
      left_minimap.contains(point_x, point_y) ||
  ...
  const bool x_in_range = (point_x >= splitter_x - grab_margin) &&
                          (point_x <= splitter_x + 2.0F * scale + grab_margin);
  const bool y_in_range = (point_y >= layout.editor_header_bounds.y) &&
                          (point_y <= layout.editor_bounds.bottom());
  ...
  for (const UI::Editor::EditorSessionDocument &document : documents) {
    const float width = UI::Editor::calculate_editor_tab_width(
        static_cast<float>(surface.get_text_width(
            device_context, *surface.m_ui_font, document.text.get_file_name())),
        surface.m_dpi_scale);
...
bool TextEditor::handle_pointer_press(
    ...
    if (m_completion_popup.is_visible() &&
      ...
      if (doc != nullptr) {
        ...
        const std::size_t scroll_offset =
            m_completion_popup.get_scroll_offset();
        ...
        const float popup_h =
            static_cast<float>(visible_count) * item_h + 4.0F * scale;
        ...
        const float code_x = layout.editor_bounds.x +
                             layout.gutter_bounds.width + 14.0F * scale -
        ...
        const std::ptrdiff_t vis_row =
            static_cast<std::ptrdiff_t>(physical_line_to_visual_row(
                m_folding, doc->get_caret_line(), doc->get_line_count()));
        const std::ptrdiff_t first_vis =
            static_cast<std::ptrdiff_t>(m_scrollbar.get_first_visible_line());
        const float caret_line_y =
            layout.editor_bounds.y +
            static_cast<float>(vis_row - first_vis + 1) * line_h;
        ...
        for (std::size_t i = 0; i < max_visible && (scroll_offset + i) < count;
          ...
          if (const auto *it = m_completion_popup.get_item(scroll_offset + i)) {
            const int w = surface.get_text_width(device_context,
                                                 *surface.m_ui_font, it->label);
            max_label_w =
                std::max(max_label_w, static_cast<float>(w) + 64.0F * scale);
          }
        }
        const float popup_w =
            std::clamp(max_label_w, 220.0F * scale, 380.0F * scale);
        ...
        if (popup_bounds.contains(point_x, point_y)) {
          ...
          if (rel_y >= 0.0F) {
            const std::size_t clicked_row =
                static_cast<std::size_t>(rel_y / item_h);
            const std::size_t target_idx = scroll_offset + clicked_row;
            if (target_idx < count) {
              while (m_completion_popup.get_selected_index() < target_idx)
                m_completion_popup.select_next();
              while (m_completion_popup.get_selected_index() > target_idx)
                m_completion_popup.select_previous();
              lock.unlock();
              static_cast<void>(handle_input(
      ...
      for (std::size_t i = 0; i < m_tab_action_menu.item_bounds.size(); ++i) {
        if (!m_tab_action_menu.items[i].is_separator &&
          ...
          if (i == 0) {
            ...
            const_cast<TextEditor *>(this)->m_preview_editors_enabled =
                !m_preview_editors_enabled;
            const_cast<TextEditor *>(this)
                ->m_tab_action_menu.items[6]
                .is_checked = m_preview_editors_enabled;
          } else if (i == 8) {
  ...
  for (std::size_t i = 0; i < 5; ++i) {
    if (!m_tab_action_bounds[i].is_empty() &&
        m_tab_action_bounds[i].contains(point_x, point_y)) {
      if (i == 0) {
        ...
        if (is_media_active()) {
          m_media_preview_mode = !m_media_preview_mode;
          std::clog << "[ZDE] Media View mode toggled: "
                    << (m_media_preview_mode ? "Media Output Preview"
                                             : "Decompiled Hex View")
                    << "\n";
        }
      } else if (i == 2) {
  ...
  if (layout.tab_bar_bounds.contains(point_x, point_y)) {
    ...
    for (std::size_t index = 0; index < documents.size(); ++index) {
      const float width = UI::Editor::calculate_editor_tab_width(
          static_cast<float>(
              surface.get_text_width(device_context, *surface.m_ui_font,
      ...
      if (bounds.contains(point_x, point_y)) {
        ...
        if (close_bounds.contains(point_x, point_y)) {
          const float shift =
              bounds.width + UI::Editor::StudioEditorMetrics::editor_tab_gap *
                                 surface.m_dpi_scale;
          const auto closing_path = documents[index].path;
          const bool is_closing_active =
              (m_controller.get_active_index() == index);

          if (m_media_player_view.current_path() == closing_path ||
              m_media_player_view.current_path().filename() ==
                  closing_path.filename() ||
              is_closing_active || documents.size() <= 1) {
            m_media_player_view.close();
          }

          const bool closed = m_controller.close_file(index);
          if (closed) {
            if (is_closing_active || m_controller.get_documents().empty() ||
                !is_media_active()) {
              m_media_player_view.close();
            }
        ...
        if (m_controller.activate_file(index)) {
          ...
          if (is_media_active()) {
            m_media_preview_mode = true;
            if (const auto *p = m_controller.get_active_path();
                p && !p->empty() && m_media_player_view.current_path() != *p) {
              m_media_player_view.close();
            }
          } else {
            m_media_player_view.close();
  ...
  if (is_media_fullscreen()) {
    ...
    if (m_media_player_view.handle_mouse_down(
            point_x, point_y, layout.workspace_bounds, surface.m_dpi_scale)) {
  ...
  if (left_is_media && m_media_preview_mode &&
      left_pane.contains(point_x, point_y)) {
    m_focused = true;
    ...
    if (m_media_player_view.has_error()) {
      ...
      const float card_w =
          std::min(480.0F * scale, left_pane.width - 40.0F * scale);
      const float card_h = 160.0F * scale;
      const UI::Rect card_rect{center_x - card_w * 0.5F,
                               center_y - card_h * 0.5F, card_w, card_h};
      const float btn_w = 110.0F * scale;
      const float btn_h = 28.0F * scale;
      const UI::Rect btn_rect{center_x - btn_w * 0.5F,
                              card_rect.y + 105.0F * scale, btn_w, btn_h};
      if (btn_rect.contains(point_x, point_y)) {
        const std::filesystem::path p =
            (left_p && !left_p->empty())
                ? *left_p
                : std::filesystem::path(std::string(left_doc->get_file_name()));
    ...
    m_pointer_selecting = false;
    const bool was_fs = m_media_player_view.is_fullscreen();
    if (m_media_player_view.handle_mouse_down(point_x, point_y, left_pane,
                                              surface.m_dpi_scale)) {
      if (was_fs != m_media_player_view.is_fullscreen()) {
        notify_fullscreen_changed();
      }
    ...
    return true;
  } else if (right_is_media && m_media_preview_mode && is_split_active &&
             right_pane.contains(point_x, point_y)) {
    m_focused = true;
    m_focused_pane = SplitPaneFocus::Right;
    m_pointer_selecting = false;
    if (m_media_player_view.handle_mouse_down(point_x, point_y, right_pane,
                                              surface.m_dpi_scale)) {
      if (m_window_handle)
        InvalidateRect(m_window_handle, nullptr, FALSE);
    }
  ...
  if (is_split_active) {
    ...
    if (right_doc != nullptr && right_bounds.contains(point_x, point_y)) {
      ...
      if (point_x < fold_margin_left && split_line < split_total_lines) {
        const_cast<UI::Editor::TextDocumentModel *>(right_doc)
            ->toggle_breakpoint(split_line);
        if (m_window_handle)
          InvalidateRect(m_window_handle, nullptr, FALSE);
        return true;
      }
      ...
      const UI::Editor::TextPosition pos = position_from_point(
    ...
    if (left_doc != nullptr && left_bounds.contains(point_x, point_y)) {
      ...
      if (point_x < fold_margin_left && line_index < total_lines) {
        const_cast<UI::Editor::TextDocumentModel *>(left_doc)
            ->toggle_breakpoint(line_index);
        if (m_window_handle)
          InvalidateRect(m_window_handle, nullptr, FALSE);
        return true;
      }
  ...
  if (document != nullptr && m_minimap.is_point(layout, point_x, point_y)) {
    ...
    const std::optional<std::size_t> target = m_minimap.handle_pointer_press(
        layout, point_x, point_y, document->get_line_count(), visible_count,
        m_scrollbar.get_first_visible_line());
    if (target) {
      static_cast<void>(m_scrollbar.scroll_to(*target));
    }
  ...
  if (document != nullptr && m_max_text_scroll > 0.0F &&
      !m_text_hscrollbar_bounds.is_empty() &&
      m_text_hscrollbar_bounds.contains(point_x, point_y)) {
    m_dragging_text_hscrollbar = true;
    m_text_hscroll_drag_start_x = point_x;
    m_text_hscroll_drag_initial_offset = m_text_scroll_offset;
    m_focused = true;
    m_pointer_selecting = false;
    m_reveal_caret_pending = false;
    m_caret_blink.reset();

    const float track_w = m_text_hscrollbar_bounds.width;
    const float thumb_w =
        std::clamp(track_w * (track_w / (track_w + m_max_text_scroll)),
                   24.0F * surface.m_dpi_scale, track_w);
    const float thumb_x =
        m_text_hscrollbar_bounds.x +
        (m_text_scroll_offset / m_max_text_scroll) * (track_w - thumb_w);
    const UI::Rect current_thumb{thumb_x, m_text_hscrollbar_bounds.y, thumb_w,
                                 m_text_hscrollbar_bounds.height};

    if (!current_thumb.contains(point_x, point_y)) {
      const float rel_x =
          std::clamp(point_x - m_text_hscrollbar_bounds.x - thumb_w * 0.5F,
                     0.0F, track_w - thumb_w);
      const float frac =
          (track_w > thumb_w) ? (rel_x / (track_w - thumb_w)) : 0.0F;
      m_text_scroll_offset =
          std::clamp(frac * m_max_text_scroll, 0.0F, m_max_text_scroll);
      m_text_hscroll_drag_initial_offset = m_text_scroll_offset;
    }
    if (m_window_handle)
      InvalidateRect(m_window_handle, nullptr, FALSE);
  ...
  const UI::Editor::TextPosition position =
      position_from_point(surface, device_context, layout, point_x, point_y);
  static_cast<void>(
...
bool TextEditor::handle_pointer_move(
  ...
  if (is_media_fullscreen()) {
    const bool media_changed = m_media_player_view.handle_mouse_move(
        point_x, point_y, layout.workspace_bounds, layout.dpi_scale);
    if (media_changed) {
      if (m_window_handle)
        InvalidateRect(m_window_handle, nullptr, FALSE);
      return true;
  ...
  if (left_is_media && m_media_preview_mode &&
      left_pane.contains(point_x, point_y)) {
    const bool media_changed = m_media_player_view.handle_mouse_move(
        point_x, point_y, left_pane, layout.dpi_scale);
    if (media_changed) {
      if (m_window_handle)
        InvalidateRect(m_window_handle, nullptr, FALSE);
      return true;
             ...
             right_pane.contains(point_x, point_y)) {
    const bool media_changed = m_media_player_view.handle_mouse_move(
        point_x, point_y, right_pane, layout.dpi_scale);
    if (media_changed) {
      if (m_window_handle)
        InvalidateRect(m_window_handle, nullptr, FALSE);
      return true;
  ...
  bool hovered_text_hscrollbar = false;
  if (m_max_text_scroll > 0.0F && !m_text_hscrollbar_bounds.is_empty()) {
    const UI::Rect hit_bounds{
        m_text_hscrollbar_bounds.x,
        m_text_hscrollbar_bounds.y - 4.0F * layout.dpi_scale,
        m_text_hscrollbar_bounds.width,
        m_text_hscrollbar_bounds.height + 4.0F * layout.dpi_scale};
    hovered_text_hscrollbar = hit_bounds.contains(point_x, point_y);
  }
  if (hovered_text_hscrollbar != m_hovered_text_hscrollbar) {
    m_hovered_text_hscrollbar = hovered_text_hscrollbar;
    changed = true;
  }
  ...
  std::optional<std::size_t> hovered_tab_action;
  for (std::size_t i = 0; i < 5; ++i) {
    if (!m_tab_action_bounds[i].is_empty() &&
        m_tab_action_bounds[i].contains(point_x, point_y)) {
      hovered_tab_action = i;
      break;
    }
  }
  if (hovered_tab_action != m_hovered_tab_action) {
    m_hovered_tab_action = hovered_tab_action;
    changed = true;
  }

  bool split_close_hover =
  ...
  if (document != nullptr && !layout.editor_bounds.is_empty() &&
      layout.editor_bounds.height > 2.0F &&
      layout.editor_bounds.contains(point_x, point_y)) {
    const float scale = layout.dpi_scale;
    const float line_height =
        (m_cached_line_height > 0.0F) ? m_cached_line_height : (20.0F * scale);
    const bool is_split_active =
        m_is_split && m_split_document_index.has_value() &&
        *m_split_document_index < m_controller.get_documents().size();
    const float splitter_x =
    ...
    const UI::Editor::TextDocumentModel *target_doc =
        is_right_pane ? m_controller.get_document(*m_split_document_index)
                      : document;
    const auto &target_folding = is_right_pane ? m_split_folding : m_folding;
    const auto &target_scrollbar =
        is_right_pane ? m_split_scrollbar : m_scrollbar;
    ...
    if (target_doc != nullptr) {
      ...
      const float clamped_y =
          std::clamp(point_y, layout.editor_bounds.y,
                     std::max(layout.editor_bounds.bottom() - 1.0F,
                              layout.editor_bounds.y));
      const std::size_t row = static_cast<std::size_t>(std::max(
          static_cast<int>((clamped_y - layout.editor_bounds.y) / line_height),
          0));
      const std::size_t line_index = visual_row_to_physical_line(
          target_folding, first_line + row, total_lines);
      ...
      if (line_index < total_lines) {
        ...
        if (!diags.empty()) {
          const float pane_left = is_right_pane ? (splitter_x + 2.0F * scale)
                                                : layout.editor_bounds.x;
          const float code_x =
              pane_left + layout.gutter_bounds.width + 14.0F * scale;
          const float line_top_y =
              layout.editor_bounds.y + static_cast<float>(row) * line_height;
          const float line_bottom_y = line_top_y + line_height;

          if (point_y >= line_top_y - 2.0F * scale &&
              point_y <= line_bottom_y + 4.0F * scale &&
              point_x >= code_x - 10.0F * scale) {
            for (const auto &d : diags) {
              ...
              if (open_angle != std::string_view::npos &&
                  close_angle != std::string_view::npos &&
                  close_angle > open_angle) {
                token_symbol = std::string(line_str.substr(
                    open_angle + 1, close_angle - open_angle - 1));
              } else {
                auto open_q = line_str.find('"');
                auto close_q = line_str.rfind('"');
                if (open_q != std::string_view::npos &&
                    close_q != std::string_view::npos && close_q > open_q) {
                  token_symbol = std::string(
                      line_str.substr(open_q + 1, close_q - open_q - 1));
                }
              }
  ...
  if (hovered_diag.has_value() != m_hovered_diagnostic.has_value() ||
      (hovered_diag.has_value() && m_hovered_diagnostic.has_value() &&
       (hovered_diag->diagnostic.message !=
            m_hovered_diagnostic->diagnostic.message ||
        hovered_diag->anchor_y != m_hovered_diagnostic->anchor_y))) {
    m_hovered_diagnostic = hovered_diag;
    changed = true;
  }

  if (!hovered_diag.has_value() && document != nullptr &&
      !layout.editor_bounds.is_empty() && layout.editor_bounds.height > 2.0F &&
      layout.editor_bounds.contains(point_x, point_y)) {
    const float scale = layout.dpi_scale;
    const float line_height =
        (m_cached_line_height > 0.0F) ? m_cached_line_height : (20.0F * scale);
    ...
    if (target_doc != nullptr) {
      ...
      if (line_index < total_lines) {
        ...
        if (rel_x >= 0.0F && !line_str.empty()) {
          ...
          const std::size_t col = std::min<std::size_t>(
              static_cast<std::size_t>(rel_x / approx_char_w),
              line_str.size() - 1);

          if (std::isalnum(static_cast<unsigned char>(line_str[col])) ||
              line_str[col] == '_') {
            std::size_t w_start = col;
            while (w_start > 0 && (std::isalnum(static_cast<unsigned char>(
                                       line_str[w_start - 1])) ||
                                   line_str[w_start - 1] == '_')) {
              --w_start;
            }
            std::size_t w_end = col;
            while (w_end < line_str.size() &&
                   (std::isalnum(static_cast<unsigned char>(line_str[w_end])) ||
                    line_str[w_end] == '_')) {
              ++w_end;
            }
            const std::string current_word =
                std::string(line_str.substr(w_start, w_end - w_start));
            if (!current_word.empty() &&
                (!m_hovered_token_pos ||
                 m_hovered_token_pos->line != line_index ||
                 m_last_hovered_word != current_word)) {
              m_hovered_token_pos = UI::Editor::TextPosition{line_index, col};
              m_last_hovered_word = current_word;
              m_hover_screen_x = point_x;
              m_hover_screen_y = layout.editor_bounds.y +
              ...
              const std::string uri = get_active_document_uri();
              const std::string fname = get_active_document_filename();
              const Language::Protocol::Position pos{.line = line_index,
                                                     .character = col};

              Language::LanguageServerManager::instance().request_hover(
                  uri, fname, pos, std::string(line_str),
                  [this, expected_word = current_word,
                   screen_x = m_hover_screen_x, screen_y = m_hover_screen_y](
                      std::optional<Language::Protocol::Hover> hover_res) {
                    if (hover_res && !hover_res->contents.empty() &&
                        m_last_hovered_word == expected_word) {
                      m_hover_tooltip.show(hover_res->contents, screen_x,
                                           screen_y);
                      if (m_window_handle != nullptr) {
                        InvalidateRect(m_window_handle, nullptr, FALSE);
                      }
  ...
  if (is_ctrl_pressed && document != nullptr &&
    ...
    if (target_doc != nullptr) {
      ...
      if (line_index < total_lines) {
        ...
        const float code_x = is_right_pane
                                 ? (splitter_x + 2.0F * scale +
                                    layout.gutter_bounds.width + 14.0F * scale)
                                 : (layout.editor_bounds.x + 14.0F * scale -
                                    m_text_scroll_offset);
        const float rel_x = point_x - code_x;

        if (rel_x >= 0.0F && !line_str.empty()) {
          const float char_w = (m_cached_char_width > 0.0F)
                                   ? m_cached_char_width
                                   : (8.0F * scale);
          const std::size_t col = std::min<std::size_t>(
              static_cast<std::size_t>(rel_x / char_w), line_str.size() - 1);
          const auto [start_col, end_col] = Language::Definition::
              SymbolDefinitionResolver::extract_symbol_range(line_str, col);
          if (end_col > start_col) {
            ...
            info.symbol =
                std::string(line_str.substr(start_col, end_col - start_col));
...
bool TextEditor::handle_pointer_drag(
  ...
  if (is_media_active() && m_media_preview_mode &&
    ...
    const float scale = surface.m_dpi_scale;
    if (is_media_fullscreen()) {
      if (m_media_player_view.handle_mouse_move(
              point_x, point_y, layout.workspace_bounds, scale)) {
        if (m_window_handle) {
          InvalidateRect(m_window_handle, nullptr, FALSE);
    ...
    const UI::Rect active_pane =
        (is_split_active && m_focused_pane == SplitPaneFocus::Right)
            ? right_pane
            : left_pane;
    if (m_media_player_view.handle_mouse_move(point_x, point_y, active_pane,
                                              scale)) {
      if (m_window_handle) {
        InvalidateRect(m_window_handle, nullptr, FALSE);
        UpdateWindow(m_window_handle);
  ...
  if (m_is_resizing_split) {
    ...
    m_split_ratio =
        std::clamp(rel_x / layout.editor_bounds.width, 0.15F, 0.85F);
    if (m_window_handle)
      InvalidateRect(m_window_handle, nullptr, FALSE);
    return true;
  }
  ...
  if (m_dragging_text_hscrollbar) {
    const float track_w = m_text_hscrollbar_bounds.width;
    const float thumb_w =
        std::clamp(track_w * (track_w / (track_w + m_max_text_scroll)),
                   24.0F * layout.dpi_scale, track_w);
    const float track_space = track_w - thumb_w;

    if (track_space > 0.0F && m_max_text_scroll > 0.0F) {
      const float delta_x = point_x - m_text_hscroll_drag_start_x;
      const float scroll_delta = (delta_x / track_space) * m_max_text_scroll;
      m_text_scroll_offset = m_text_hscroll_drag_initial_offset + scroll_delta;
      m_text_scroll_offset =
  ...
  if (point_x < layout.editor_bounds.x) {
    ...
    m_text_scroll_offset =
        std::min(m_max_text_scroll, m_text_scroll_offset + 25.0F * scale);
  }
...
bool TextEditor::handle_pointer_release() noexcept {
  ...
  if (m_dragging_text_hscrollbar) {
    m_dragging_text_hscrollbar = false;
    if (m_window_handle)
      InvalidateRect(m_window_handle, nullptr, FALSE);
    return true;
  }
  ...
  if (m_tab_drag_drop.is_dragging()) {
    if (m_active_drop_zone == SplitDropZone::Right ||
      ...
      if (m_controller.get_active_index() == dragged_idx &&
          m_controller.get_documents().size() > 1) {
      ...
      std::clog
          << "[ZDE] Split Editor activated via drag-and-drop with file index: "
          << dragged_idx << "\n";
    }
    m_active_drop_zone = SplitDropZone::None;
  ...
  const bool minimap_was_dragging = m_minimap.handle_pointer_release() ||
                                    m_split_minimap.handle_pointer_release();
  const bool scrollbar_was_dragging =
      m_scrollbar.handle_pointer_release() ||
      m_split_scrollbar.handle_pointer_release();
...
bool TextEditor::handle_scroll(
  ...
  // Tab bar scroll isolation: horizontal or vertical mouse wheel over tabs
  // scrolls tab bar only
  if (layout.tab_bar_bounds.contains(event.point_x, event.point_y)) {
    ...
    if (delta != 0.0f) {
      ...
      if (m_tab_scroll_offset < 0.0f)
        m_tab_scroll_offset = 0.0f;
      if (m_tab_scroll_offset > m_max_tab_scroll)
  ...
  if (is_media_fullscreen()) {
    if (event.delta_y != 0) {
      const float new_vol = std::clamp(m_media_player_view.volume() +
                                           (event.delta_y > 0 ? 0.05f : -0.05f),
                                       0.0f, 1.0f);
  ...
  if (left_is_media && m_media_preview_mode &&
      left_pane.contains(event.point_x, event.point_y)) {
    if (event.delta_y != 0) {
      const float new_vol = std::clamp(m_media_player_view.volume() +
                                           (event.delta_y > 0 ? 0.05f : -0.05f),
                                       0.0f, 1.0f);
    ...
    return false;
             ...
             right_pane.contains(event.point_x, event.point_y)) {
    if (event.delta_y != 0) {
      const float new_vol = std::clamp(m_media_player_view.volume() +
                                           (event.delta_y > 0 ? 0.05f : -0.05f),
                                       0.0f, 1.0f);
  ...
  if (event.delta_x != 0) {
    if (layout.editor_bounds.contains(event.point_x, event.point_y)) {
      ...
      if (m_text_scroll_offset < 0.0f)
        m_text_scroll_offset = 0.0f;
      if (m_text_scroll_offset > m_max_text_scroll)
        m_text_scroll_offset = m_max_text_scroll;
      return true;
    }
    return false;
  }
  ...
  if (m_completion_popup.is_visible() &&
      m_completion_popup.is_point_inside(event.point_x, event.point_y,
  ...
  if (layout.editor_bounds.contains(event.point_x, event.point_y) ||
    ...
    if (is_split_active && event.point_x > splitter_x) {
      if (const UI::Editor::TextDocumentModel *split_doc =
        ...
        const std::size_t visible_count = static_cast<std::size_t>(std::max(
            static_cast<int>(layout.editor_bounds.height / line_height), 1));
        m_split_scrollbar.synchronize(split_doc->get_line_count(),
                                      visible_count);
...
bool TextEditor::handle_input(UI::Editor::EditorInputCommand command,
  ...
  if (is_media_active() && m_media_preview_mode) {
    if (command == UI::Editor::EditorInputCommand::MoveLeft) {
      m_media_player_view.seek_relative(-5.0);
      if (m_window_handle)
        InvalidateRect(m_window_handle, nullptr, FALSE);
      return true;
    }
    if (command == UI::Editor::EditorInputCommand::MoveRight) {
      m_media_player_view.seek_relative(5.0);
      if (m_window_handle)
        InvalidateRect(m_window_handle, nullptr, FALSE);
      return true;
      ...
      if (command == UI::Editor::EditorInputCommand::InsertTab ||
        ...
        if (const auto *item = m_completion_popup.get_selected_item()) {
          if (auto *doc = m_controller.get_active_document(); doc != nullptr) {
            ...
            const std::string fname = get_active_document_filename();
            const bool is_html_or_emmet = fname.ends_with(".html") ||
                                          fname.ends_with(".htm") ||
                                          fname.ends_with(".xhtml");
            const bool is_include_line =
                current_line.find("#include") != std::string_view::npos ||
                current_line.find("#import") != std::string_view::npos;
            ...
            // Avoid double delimiters (e.g. if closing > or " already exists
            // after caret)
            if (caret_col < current_line.size()) {
              const char after_caret = current_line[caret_col];
              if (after_caret == '>' && text_to_insert.ends_with('>')) {
                text_to_insert.pop_back();
              } else if (after_caret == '"' && text_to_insert.ends_with('"')) {
                text_to_insert.pop_back();
              }
            }
  ...
  if (m_focused && m_input_router.is_vim_active()) {
    auto *doc = get_focused_document();
    if (doc != nullptr) {
      if (command == UI::Editor::EditorInputCommand::Escape) {
...
std::optional<bool> TextEditor::handle_command(std::string_view command_id) {
  if (command_id == Commands::CommandIds::view_split_right ||
      command_id == "zde.editor.split_right" ||
  ...
  if (command_id == Commands::CommandIds::window_next_tab ||
      command_id == "zde.editor.nextTab") {
    const auto doc_count = m_controller.get_documents().size();
    if (doc_count > 1) {
      const std::size_t active = m_controller.get_active_index().value_or(0);
  ...
  if (command_id == Commands::CommandIds::window_prev_tab ||
      command_id == "zde.editor.prevTab") {
    const auto doc_count = m_controller.get_documents().size();
    if (doc_count > 1) {
      const std::size_t active = m_controller.get_active_index().value_or(0);
  ...
  if (command_id == Commands::CommandIds::file_close_all ||
      command_id == "zde.editor.closeAll" ||
...
TextEditor::is_command_enabled(std::string_view command_id) const noexcept {
  if (command_id == Commands::CommandIds::view_split_right ||
      ...
      command_id == "zde.editor.focus_first_group" ||
...
bool TextEditor::handle_text_input(std::string_view utf8_text) {
  ...
  if (is_media_active() && m_media_preview_mode) {
    if (utf8_text == " ") {
      m_media_player_view.toggle_play_pause();
      if (m_window_handle)
        InvalidateRect(m_window_handle, nullptr, FALSE);
      return true;
    }
  ...
  if (changed) {
...

    {
      ...
      const std::size_t num_lines = doc->get_line_count();
      for (std::size_t i = 0; i < num_lines; ++i) {
        total_len += doc->get_line(i).size() + 1;
      }
      content.reserve(total_len);
      for (std::size_t i = 0; i < num_lines; ++i) {
        content.append(doc->get_line(i));
        content.push_back('\n');
      }
      ...
      const bool is_trigger_char =
          utf8_text == "." || utf8_text == ">" || utf8_text == ":" ||
          utf8_text == "/" || utf8_text == "\\" || utf8_text == "(" ||
          utf8_text == "," || is_include_context || current_word.size() >= 1;
          ...
          if (open_pos != std::string_view::npos && open_pos + 1 < caret_col) {
            ...
            if (first_char != '/' && first_char != '!' && first_char != '?') {
              ...
              while (idx < caret_col - 1 &&
                     !std::isspace(
                         static_cast<unsigned char>(current_line[idx])) &&
            ...
            [this](std::optional<Language::Protocol::SignatureHelp> help) {
              ...
              if (const auto *current_doc =
                ...
                const std::string_view line =
                    current_doc->get_line(current_doc->get_caret_line());
                const std::size_t col = current_doc->get_caret_column();
                const std::string_view pfx =
                    line.substr(0, std::min(col, line.size()));
                std::size_t op = 0;
                std::size_t cp = 0;
                for (char ch : pfx) {
                  if (ch == '(')
      ...
      if (utf8_text == " " || utf8_text == ";" || utf8_text == ")" ||
        ...
        Language::Protocol::Position pos{.line = doc->get_caret_line(),
                                         .character = doc->get_caret_column()};
        const bool is_html_file = fname.ends_with(".html") ||
                                  fname.ends_with(".htm") ||
                                  fname.ends_with(".xhtml");
        const bool is_actual_trigger =
            (utf8_text == "." || utf8_text == ">" || utf8_text == ":" ||
             utf8_text == "/" || utf8_text == "\\" || utf8_text == "<" ||
             utf8_text == "\"" || utf8_text == "#" ||
             (is_html_file && (utf8_text == "!" || utf8_text == "+" ||
                               utf8_text == "*" || utf8_text == "$")));
              ...
              if (!items.empty()) {
                ...
                if (const auto *current_doc =
                  ...
                  const std::string current_fname =
                      get_active_document_filename();
                  const bool is_html = current_fname.ends_with(".html") ||
                                       current_fname.ends_with(".htm") ||
                                       current_fname.ends_with(".xhtml");
                  const bool is_inc_line =
                      line.find("#include") != std::string_view::npos ||
                      line.find("#import") != std::string_view::npos;
                  std::size_t start = std::min(col, line.size());
          ...
          if (!items.empty()) {
            ...
            if (const auto *current_doc = m_controller.get_active_document()) {
              const std::string_view line =
                  current_doc->get_line(current_doc->get_caret_line());
              const std::size_t col = current_doc->get_caret_column();
              const std::string current_fname = get_active_document_filename();
              const bool is_html = current_fname.ends_with(".html") ||
                                   current_fname.ends_with(".htm") ||
                                   current_fname.ends_with(".xhtml");
              const bool is_inc_line =
...
bool TextEditor::is_minimap_point(
  ...
  if (m_is_split && m_split_document_index.has_value() &&
    ...
    return (!left_minimap.is_empty() &&
            left_minimap.contains(point_x, point_y)) ||
           (!right_minimap.is_empty() &&
            right_minimap.contains(point_x, point_y));
  }
...
bool TextEditor::tick_animations() noexcept {
  ...
  if (const UI::Editor::TextDocumentModel *doc =
    ...
    if (m_last_brace_caret != current_caret) {
      ...
      auto [open_brace, close_brace, pulsing_brace] =
          find_enclosing_braces(*doc);
  ...
  if (m_is_split && m_split_document_index.has_value() &&
    ...
    if (const UI::Editor::TextDocumentModel *split_doc =
            m_controller.get_document(*m_split_document_index)) {
      UI::Editor::TextPosition current_split_caret{
          split_doc->get_caret_line(), split_doc->get_caret_column()};
      if (m_split_last_brace_caret != current_split_caret) {
        m_split_last_brace_caret = current_split_caret;
        auto [open_brace, close_brace, pulsing_brace] =
            find_enclosing_braces(*split_doc);
...
void TextEditor::render(
  ...
  if (!is_split_active) {
    if (left_doc != nullptr && !(left_is_media && m_media_preview_mode)) {
      ...
      const std::size_t visible_count = static_cast<std::size_t>(std::max(
          static_cast<int>(layout.editor_bounds.height / line_height), 1));
      m_minimap.render(surface, device_context, layout, *left_doc,
    ...
    // --- Left Pane Minimap & Scrollbar (only if not media preview) ---
    const float scroll_top_y = layout.editor_bounds.y;
    const float scroll_total_h = layout.editor_bounds.height;
    const float scrollbar_w = FIXED_SCROLLBAR_WIDTH * scale;
    const UI::Rect left_bounds{layout.editor_bounds.x, scroll_top_y,
    ...
    if (left_doc != nullptr && !(left_is_media && m_media_preview_mode)) {
      ...
      if (!left_minimap.is_empty()) {
        m_minimap.render(surface, device_context, left_layout, *left_doc,
                         m_scrollbar.get_first_visible_line(), visible_count);
      }
    }

    // --- Right Pane Minimap & Scrollbar (only if not media preview) ---
    const UI::Rect right_bounds{splitter_x + 2.0F * scale, scroll_top_y,
                                layout.editor_bounds.right() -
                                    (splitter_x + 2.0F * scale),
                                scroll_total_h};
    ...
    if (const UI::Editor::TextDocumentModel *right_doc =
            m_controller.get_document(*m_split_document_index)) {
      const bool right_is_media = UI::Editor::MediaPlayerView::is_media_file(
          std::string(right_doc->get_file_name()));
    ...
    // Draw continuous split divider line on TOP of minimap and scrollbar (full
    // height from tab bar to editor bottom)
    const bool split_highlight = m_is_resizing_split || m_hovered_split_resize;
    const UI::Theme::Color divider_col =
        split_highlight ? surface.m_palette.accent : surface.m_palette.border;
    const float divider_top_y = (!layout.editor_header_bounds.is_empty() &&
                                 layout.editor_header_bounds.height > 2.0F)
...
void TextEditor::draw_tab_strip(
  ...
  SaveDC(device_context);
  IntersectClipRect(device_context, static_cast<int>(layout.tab_bar_bounds.x),
  ...
  for (std::size_t index = 0; index < documents.size(); ++index) {
    ...
    const float offset_x = m_tab_animated_offset_x.contains(id)
                               ? m_tab_animated_offset_x[id]
                               : 0.0f;
  ...
  if (m_tab_animated_offset_x.size() > documents.size() + 8) {
    ...
    std::erase_if(m_tab_animated_offset_x, [&](const auto &item) {
  ...
  auto draw_single_tab = [&](std::size_t tab_index) {
    ...
    if (is_modern) {
      const float v_pad = std::round(4.0F * surface.m_dpi_scale);
      const float h_pad = std::round(2.0F * surface.m_dpi_scale);
      const UI::Rect pill_bounds{
          bounds.x + h_pad,
          bounds.y + v_pad,
          std::max(0.0F, bounds.width - h_pad * 2.0F),
          std::max(0.0F, bounds.height - v_pad * 2.0F),
      };
      const float pill_radius =
          std::max(std::round(6.0F * surface.m_dpi_scale), 5.0F);

      if (is_dragging_this) {
        const UI::Rect shadow_rect{pill_bounds.x + 2.0F * surface.m_dpi_scale,
                                   pill_bounds.y + 2.0F * surface.m_dpi_scale,
                                   pill_bounds.width, pill_bounds.height};
        surface.fill_rounded_rectangle(device_context, shadow_rect,
                                       UI::Theme::Color{0, 0, 0, 90},
                                       pill_radius);
        surface.fill_rounded_rectangle(device_context, pill_bounds,
                                       UI::Theme::Color{45, 48, 56, 255},
                                       pill_radius);
        surface.draw_rounded_rectangle(device_context, pill_bounds,
                                       UI::Theme::Color{53, 132, 228, 230},
                                       pill_radius);
      } else if (active) {
        // JetBrains New UI active tab: solid dark pill floating on titlebar
        const UI::Theme::Color active_bg = surface.m_theme.is_dark
            ? UI::Theme::Color{36, 38, 44, 255}
            : UI::Theme::Color{255, 255, 255, 255};
        surface.fill_rounded_rectangle(device_context, pill_bounds,
                                       active_bg, pill_radius);
        // Subtle outline border
        const UI::Theme::Color border_col = surface.m_theme.is_dark
            ? UI::Theme::Color{68, 72, 84, 255}
            : UI::Theme::Color{210, 213, 220, 255};
        surface.draw_rounded_rectangle(device_context, pill_bounds,
                                       border_col, pill_radius);
      } else if (tab_hovered) {
        // JetBrains inactive hover pill
        const UI::Theme::Color hover_bg = surface.m_theme.is_dark
            ? UI::Theme::Color{255, 255, 255, 20}
            : UI::Theme::Color{0, 0, 0, 15};
        surface.fill_rounded_rectangle(device_context, pill_bounds,
                                       hover_bg, pill_radius);
      }
    } else {
      if (is_dragging_this) {
        const UI::Rect shadow_rect{bounds.x + 2.0F * surface.m_dpi_scale,
                                   bounds.y + 2.0F * surface.m_dpi_scale,
      ...
      surface.fill_rectangle(
          device_context, bounds,
          is_dragging_this
              ? UI::Theme::Color{38, 42, 50, 255}
              : (active ? surface.m_palette.tab_active_background
                        : (tab_hovered ? surface.m_palette.active_line_background
                                       : surface.m_palette.tab_background)));

      const UI::Theme::Color tab_edge_color =
          is_dragging_this ? UI::Theme::Color{53, 132, 228, 230}
                           : surface.m_palette.border;
      ...
      if (active && !is_dragging_this) {
        ...
        surface.fill_rectangle(
            device_context,
            UI::Rect{bounds.x, bounds.y, bounds.width,
        ...
        surface.draw_line(device_context, tab_left, tab_top, tab_left, tab_bottom,
                          tab_edge_color);
        surface.draw_line(device_context, tab_right, tab_top, tab_right,
                          tab_bottom, tab_edge_color);
      } else {
        ...
        surface.draw_line(device_context, tab_right, tab_top, tab_right,
                          tab_bottom, tab_edge_color);
        surface.draw_line(device_context, tab_left, tab_bottom, tab_right,
                          tab_bottom, tab_edge_color);
      }
    }

    const UI::Theme::Color tab_icon_bg = is_modern
        ? (active ? UI::Theme::Color{36, 38, 44, 255} : surface.m_theme.titlebar_background)
        : surface.m_palette.tab_background;
    ...
    surface.draw_svg_icon(
        ...
        surface.m_palette.text_primary, tab_icon_bg, true);
    const float label_x =
        bounds.x + UI::Editor::StudioEditorMetrics::editor_tab_label_offset *
                       surface.m_dpi_scale;
    const float max_label_width =
        bounds.right() - label_x -
        (UI::Editor::StudioEditorMetrics::editor_tab_close_width + 4.0F) *
            surface.m_dpi_scale;

    std::string display_name = std::string(document.get_file_name());
    if (max_label_width > 0.0F &&
        surface.get_text_width(device_context, *surface.m_ui_font,
                               display_name) > max_label_width) {
    ...
    surface.draw_text(device_context, *surface.m_ui_font, display_name, label_x,
  ...
  if (!is_modern) {
    ...
    if (active_index && *active_index < m_tab_count) {
      ...
      surface.draw_line(device_context, tab_bar_left, tab_bar_bottom, active_left,
                        tab_bar_bottom, surface.m_palette.border);
      surface.draw_line(device_context, active_right, tab_bar_bottom,
                        tab_bar_right, tab_bar_bottom, surface.m_palette.border);
    } else {
      surface.draw_line(device_context, tab_bar_left, tab_bar_bottom,
                        tab_bar_right, tab_bar_bottom, surface.m_palette.border);
    }
  }
  ...
  if (m_max_tab_scroll > 0.0f) {
    ...
    const UI::Theme::Color thumb_color = m_hovered_tab_scrollbar
                                             ? surface.m_palette.accent
                                             : surface.m_palette.text_muted;
...
void TextEditor::draw_media_pane(const StudioWorkspaceRenderer &surface,
                                 HDC device_context,
                                 const UI::Rect &pane_bounds,
                                 const std::filesystem::path &doc_path) const {
  if (pane_bounds.is_empty() || pane_bounds.width <= 4.0F ||
      pane_bounds.height <= 4.0F) {
    return;
  }

  if (m_media_player_view.current_path() != doc_path) {
    const_cast<TextEditor *>(this)->m_media_player_view.open(doc_path);
  }

  const_cast<TextEditor *>(this)->m_media_player_view.update();
  const float scale = surface.m_dpi_scale;

  // 1. Fill entire pane seamlessly with editor background
  surface.fill_rectangle(device_context, pane_bounds,
                         surface.m_palette.editor_background);
  ...
  if (m_media_player_view.has_error()) {
    ...
    const float card_w =
        std::min(480.0F * scale, pane_bounds.width - 40.0F * scale);
    const float card_h = 160.0F * scale;
    const UI::Rect card_rect{center_x - card_w * 0.5F, center_y - card_h * 0.5F,
                             card_w, card_h};

    surface.fill_rounded_rectangle(device_context, card_rect,
                                   UI::Theme::Color{28, 22, 24, 240},
                                   8.0F * scale);
    surface.draw_rectangle(device_context, card_rect,
                           UI::Theme::Color{180, 50, 60, 180});
    ...
    if (surface.m_large_font) {
      ...
      surface.draw_text(device_context, *surface.m_large_font, title,
                        center_x - static_cast<float>(tw) * 0.5F,
                        card_rect.y + 24.0F * scale,
                        UI::Theme::Color{245, 90, 100, 255});
    }

    std::string err = m_media_player_view.error_message();
    if (err.empty())
      err = "Unable to decode or render media stream.";
    if (surface.m_ui_font) {
      const int ew = surface.m_ui_font->getTextWidth(device_context, err);
      surface.draw_text(device_context, *surface.m_ui_font, err,
                        center_x - static_cast<float>(ew) * 0.5F,
                        card_rect.y + 60.0F * scale,
                        surface.m_palette.text_muted);
    }
    ...
    const UI::Rect btn_rect{center_x - btn_w * 0.5F,
                            card_rect.y + 105.0F * scale, btn_w, btn_h};
    surface.fill_rounded_rectangle(device_context, btn_rect,
                                   surface.m_palette.accent, 4.0F * scale);
    if (surface.m_ui_font) {
      const std::string retry_text = "Reload Media";
      const int rw =
          surface.m_ui_font->getTextWidth(device_context, retry_text);
      surface.draw_text(device_context, *surface.m_ui_font, retry_text,
                        center_x - static_cast<float>(rw) * 0.5F,
                        btn_rect.y + btn_h * 0.5F,
                        UI::Theme::Color{255, 255, 255, 255});
    }
    return;
  }
  ...
  if (is_audio) {
    ...
    const UI::Rect icon_rect{center_x - icon_sz * 0.5F,
                             center_y - 100.0F * scale, icon_sz, icon_sz};
    surface.fill_rounded_rectangle(device_context, icon_rect,
                                   surface.m_palette.accent, 14.0F * scale);
    ...
    surface.draw_svg_icon(device_context, "music.svg", note_center_x,
                          note_center_y, round_to_int(28.0F * scale),
                          UI::Theme::Color{255, 255, 255, 255},
    ...
    for (int b = 0; b < total_bars; ++b) {
      ...
      UI::Theme::Color bar_col = m_media_player_view.is_playing()
                                     ? surface.m_palette.accent
                                     : UI::Theme::Color{80, 85, 105, 180};
      surface.fill_rounded_rectangle(device_context,
                                     UI::Rect{bx, by, bar_w, bar_h}, bar_col,
                                     2.0F * scale);
    }

    // Centered Title
    const std::string filename_str = doc_path.filename().string();
    if (surface.m_large_font) {
      const int title_w =
          surface.m_large_font->getTextWidth(device_context, filename_str);
      surface.draw_text(device_context, *surface.m_large_font, filename_str,
                        center_x - static_cast<float>(title_w) * 0.5F,
                        center_y + 16.0F * scale,
                        surface.m_palette.text_primary);
    } else if (surface.m_ui_font) {
      const int title_w =
          surface.m_ui_font->getTextWidth(device_context, filename_str);
      surface.draw_text(device_context, *surface.m_ui_font, filename_str,
                        center_x - static_cast<float>(title_w) * 0.5F,
                        center_y + 16.0F * scale,
                        surface.m_palette.text_primary);
    }
    ...
    // Centered Info
    const std::string ext_str = doc_path.extension().string();
    const std::string audio_info =
        "Audio Stream • " + (ext_str.empty() ? "Audio" : ext_str.substr(1)) +
        " • " + m_media_player_view.format_time_display();
    if (surface.m_small_font) {
      const int info_w =
          surface.m_small_font->getTextWidth(device_context, audio_info);
      surface.draw_text(device_context, *surface.m_small_font, audio_info,
                        center_x - static_cast<float>(info_w) * 0.5F,
                        center_y + 40.0F * scale, surface.m_palette.text_muted);
    }

    // Centered Scrubber Track
    const float max_track_w =
        std::min(480.0F * scale, pane_bounds.width - 40.0F * scale);
    const float track_x = center_x - max_track_w * 0.5F;
    const float track_w = max_track_w;
    const float track_h = 4.0F * scale;
    const float track_y = center_y + 70.0F * scale;
    const UI::Rect track_rect{track_x, track_y, track_w, track_h};
    surface.fill_rounded_rectangle(device_context, track_rect,
                                   UI::Theme::Color{65, 70, 85, 220},
                                   2.0F * scale);

    const float progress = m_media_player_view.progress_ratio();
    if (progress > 0.0F) {
      const UI::Rect prog_rect{track_rect.x, track_rect.y,
                               track_rect.width * progress, track_h};
      surface.fill_rounded_rectangle(device_context, prog_rect,
                                     surface.m_palette.accent, 2.0F * scale);

      const float knob_x = track_rect.x + track_rect.width * progress;
      const float knob_y = track_y + track_h * 0.5F;
      const float knob_r = (m_media_player_view.is_scrubber_hovered() ||
                            m_media_player_view.is_scrubbing())
                               ? (6.0F * scale)
                               : (4.5F * scale);
      const UI::Rect knob_rect{knob_x - knob_r, knob_y - knob_r, knob_r * 2.0F,
                               knob_r * 2.0F};
      surface.fill_rounded_rectangle(device_context, knob_rect,
                                     UI::Theme::Color{255, 255, 255, 255},
                                     knob_r);
    }

    // Centered Controls Row
    const float ctrl_y = center_y + 95.0F * scale;
    const float play_btn_sz = 36.0F * scale;
    const UI::Rect play_btn{center_x - play_btn_sz * 0.5F, ctrl_y, play_btn_sz,
                            play_btn_sz};
    if (m_media_player_view.is_play_hovered()) {
      surface.fill_rounded_rectangle(device_context, play_btn,
                                     UI::Theme::Color{255, 255, 255, 30},
                                     8.0F * scale);
    } else {
      surface.fill_rounded_rectangle(device_context, play_btn,
                                     UI::Theme::Color{255, 255, 255, 15},
                                     8.0F * scale);
    }
    const std::string_view play_icon_asset =
        m_media_player_view.is_playing() ? "debug-pause.svg" : "play.svg";
    ...
    surface.draw_svg_icon(device_context, play_icon_asset, play_center_x,
                          play_center_y, play_icon_sz,
                          UI::Theme::Color{255, 255, 255, 255},
                          UI::Theme::Color{0, 0, 0, 0}, false);

    const std::string time_text = m_media_player_view.format_time_display();
    if (surface.m_small_font) {
      surface.draw_text(device_context, *surface.m_small_font, time_text,
                        track_x, ctrl_y + play_btn_sz * 0.5F,
                        UI::Theme::Color{255, 255, 255, 240});
    }

    // Volume Capsule on the right
    const float vol_w = std::min(90.0F * scale, max_track_w * 0.3F);
    const float vol_h = 24.0F * scale;
    const UI::Rect vol_rect{track_x + track_w - vol_w,
                            ctrl_y + (play_btn_sz - vol_h) * 0.5F, vol_w,
                            vol_h};
    surface.fill_rounded_rectangle(device_context, vol_rect,
                                   UI::Theme::Color{35, 40, 52, 220},
                                   10.0F * scale);
    const std::string_view vol_icon_asset =
        m_media_player_view.is_muted() ? "mute.svg" : "unmute.svg";
    ...
    surface.draw_svg_icon(device_context, vol_icon_asset, vol_center_x,
                          vol_center_y, vol_icon_sz,
                          UI::Theme::Color{255, 255, 255, 220},
                          UI::Theme::Color{0, 0, 0, 0}, false);
    ...
    const UI::Rect vol_track{
        vol_track_x, vol_rect.y + (vol_rect.height - vol_track_h) * 0.5F,
        vol_track_w, vol_track_h};
    surface.fill_rounded_rectangle(device_context, vol_track,
                                   UI::Theme::Color{90, 95, 110, 200},
                                   1.5F * scale);
    const float vol_val =
        m_media_player_view.is_muted() ? 0.0F : m_media_player_view.volume();
    const UI::Rect vol_fill{vol_track.x, vol_track.y, vol_track.width * vol_val,
                            vol_track_h};
    surface.fill_rounded_rectangle(device_context, vol_fill,
                                   UI::Theme::Color{255, 255, 255, 255},
                                   1.5F * scale);
  ...
  const UI::Rect canvas_rect =
      m_media_player_view.calculate_video_canvas_bounds(pane_bounds);
  ...
  if (frame != nullptr && !frame->data.empty() && frame->width > 0 &&
      frame->height > 0) {
    BITMAPINFO bmi{};
    ...
    SaveDC(device_context);
    IntersectClipRect(device_context, round_to_int(pane_bounds.x),
                      round_to_int(pane_bounds.y),
                      round_to_int(pane_bounds.right()),
                      round_to_int(pane_bounds.bottom()));

    // Use COLORONCOLOR to preserve crisp pixel contrast and 100% sharpness
    // without GDI halftone blurring
    SetStretchBltMode(device_context, COLORONCOLOR);

    StretchDIBits(device_context, round_to_int(canvas_rect.x),
                  round_to_int(canvas_rect.y), round_to_int(canvas_rect.width),
                  round_to_int(canvas_rect.height), 0, 0, frame->width,
                  frame->height, frame->data.data(), &bmi, DIB_RGB_COLORS,
                  SRCCOPY);

    RestoreDC(device_context, -1);
  } else {
    // Centered stream status text
    if (surface.m_ui_font) {
      const std::string badge = m_media_player_view.is_open()
                                    ? "ZDE Media Stream Loading..."
                                    : "Opening stream...";
      const int badge_w =
          surface.m_ui_font->getTextWidth(device_context, badge);
      surface.draw_text(device_context, *surface.m_ui_font, badge,
                        pane_bounds.x +
                            (pane_bounds.width - static_cast<float>(badge_w)) *
                                0.5F,
                        pane_bounds.y + pane_bounds.height * 0.45F,
                        surface.m_palette.text_muted);
    }
  }

  // Render Browser HTML5-Style Minimalist Floating Bottom HUD Overlay docked to
  // video pane
  ...
  auto fade_col = [hud_alpha](UI::Theme::Color c) -> UI::Theme::Color {
    ...
    return UI::Theme::Color{
        static_cast<std::uint8_t>(
            std::clamp(round_to_int(static_cast<float>(c.red) * a), 0, 255)),
        static_cast<std::uint8_t>(
            std::clamp(round_to_int(static_cast<float>(c.green) * a), 0, 255)),
        static_cast<std::uint8_t>(
            std::clamp(round_to_int(static_cast<float>(c.blue) * a), 0, 255)),
        static_cast<std::uint8_t>(
            std::clamp(round_to_int(static_cast<float>(c.alpha) * a), 0, 255))};
  };

  // Render Browser HTML5-Style Minimalist Floating Bottom HUD Overlay docked to
  // video pane
  const UI::Rect hud =
      m_media_player_view.calculate_hud_bounds(pane_bounds, scale);

  // Sleek transparent floating glass background (video shines through cleanly)
  surface.fill_rounded_rectangle(device_context, hud,
                                 fade_col(UI::Theme::Color{12, 15, 22, 115}),
                                 10.0F * scale);

  // Play / Pause Button (▶ / ❚❚)
  const UI::Rect play_btn =
      m_media_player_view.calculate_play_button_bounds(hud, scale);
  if (m_media_player_view.is_play_hovered()) {
    surface.fill_rounded_rectangle(
        device_context, play_btn, fade_col(UI::Theme::Color{255, 255, 255, 40}),
        6.0F * scale);
  }
  const std::string_view play_icon_asset =
      m_media_player_view.is_playing() ? "debug-pause.svg" : "play.svg";
  ...
  surface.draw_svg_icon(device_context, play_icon_asset, play_center_x,
                        play_center_y, play_icon_sz,
                        fade_col(UI::Theme::Color{255, 255, 255, 255}),
                        UI::Theme::Color{0, 0, 0, 0}, false);
  ...
  if (surface.m_small_font && hud.width >= 340.0F * scale) {
    surface.draw_text(device_context, *surface.m_small_font, time_text,
                      play_btn.right() + 10.0F * scale,
                      hud.y + hud.height * 0.5F,
                      fade_col(UI::Theme::Color{255, 255, 255, 230}));
  }

  // Scrubber Track & Progress across the center (guaranteed no collision with
  // time text)
  const UI::Rect scrubber =
      m_media_player_view.calculate_scrubber_bounds(hud, scale);
  ...
  surface.fill_rounded_rectangle(device_context, track_rect,
                                 fade_col(UI::Theme::Color{255, 255, 255, 50}),
                                 2.0F * scale);
  ...
  if (progress > 0.0F) {
    const UI::Rect prog_rect{track_rect.x, track_rect.y,
                             track_rect.width * progress, track_h};
    surface.fill_rounded_rectangle(
        device_context, prog_rect,
        fade_col(UI::Theme::Color{255, 255, 255, 255}), 2.0F * scale);
  }

  // Knob (Smooth white circle) - always visible and accurately positioned at
  // progress ratio
  const float knob_x =
      track_rect.x + track_rect.width * std::clamp(progress, 0.0F, 1.0F);
  const float knob_y = track_y + track_h * 0.5F;
  const float knob_r = (m_media_player_view.is_scrubber_hovered() ||
                        m_media_player_view.is_scrubbing())
                           ? (6.5F * scale)
  ...
  const UI::Rect vol_rect =
      m_media_player_view.calculate_volume_bounds(hud, scale);
  surface.fill_rounded_rectangle(device_context, vol_rect,
                                 fade_col(UI::Theme::Color{255, 255, 255, 22}),
                                 10.0F * scale);
  const std::string_view vol_icon_asset =
      m_media_player_view.is_muted() ? "mute.svg" : "unmute.svg";
  ...
  surface.draw_svg_icon(device_context, vol_icon_asset, vol_center_x,
                        vol_center_y, vol_icon_sz,
                        fade_col(UI::Theme::Color{255, 255, 255, 225}),
                        UI::Theme::Color{0, 0, 0, 0}, false);
  ...
  if (vol_rect.width >= 50.0F * scale) {
    ...
    const UI::Rect vol_track{
        ...
        vol_track_w, vol_track_h};
    surface.fill_rounded_rectangle(
        device_context, vol_track,
        fade_col(UI::Theme::Color{255, 255, 255, 45}), 1.5F * scale);
    ...
    const UI::Rect vol_fill{vol_track.x, vol_track.y, vol_track.width * vol_val,
                            vol_track_h};
    surface.fill_rounded_rectangle(
        device_context, vol_fill,
        fade_col(UI::Theme::Color{255, 255, 255, 255}), 1.5F * scale);
  }

  // Fullscreen Button (Minimalist corner brackets icon)
  const UI::Rect fs_btn =
      m_media_player_view.calculate_fullscreen_button_bounds(hud, scale);
  if (m_media_player_view.is_fullscreen_hovered()) {
    surface.fill_rounded_rectangle(
        device_context, fs_btn, fade_col(UI::Theme::Color{255, 255, 255, 40}),
        6.0F * scale);
  }
  const std::string_view fs_icon_asset = m_media_player_view.is_fullscreen()
                                             ? "media-exit-fullscreen.svg"
  ...
  surface.draw_svg_icon(device_context, fs_icon_asset, fs_center_x, fs_center_y,
                        fs_icon_sz,
                        fade_col(UI::Theme::Color{255, 255, 255, 255}),
                        UI::Theme::Color{0, 0, 0, 0}, false);
  ...
  if (!badge_str.empty() && surface.m_small_font) {
    const int badge_w =
        surface.m_small_font->getTextWidth(device_context, badge_str) +
        static_cast<int>(16.0F * scale);
    const UI::Rect badge_rect{pane_bounds.right() - badge_w - 16.0F * scale,
                              pane_bounds.y + 14.0F * scale,
                              static_cast<float>(badge_w), 22.0F * scale};
    surface.fill_rounded_rectangle(device_context, badge_rect,
                                   fade_col(UI::Theme::Color{18, 20, 26, 210}),
                                   4.0F * scale);
    surface.draw_text(device_context, *surface.m_small_font, badge_str,
                      badge_rect.x + 8.0F * scale,
                      badge_rect.y + badge_rect.height * 0.5F,
                      fade_col(surface.m_palette.text_muted));
  }
}
...
void TextEditor::draw_document(
  ...
  if (document == nullptr) {
    ...
    const RECT clip_rc = {static_cast<LONG>(layout.editor_bounds.x),
    ...
    IntersectClipRect(device_context, clip_rc.left, clip_rc.top, clip_rc.right,
                      clip_rc.bottom);
    ...
    if (settings_service.get_schema().has_setting("workbench.mascot.image")) {
      const std::string custom_img =
          settings_service.get<std::string>("workbench.mascot.image");
      if (!custom_img.empty()) {
    ...
    if (settings_service.get_schema().has_setting(
            "workbench.mascot.renderMode")) {
      mascot_mode =
          settings_service.get<std::string>("workbench.mascot.renderMode");
    }
    ...
    if (settings_service.get_schema().has_setting("workbench.app.title")) {
      const std::string custom_title =
          settings_service.get<std::string>("workbench.app.title");
    ...
    // Resolve mascot asset once for both ASCII and PNG modes (supports Inno
    // Setup installed layout & dev paths)
    const auto resolved_mascot_path =
        Utility::Ascii::AsciiArtConverter::resolve_image_path(mascot_asset);
    const std::string effective_mascot = (!resolved_mascot_path.empty())
                                             ? resolved_mascot_path.string()
                                             : mascot_asset;
    ...
    if (mascot_mode == "ascii") {
      rendered_ascii = Utility::Ascii::AsciiMascotRenderer::render_mascot_ascii(
          ...
          to_color_ref(surface.m_palette.editor_background), dpi);
    }
    if (!rendered_ascii) {
      surface.draw_png_icon(device_context, effective_mascot,
    ...
    if (surface.m_large_font) {
      surface.draw_text(device_context, *surface.m_large_font, title, text_x,
                        start_y + 36.0F * dpi, surface.m_palette.text_primary);
    } else if (surface.m_ui_font) {
      surface.draw_text(device_context, *surface.m_ui_font, title, text_x,
                        start_y + 36.0F * dpi, surface.m_palette.text_primary);
    }
    ...
    if (surface.m_small_font || surface.m_ui_font) {
      ...
      for (std::size_t i = 0; i < shortcuts.size(); ++i) {
        ...
        surface.draw_text(device_context, font, shortcuts[i].key, text_x, row_y,
                          surface.m_palette.text_primary);
        surface.draw_text(device_context, font, shortcuts[i].label,
                          text_x + key_col_w, row_y,
                          surface.m_palette.text_muted);
      }
    }
  ...
  const auto token_color =
    ...
    case UI::Editor::EditorTokenKind::Directive:
      return surface.m_palette.directive;
    case UI::Editor::EditorTokenKind::Macro:
      return surface.m_palette.macro_symbol;
    case UI::Editor::EditorTokenKind::IncludeHeader:
      return surface.m_palette.include_header;
  ...
  const std::filesystem::path left_path =
      (active_p && !active_p->empty())
          ...
          : std::filesystem::path(std::string(document->get_file_name()));
  const bool left_is_media =
      UI::Editor::MediaPlayerView::is_media_file(left_path);
  ...
  const float scrollbar_w = FIXED_SCROLLBAR_WIDTH * scale;
  const float left_minimap_w = (half_w >= MIN_PANE_WIDTH_FOR_MINIMAP * scale)
                                   ? (FIXED_MINIMAP_WIDTH * scale)
                                   : 0.0F;
  const float left_code_limit =
      is_split_active
          ? (left_bounds_r - scrollbar_w - left_minimap_w)
          : (layout.editor_bounds.right() - layout.scrollbar_bounds.width -
             layout.minimap_bounds.width);
  const float left_right_limit = splitter_x_coord;
  const float available_code_width =
      std::max(left_code_limit - (layout.editor_bounds.x + 14.0F * scale),
               20.0F * scale);
  ...
  if (left_is_media && m_media_preview_mode) {
    ...
    if (m_reveal_caret_pending && is_this_pane_focused) {
      ...
      // Reveal caret horizontally with generous padding
      const std::string_view caret_line_str =
          document->get_line(document->get_caret_line());
      const std::string_view caret_prefix = caret_line_str.substr(
          0, std::min(document->get_caret_column(), caret_line_str.size()));
      const float caret_pixel_x = static_cast<float>(surface.get_text_width(
          device_context, *surface.m_editor_font, caret_prefix));

      const float pad_right = 60.0F * scale;
      const float pad_left = 30.0F * scale;
      if (caret_pixel_x < m_text_scroll_offset + pad_left) {
        const_cast<TextEditor *>(this)->m_text_scroll_offset =
            std::max(0.0F, caret_pixel_x - pad_left);
      } else if (caret_pixel_x >
                 m_text_scroll_offset + available_code_width - pad_right) {
        const_cast<TextEditor *>(this)->m_text_scroll_offset =
            std::max(0.0F, caret_pixel_x - available_code_width + pad_right);
      }
    ...
    const std::size_t first_line =
        visual_row_to_physical_line(m_folding, first_visual_row, total_lines);
    ...
    if (m_last_folded_doc != document ||
        m_last_folded_line_count != total_lines || needs_window_shift) {
    ...
    const float fold_margin =
        ...
        surface.m_dpi_scale;
    const float gutter_line_x =
        layout.gutter_bounds.right() - fold_margin - 1.0F;
    surface.draw_line(
        device_context, round_to_int(gutter_line_x),
        round_to_int(layout.gutter_bounds.y), round_to_int(gutter_line_x),
        round_to_int(layout.gutter_bounds.bottom()), surface.m_palette.border);
    ...
    // --- Indent and Scope guide rendering (VS Code style) ---
    {
      ...
      const UI::Components::ActiveIndentScope active_scope =
          m_folding.get_active_indent_scope(document->get_caret_line(),
                                            tab_size);
      ...
      for (std::size_t line_index = first_line;
        ...
        for (std::size_t col = 0; col + tab_size <= line_indent;
             col += tab_size) {
          ...
          surface.draw_line(device_context, round_to_int(guide_x),
                            round_to_int(y_top), round_to_int(guide_x),
                            round_to_int(y_bottom),
    ...
    for (std::size_t line_index = first_line;
      ...
      const bool active_line =
          ...
          (!is_split_active || m_focused_pane == SplitPaneFocus::Left);
      ...
      std::string number;
      if (m_relative_line_numbers && !active_line) {
        const auto diff =
            std::abs(static_cast<long long>(line_index) -
                     static_cast<long long>(document->get_caret_line()));
        number = std::to_string(diff);
      } else {
        number = std::to_string(line_index + 1);
      }
      const float line_num_right = layout.gutter_bounds.right() - fold_margin -
                                   10.0F * surface.m_dpi_scale;
      const float num_w = static_cast<float>(surface.get_text_width(
          device_context, *surface.m_editor_font, number));
      const float number_x = std::max(line_num_right - num_w,
                                      layout.gutter_bounds.x + bp_lane_width);
      surface.draw_text(device_context, *surface.m_editor_font, number,
                        number_x, center_y,
                        active_line ? surface.m_palette.text_primary
                                    : surface.m_palette.text_muted);

      // Breakpoint marker (red circle / hover preview) centered in dedicated
      // breakpoint lane
      const bool has_bp = document->has_breakpoint(line_index);
      const bool is_bp_hovered = (!has_bp && m_hovered_gutter_line &&
                                  *m_hovered_gutter_line == line_index);
      if (has_bp || is_bp_hovered) {
        const UI::Theme::Color bp_color =
            has_bp ? UI::Theme::Color{229, 20, 0, 255}
      ...
      if (has_gutter_marker(line)) {
        ...
        HPEN pen =
            CreatePen(PS_SOLID, 1, to_color_ref(surface.m_palette.text_muted));
      ...
      if (fold_marker == UI::Components::FoldMarker::Expanded ||
        ...
        const int box_half =
            std::max(round_to_int(4.5F * surface.m_dpi_scale), 4);
        RECT box_rect{fold_cx - box_half, fold_cy - box_half,
                      fold_cx + box_half, fold_cy + box_half};

        HBRUSH bg_brush = CreateSolidBrush(
            to_color_ref(active_line ? surface.m_palette.active_line_background
                                     : surface.m_palette.editor_background));
        ...
        HPEN box_pen =
            CreatePen(PS_SOLID, 1,
                      to_color_ref(fold_hovered ? surface.m_palette.accent
                                                : surface.m_palette.border));
        HGDIOBJ old_pen = SelectObject(device_context, box_pen);
        HGDIOBJ old_brush =
            SelectObject(device_context, GetStockObject(HOLLOW_BRUSH));
        Rectangle(device_context, box_rect.left, box_rect.top, box_rect.right,
                  box_rect.bottom);
        ...
        surface.draw_line(device_context, fold_cx - box_half + sign_inset,
                          fold_cy, fold_cx + box_half - sign_inset, fold_cy,
                          fold_hovered ? surface.m_palette.accent
                                       : surface.m_palette.text_muted);

        if (fold_marker == UI::Components::FoldMarker::Collapsed) {
          // Vertical line of + (only when collapsed)
          surface.draw_line(device_context, fold_cx,
                            fold_cy - box_half + sign_inset, fold_cx,
                            fold_cy + box_half - sign_inset,
                            fold_hovered ? surface.m_palette.accent
                                         : surface.m_palette.text_muted);
        }
        ...
        surface.draw_line(device_context, fold_cx,
                          round_to_int(center_y - line_height * 0.5F), fold_cx,
                          fold_cy, surface.m_palette.border);
        surface.draw_line(device_context, fold_cx, fold_cy,
      ...
      if (fold_marker == UI::Components::FoldMarker::Expanded) {
        const int box_half =
            std::max(round_to_int(4.5F * surface.m_dpi_scale), 4);
        surface.draw_line(device_context, fold_cx, fold_cy + box_half, fold_cx,
                          round_to_int(center_y + line_height * 0.5F),
                          surface.m_palette.border);
      }
    }
    ...
    IntersectClipRect(
        device_context, static_cast<int>(layout.editor_bounds.x),
      ...
      for (const auto &cursor : document->get_all_cursors()) {
        if (cursor.has_selection()) {
          ...
          for (std::size_t line_index = start_line;
               line_index <= end_line && line_index < total_lines;
               ++line_index) {
            if (m_folding.is_line_hidden(line_index) ||
            ...
            const std::size_t visual_row =
                physical_line_to_visual_row(m_folding, line_index, total_lines);
            const float screen_y = layout.editor_bounds.y +
                                   (static_cast<float>(visual_row) -
            ...
            if (screen_y + line_height >= layout.editor_bounds.y &&
              ...
              surface.fill_rounded_rectangle(
                  device_context,
                  UI::Rect{static_cast<float>(snap_x),
                           static_cast<float>(snap_y),
    ...
    for (std::size_t line_index = first_line;
      ...
      const float current_line_width = static_cast<float>(
          surface.get_text_width(device_context, *surface.m_editor_font, line));
          ...
          for (const auto &cursor : document->get_all_cursors()) {
            if (cursor.has_selection()) {
              ...
              if (line_index >= sel.start.line && line_index <= sel.end.line) {
                line_has_sel = true;
                sel_start_col =
                    (line_index == sel.start.line) ? sel.start.column : 0;
                sel_end_col =
                    (line_index == sel.end.line) ? sel.end.column : line.size();
                break;
              }
            }
          }
        ...
        if (is_all || is_boundary || (is_sel_only && line_has_sel)) {
          ...
          for (std::size_t col = 0; col < line.size(); ++col) {
            ...
            if (is_sel_only) {
              ...
              const bool is_leading = (first_non_ws == std::string_view::npos ||
                                       col < first_non_ws);
              const bool is_trailing =
                  (last_non_ws != std::string_view::npos && col > last_non_ws);
              const bool is_multi =
                  (col + 1 < line.size() && line[col + 1] == ' ') ||
                  (col > 0 && line[col - 1] == ' ');
              if (!is_leading && !is_trailing && !is_multi)
                continue;
            }
            ...
            const float char_x =
                code_x + static_cast<float>(surface.get_text_width(
                             device_context, *surface.m_editor_font,
                             line.substr(0, col)));
            if (char_x < layout.editor_bounds.x || char_x > left_right_limit)
              continue;

            if (ch == ' ') {
              const float sp_w = (m_cached_char_width > 0.0F)
                                     ? m_cached_char_width
                                     : (8.0F * surface.m_dpi_scale);
              const float dot_x = char_x + sp_w * 0.5F;
              surface.fill_rectangle(device_context,
                                     UI::Rect{dot_x - dot_r, center_y - dot_r,
              ...
              const float arrow_len =
                  std::max(4.0F * surface.m_dpi_scale,
                           (next_x - char_x) - 4.0F * surface.m_dpi_scale);
              surface.draw_line(
                  device_context,
                  round_to_int(char_x + 2.0F * surface.m_dpi_scale),
                  round_to_int(center_y),
                  round_to_int(char_x + 2.0F * surface.m_dpi_scale + arrow_len),
                  round_to_int(center_y), surface.m_palette.indent_guide);
            }
          }
        }
      }
      ...
      const auto semantic_spans = Language::LanguageServerManager::instance()
                                      .get_semantic_tokens_manager()
                                      .get_tokens_for_line(doc_uri, line_index);

      auto is_token_unused = [&](std::size_t tok_start,
                                 std::size_t tok_len) -> bool {
        if (is_line_inactive)
          return true;
        ...
        for (const auto &diag : line_diags) {
          ...
          if (line.find("%macro") != std::string::npos)
            return true;
          std::size_t diag_start = (diag.range.start.line == line_index)
                                       ? diag.range.start.character
                                       : 0;
          std::size_t diag_end = (diag.range.end.line == line_index)
          ...
          if (tok_start < diag_end && tok_end > diag_start)
            return true;
        }
        return false;
      };
      ...
      auto get_effective_token_color =
          [&](UI::Editor::EditorTokenKind kind, std::size_t tok_start,
              std::size_t tok_len) -> UI::Theme::Color {
        // If this token is hovered while Ctrl is held, highlight with vibrant
        // accent color (VS Code link style)
        if (m_ctrl_hovered_token && m_ctrl_hovered_token->line == line_index) {
          if (tok_start < m_ctrl_hovered_token->end_col &&
              (tok_start + tok_len) > m_ctrl_hovered_token->start_col) {
        ...
        if (is_token_unused(tok_start, tok_len)) {
          return UI::Theme::dim_color(base,
                                      surface.m_palette.editor_background);
        }
        return base;
      };
      ...
      if (syntax_highlighting) {
        ...
        for (std::size_t token_index = 0; token_index < token_count;
          ...
          UI::Editor::TextPosition active_brace_pos{};
          ...
          if (has_animated_brace) {
            if (brace_offset > 0) {
              ...
              surface.draw_text(device_context, *surface.m_editor_font, pre,
                                token_x, center_y,
                                get_effective_token_color(
                                    token.kind, rendered_bytes, brace_offset));
              token_x += static_cast<float>(surface.get_text_width(
                  device_context, *surface.m_editor_font, pre));
            }

            std::string_view brace_char = token.text.substr(brace_offset, 1);
            float pulse = m_brace_animation.get_brace_scale(active_brace_pos);
            float brace_w = static_cast<float>(surface.get_text_width(
                device_context, *surface.m_editor_font, brace_char));
            ...
            surface.draw_scaled_text(device_context, *surface.m_editor_font,
                                     brace_char, token_x, center_y, pulse,
                                     surface.m_palette.accent);
            token_x += brace_w;
            ...
            if (brace_offset + 1 < token.text.size()) {
              ...
              surface.draw_text(
                  device_context, *surface.m_editor_font, post, token_x,
                  center_y,
                  get_effective_token_color(token.kind,
                                            rendered_bytes + brace_offset + 1,
                                            post.size()));
              token_x += static_cast<float>(surface.get_text_width(
                  device_context, *surface.m_editor_font, post));
            }
          } else {
            surface.draw_text(device_context, *surface.m_editor_font,
                              token.text, token_x, center_y,
                              get_effective_token_color(token.kind,
                                                        rendered_bytes,
                                                        token.text.size()));

            if (token.kind == UI::Editor::EditorTokenKind::IncludeHeader &&
                !is_line_inactive && token.text.size() > 2) {
              const char open_c = token.text.front();
              const char close_c = token.text.back();
              if ((open_c == '<' && close_c == '>') ||
                  (open_c == '"' && close_c == '"')) {
                const float open_w = static_cast<float>(surface.get_text_width(
                    device_context, *surface.m_editor_font,
                    token.text.substr(0, 1)));
                const float inner_w = static_cast<float>(surface.get_text_width(
                    device_context, *surface.m_editor_font,
                    token.text.substr(1, token.text.size() - 2)));
                const int underline_y =
                    round_to_int(center_y + line_height * 0.42F);
                surface.draw_line(
                    device_context, round_to_int(token_x + open_w), underline_y,
                    round_to_int(token_x + open_w + inner_w), underline_y,
        ...
        if (rendered_bytes < line.size()) {
          const bool leftover_unused =
              is_token_unused(rendered_bytes, line.size() - rendered_bytes);
          surface.draw_text(
              device_context, *surface.m_editor_font,
              line.substr(rendered_bytes), token_x, center_y,
              leftover_unused
                  ? UI::Theme::dim_color(surface.m_palette.text_primary,
                                         surface.m_palette.editor_background)
                  : surface.m_palette.text_primary);
        }
      } else {
        ...
        surface.draw_text(
            device_context, *surface.m_editor_font, line, code_x, center_y,
            line_unused
                ? UI::Theme::dim_color(surface.m_palette.text_primary,
                                       surface.m_palette.editor_background)
                : surface.m_palette.text_primary);
      ...
      if (!is_line_inactive) {
        for (const auto &diag : line_diags) {
          std::size_t start_col = diag.range.start.line == line_index
                                      ? diag.range.start.character
                                      : 0;
          ...
          if (start_col > 0 && start_col <= line.size()) {
            diag_start_x += static_cast<float>(
                surface.get_text_width(device_context, *surface.m_editor_font,
                                       line.substr(0, start_col)));
          }
          float diag_width = 8.0F;
          if (end_col > start_col && start_col < line.size()) {
            diag_width = static_cast<float>(surface.get_text_width(
                device_context, *surface.m_editor_font,
                line.substr(start_col, end_col - start_col)));
          }

          UI::Theme::Color squiggle_color =
          ...
          while (wave_x < wave_end_x) {
            ...
            const float y1 =
                wave_up ? (wave_y - wave_amp) : (wave_y + wave_amp);
            const float y2 =
                wave_up ? (wave_y + wave_amp) : (wave_y - wave_amp);
            surface.draw_line(device_context, round_to_int(wave_x),
                              round_to_int(y1), round_to_int(next_x),
                              round_to_int(y2), squiggle_color);
            wave_x = next_x;
            wave_up = !wave_up;
          }
        }
      ...
      if (m_ctrl_hovered_token && m_ctrl_hovered_token->line == line_index) {
        const std::size_t sc =
            std::min(m_ctrl_hovered_token->start_col, line.size());
        const std::size_t ec =
            std::min(m_ctrl_hovered_token->end_col, line.size());
        if (ec > sc) {
          const float u_start_x =
              code_x +
              static_cast<float>(surface.get_text_width(
                  device_context, *surface.m_editor_font, line.substr(0, sc)));
          const float u_width = static_cast<float>(
              surface.get_text_width(device_context, *surface.m_editor_font,
                                     line.substr(sc, ec - sc)));
          const float u_y = center_y + line_height * 0.42F;

          surface.draw_line(device_context, round_to_int(u_start_x),
                            round_to_int(u_y),
                            round_to_int(u_start_x + u_width),
                            round_to_int(u_y), surface.m_palette.accent);
        }
      }
      ...
      if (!line_diags.empty()) {
        ...
        const bool is_collapsed = (m_folding.get_marker(line_index) ==
                                   UI::Components::FoldMarker::Collapsed);
        const float lens_start_x = code_x + current_line_width +
                                   (is_collapsed ? 42.0F * dpi : 20.0F * dpi);
        const float avail_w =
            std::max(left_code_limit - lens_start_x - 16.0F * dpi, 80.0F * dpi);
        ...
        int msg_w =
            surface.get_text_width(device_context, *surface.m_ui_font, msg);
        const float max_msg_w =
            std::max(avail_w - (pad_x * 2.0F + icon_size + gap), 40.0F * dpi);
        if (static_cast<float>(msg_w) > max_msg_w && msg.size() > 8) {
          while (msg.size() > 4 && static_cast<float>(surface.get_text_width(
                                       device_context, *surface.m_ui_font,
                                       msg + "...")) > max_msg_w) {
            msg.pop_back();
          }
          msg += "...";
        ...
        surface.fill_rounded_rectangle(device_context, badge_rect, badge_bg,
                                       4.0F * dpi);
        ...
        surface.draw_svg_icon(device_context, icon_asset, icon_cx, icon_cy,
        ...
        surface.draw_text(device_context, *surface.m_ui_font, msg, text_x,
                          center_y, badge_fg);
      }

      // Collapsed code placeholder badge (...)
      if (m_folding.get_marker(line_index) ==
          UI::Components::FoldMarker::Collapsed) {
        const float badge_x =
            code_x + current_line_width + 8.0F * surface.m_dpi_scale;
        ...
        for (const auto &cur : document->get_all_cursors()) {
          if (cur.line == line_index) {
            ...
            const int caret_x = round_to_int(
                code_x + static_cast<float>(surface.get_text_width(
                             device_context, *surface.m_editor_font, prefix)));
            const float mono_char_w = static_cast<float>(
                std::max(surface.get_text_width(device_context,
                                                *surface.m_editor_font, "X"),
                         1));
            const float font_h =
                surface.m_editor_font
                    ? static_cast<float>(
                          surface.m_editor_font->getHeight(device_context))
                    : (16.0F * surface.m_dpi_scale);
            const float block_h =
                std::min(font_h, line_height - 3.0F * surface.m_dpi_scale);
            const float block_y = center_y - block_h * 0.5F;

            const std::string_view effective_cursor =
                m_input_router.is_vim_active()
                    ? m_input_router.get_cursor_shape()
                    : std::string_view(m_cursor_style);

            float char_w = mono_char_w;
            std::string_view char_view;
            if (cur.column < line.size()) {
              std::size_t char_len = 1;
              const unsigned char lead =
                  static_cast<unsigned char>(line[cur.column]);
              if ((lead & 0xE0) == 0xC0 && cur.column + 1 < line.size())
                char_len = 2;
              else if ((lead & 0xF0) == 0xE0 && cur.column + 2 < line.size())
                char_len = 3;
              else if ((lead & 0xF8) == 0xF0 && cur.column + 3 < line.size())
                char_len = 4;

              char_view = line.substr(cur.column, char_len);
              if (char_view != "\t" && char_view != "\r" && char_view != "\n") {
                const int measured = surface.get_text_width(
                    device_context, *surface.m_editor_font, char_view);
                if (measured > 0) {
                  char_w = static_cast<float>(measured);
                }
              }
            }

            if (effective_cursor == "Block") {
              const UI::Rect block_rect{static_cast<float>(caret_x), block_y,
                                        char_w, block_h};
              const UI::Theme::Color block_color{174, 175, 173, 240};
              surface.fill_rectangle(device_context, block_rect, block_color);
              if (!char_view.empty() && char_view != "\t" &&
                  char_view != "\r" && char_view != "\n") {
                surface.draw_text(device_context, *surface.m_editor_font,
                                  char_view, static_cast<float>(caret_x),
                                  center_y,
                                  surface.m_palette.editor_background);
              }
            } else if (effective_cursor == "Underline") {
              const int u_y = round_to_int(center_y + block_h * 0.5F);
              const int u_w = round_to_int(char_w);
              surface.draw_line(device_context, caret_x, u_y, caret_x + u_w,
                                u_y, surface.m_palette.text_primary);
              surface.draw_line(device_context, caret_x, u_y - 1, caret_x + u_w,
                                u_y - 1, surface.m_palette.text_primary);
            } else {
              surface.draw_line(device_context, caret_x, round_to_int(block_y),
                                caret_x, round_to_int(block_y + block_h),
                                surface.m_palette.text_primary);
            }
          }
        }
    ...
    if (m_max_text_scroll == 0.0F) {
      ...
      // Draw modern responsive horizontal scrollbar track and thumb
      const float track_x = layout.editor_bounds.x;
      const float track_w = is_split_active ? (left_bounds_r - track_x)
                                            : layout.editor_bounds.width;
      const float track_h = 10.0F * surface.m_dpi_scale;
      const float track_y = layout.editor_bounds.bottom() - track_h;
      m_text_hscrollbar_bounds = UI::Rect{track_x, track_y, track_w, track_h};
      ...
      const float thumb_x =
          ...
          (m_text_scroll_offset / m_max_text_scroll) * (track_w - thumb_w);
      const float thumb_h =
          (m_dragging_text_hscrollbar || m_hovered_text_hscrollbar)
              ? (7.0F * surface.m_dpi_scale)
              : (4.0F * surface.m_dpi_scale);
      const float thumb_y = track_y + (track_h - thumb_h) * 0.5F;
      const UI::Rect thumb_bounds{thumb_x, thumb_y, thumb_w, thumb_h};

      const UI::Theme::Color thumb_color =
          m_dragging_text_hscrollbar
              ? surface.m_palette.accent
              : (m_hovered_text_hscrollbar
                     ? UI::Theme::Color{200, 205, 215, 200}
                     : UI::Theme::Color{140, 145, 155, 130});

      surface.fill_rounded_rectangle(device_context, thumb_bounds, thumb_color,
                                     thumb_h * 0.5F);
    }
  }
  ...
  if (m_is_split && m_split_document_index.has_value() &&
    ...
    if (split_doc_ptr != nullptr) {
      ...
      const float right_w =
          std::max(layout.editor_bounds.right() - right_x, 0.0F);
      const UI::Rect right_pane{right_x, layout.editor_bounds.y, right_w,
                                layout.editor_bounds.height};

      const std::filesystem::path right_path =
          std::string(split_doc.get_file_name());
      const bool right_is_media =
          UI::Editor::MediaPlayerView::is_media_file(right_path);

      if (right_is_media && m_media_preview_mode) {
        draw_media_pane(surface, device_context, right_pane, right_path);
      } else {
        const float right_gutter_w = layout.gutter_bounds.width;
        const UI::Rect right_gutter{right_pane.x, right_pane.y, right_gutter_w,
                                    right_pane.height};
        const float right_scrollbar_w = FIXED_SCROLLBAR_WIDTH * scale;
        const float right_minimap_w =
            (right_pane.width >= MIN_PANE_WIDTH_FOR_MINIMAP * scale)
                ? (FIXED_MINIMAP_WIDTH * scale)
                : 0.0F;
        const float right_code_limit =
            std::max(right_pane.right() - right_scrollbar_w - right_minimap_w,
                     right_gutter.right());
        const UI::Rect right_code{
            right_gutter.right(), right_pane.y,
        ...
        const float fold_margin =
            UI::Editor::StudioEditorMetrics::fold_margin_width * scale;
        const float right_gutter_line_x =
            right_gutter.right() - fold_margin - 1.0F;
        surface.draw_line(
            device_context, round_to_int(right_gutter_line_x),
            round_to_int(right_pane.y), round_to_int(right_gutter_line_x),
            round_to_int(right_pane.bottom()), surface.m_palette.border);

        const std::size_t split_tab_size =
            split_doc.get_status().indent_width > 0
                ? split_doc.get_status().indent_width
                : 4;

        const std::size_t split_total_lines = split_doc.get_line_count();
        if (m_split_last_folded_doc != &split_doc ||
        ...
        if (m_reveal_caret_pending && m_focused_pane == SplitPaneFocus::Right) {
          static_cast<void>(m_split_scrollbar.reveal_line(
              physical_line_to_visual_row(m_split_folding,
                                          split_doc.get_caret_line(),
                                          split_total_lines)));
          m_reveal_caret_pending = false;
        }
        const std::size_t split_first_line =
            m_split_scrollbar.get_first_visible_line();

        // Right Pane Indent & Scope Guides
        {
          const float space_width = static_cast<float>(surface.get_text_width(
              device_context, *surface.m_editor_font, " "));
          const UI::Components::ActiveIndentScope active_scope =
              m_split_folding.get_active_indent_scope(
                  split_doc.get_caret_line(), split_tab_size);

          std::size_t r_guide = 0;
          for (std::size_t line_index = split_first_line;
               r_guide < visible_count && line_index < split_total_lines;
               ++line_index) {
            if (m_split_folding.is_line_hidden(line_index))
              continue;
            const float cy =
                first_center_y + static_cast<float>(r_guide) * line_height;
            ++r_guide;
            ...
            const std::size_t line_indent =
                m_split_folding.get_effective_indent(line_index);
            if (line_indent < split_tab_size)
              continue;

            for (std::size_t col = 0; col + split_tab_size <= line_indent;
                 col += split_tab_size) {
              const float guide_x =
                  right_code.x + static_cast<float>(col) * space_width;
              if (guide_x < right_code.x || guide_x > right_code.right())
                continue;
              ...
              surface.draw_line(
                  device_context, round_to_int(guide_x), round_to_int(y_top),
        ...
        for (std::size_t line_index = split_first_line;
             r_pass1 < visible_count && line_index < split_total_lines;
             ++line_index) {
          if (m_split_folding.is_line_hidden(line_index))
            continue;
          const float cy =
              first_center_y + static_cast<float>(r_pass1) * line_height;
          ++r_pass1;

          const bool is_active_line =
              (line_index == split_doc.get_caret_line()) &&
              (m_focused_pane == SplitPaneFocus::Right);
          if (is_active_line && !split_doc.has_selection()) {
            surface.fill_rectangle(
                device_context,
                UI::Rect{right_gutter.x, cy - line_height * 0.5F,
                         right_code.right() - right_gutter.x, line_height},
                surface.m_palette.active_line_background);
          }

          std::string num_str;
          if (m_relative_line_numbers && !is_active_line) {
            const auto diff =
                std::abs(static_cast<long long>(line_index) -
                         static_cast<long long>(split_doc.get_caret_line()));
            num_str = std::to_string(diff);
          } else {
            num_str = std::to_string(line_index + 1);
          }
          const float nx =
              right_gutter.right() - fold_margin - 6.0F * scale -
              static_cast<float>(surface.get_text_width(
                  device_context, *surface.m_editor_font, num_str));
          surface.draw_text(device_context, *surface.m_editor_font, num_str, nx,
                            cy,
                            is_active_line ? surface.m_palette.text_primary
                                           : surface.m_palette.text_muted);
          ...
          const UI::Components::FoldMarker r_fold =
              m_split_folding.get_marker(line_index);
          const float r_fold_center_x =
              right_gutter.right() - fold_margin * 0.5F;
          ...
          if (r_fold == UI::Components::FoldMarker::Expanded ||
            ...
            RECT box_rect{r_cx - box_half, r_cy - box_half, r_cx + box_half,
                          r_cy + box_half};
            HBRUSH bg_brush = CreateSolidBrush(to_color_ref(
                is_active_line ? surface.m_palette.active_line_background
                               : surface.m_palette.editor_background));
            FillRect(device_context, &box_rect, bg_brush);
            DeleteObject(bg_brush);

            HPEN box_pen =
                CreatePen(PS_SOLID, 1, to_color_ref(surface.m_palette.border));
            HGDIOBJ old_pen = SelectObject(device_context, box_pen);
            HGDIOBJ old_brush =
                SelectObject(device_context, GetStockObject(HOLLOW_BRUSH));
            Rectangle(device_context, box_rect.left, box_rect.top,
            ...
            surface.draw_line(device_context, r_cx - box_half + sign_inset,
                              r_cy, r_cx + box_half - sign_inset, r_cy,
                              surface.m_palette.text_muted);

            if (r_fold == UI::Components::FoldMarker::Collapsed) {
              surface.draw_line(
                  device_context, r_cx, r_cy - box_half + sign_inset, r_cx,
                  r_cy + box_half - sign_inset, surface.m_palette.text_muted);
            }
          } else if (r_fold == UI::Components::FoldMarker::Continuation) {
            // Vertical scope guide line
            surface.draw_line(device_context, r_cx,
                              round_to_int(cy - line_height * 0.5F), r_cx,
                              round_to_int(cy + line_height * 0.5F),
                              surface.m_palette.border);
          } else if (r_fold == UI::Components::FoldMarker::End) {
            // Corner guide (╰)
            surface.draw_line(device_context, r_cx,
                              round_to_int(cy - line_height * 0.5F), r_cx, r_cy,
                              surface.m_palette.border);
            surface.draw_line(device_context, r_cx, r_cy,
          ...
          if (r_fold == UI::Components::FoldMarker::Expanded) {
            ...
            surface.draw_line(device_context, r_cx, r_cy + box_half, r_cx,
                              round_to_int(cy + line_height * 0.5F),
                              surface.m_palette.border);
          }
        }
        ...
        SaveDC(device_context);
        IntersectClipRect(device_context, static_cast<int>(right_code.x),
        ...
        // Right Selection Highlight: Direct & Instant (0 animation delay)
        const std::size_t split_first_visual_row =
            m_split_scrollbar.get_first_visible_line();
        if (m_focused_pane == SplitPaneFocus::Right) {
          for (const auto &cursor : split_doc.get_all_cursors()) {
            if (cursor.has_selection()) {
              const UI::Editor::TextSelection sel = cursor.get_selection();
              const std::size_t s_line =
                  std::max(sel.start.line, split_first_line);
              const std::size_t e_line =
                  std::min(sel.end.line, split_first_line + visible_count);
              for (std::size_t li = s_line;
                   li <= e_line && li < split_total_lines; ++li) {
                if (m_split_folding.is_line_hidden(li) ||
                    li >= split_doc.get_line_count()) {
                  continue;
                }
                const std::string_view line_str = split_doc.get_line(li);
                const std::size_t sel_start =
                    (li == sel.start.line) ? sel.start.column : 0;
                const std::size_t sel_end =
                    (li == sel.end.line) ? sel.end.column : line_str.size();
                const float sel_x = static_cast<float>(surface.get_text_width(
                    device_context, *surface.m_editor_font,
                    line_str.substr(0, sel_start)));
                float sel_w = static_cast<float>(surface.get_text_width(
                    device_context, *surface.m_editor_font,
                    line_str.substr(sel_start, sel_end - sel_start)));
                if (li < sel.end.line) {
                  sel_w += 6.0F * scale;
                }
                ...
                const std::size_t visual_row = physical_line_to_visual_row(
                    m_split_folding, li, split_total_lines);
                const float screen_y =
                    right_pane.y +
                    (static_cast<float>(visual_row) -
                     static_cast<float>(split_first_visual_row)) *
                        line_height;
                const float screen_x = right_code_x + sel_x;

                if (screen_y + line_height >= right_pane.y &&
        ...
        const bool right_syntax =
            UI::Editor::supports_editor_syntax_highlighting(
                split_doc.get_file_name());
        std::size_t r_pass2 = 0;
        for (std::size_t line_index = split_first_line;
             r_pass2 < visible_count && line_index < split_total_lines;
             ++line_index) {
          if (m_split_folding.is_line_hidden(line_index))
            continue;
          const float cy =
              first_center_y + static_cast<float>(r_pass2) * line_height;
          ++r_pass2;

          const std::string_view lstr = split_doc.get_line(line_index);
          float tok_x = right_code_x;

          const bool r_is_line_inactive =
              split_doc.is_line_inactive(line_index);
          ...
          const auto r_semantic_spans =
              Language::LanguageServerManager::instance()
                  .get_semantic_tokens_manager()
                  .get_tokens_for_line(r_doc_uri, line_index);

          auto is_r_token_unused = [&](std::size_t tok_start,
                                       std::size_t tok_len) -> bool {
            ...
            for (const auto &diag : r_diags) {
              if (!diag.is_unnecessary())
                continue;
              if (lstr.find("#include") != std::string::npos)
                return true;
              if (lstr.find("#define") != std::string::npos)
                return true;
              if (lstr.find("#undef") != std::string::npos)
                return true;
              if (lstr.find("%define") != std::string::npos)
                return true;
              if (lstr.find("%macro") != std::string::npos)
                return true;
              std::size_t diag_start = (diag.range.start.line == line_index)
                                           ? diag.range.start.character
                                           : 0;
          ...
          auto get_effective_r_token_color =
              [&](UI::Editor::EditorTokenKind kind, std::size_t tok_start,
                  std::size_t tok_len) -> UI::Theme::Color {
            if (m_ctrl_hovered_token &&
                m_ctrl_hovered_token->line == line_index) {
              if (tok_start < m_ctrl_hovered_token->end_col &&
                  (tok_start + tok_len) > m_ctrl_hovered_token->start_col) {
                return surface.m_palette.accent;
              }
          ...
          if (right_syntax) {
            std::size_t rbytes = 0;
            std::array<UI::Editor::EditorToken,
                       UI::Editor::maximum_editor_tokens>
                rtokens{};
            auto rline_state = split_doc.get_line_state(line_index);
            const std::size_t rcount = UI::Editor::tokenize_editor_line(
                lstr, rtokens, split_doc.get_file_name(), &rline_state);
            for (std::size_t ti = 0; ti < rcount; ++ti) {
              const auto &t = rtokens[ti];

              bool has_animated_brace = false;
              std::size_t brace_offset = 0;
              UI::Editor::TextPosition active_r_brace_pos{};

              if (m_split_brace_animation.has_active_braces()) {
                if (auto open_pos = m_split_brace_animation.get_open_brace();
                    open_pos && open_pos->line == line_index) {
                  if (open_pos->column >= rbytes &&
                      open_pos->column < rbytes + t.text.size()) {
                    has_animated_brace = true;
                    brace_offset = open_pos->column - rbytes;
                    active_r_brace_pos = *open_pos;
                  }
                }
                if (!has_animated_brace) {
                  if (auto close_pos =
                          m_split_brace_animation.get_close_brace();
                      close_pos && close_pos->line == line_index) {
                    if (close_pos->column >= rbytes &&
                        close_pos->column < rbytes + t.text.size()) {
                      has_animated_brace = true;
                      brace_offset = close_pos->column - rbytes;
                      active_r_brace_pos = *close_pos;
                    }
                  }
                }
              }
              ...
              if (has_animated_brace) {
                if (brace_offset > 0) {
                  ...
                  surface.draw_text(device_context, *surface.m_editor_font, pre,
                                    tok_x, cy,
                                    get_effective_r_token_color(t.kind, rbytes,
                                                                brace_offset));
                ...
                float pulse =
                    m_split_brace_animation.get_brace_scale(active_r_brace_pos);
                float brace_w = static_cast<float>(surface.get_text_width(
                    device_context, *surface.m_editor_font, brace_char));
                ...
                surface.fill_rounded_rectangle(
                    device_context,
                    UI::Rect{tok_x - extra_w - 2.0F, screen_y - extra_h,
                             brace_w + extra_w * 2.0F + 4.0F,
                             line_height + extra_h * 2.0F},
                    pulse_color, 3.0F * surface.m_dpi_scale * pulse);

                surface.draw_scaled_text(device_context, *surface.m_editor_font,
                                         brace_char, tok_x, cy, pulse,
                                         surface.m_palette.accent);
                tok_x += brace_w;
                ...
                if (brace_offset + 1 < t.text.size()) {
                  ...
                  surface.draw_text(
                      device_context, *surface.m_editor_font, post, tok_x, cy,
                      get_effective_r_token_color(
                          t.kind, rbytes + brace_offset + 1, post.size()));
                  tok_x += static_cast<float>(surface.get_text_width(
                      device_context, *surface.m_editor_font, post));
                }
              } else {
                surface.draw_text(
                    device_context, *surface.m_editor_font, t.text, tok_x, cy,
                    get_effective_r_token_color(t.kind, rbytes, t.text.size()));

                if (t.kind == UI::Editor::EditorTokenKind::IncludeHeader &&
                    !r_is_line_inactive && t.text.size() > 2) {
                  ...
                  if ((open_c == '<' && close_c == '>') ||
                    ...
                    const int underline_y =
                        round_to_int(cy + line_height * 0.42F);
                    surface.draw_line(
                        device_context, round_to_int(tok_x + open_w),
                        underline_y, round_to_int(tok_x + open_w + inner_w),
                        underline_y, surface.m_palette.include_header);
                  }
                }
            ...
            if (rbytes < lstr.size()) {
              const bool leftover_unused =
                  is_r_token_unused(rbytes, lstr.size() - rbytes);
              surface.draw_text(device_context, *surface.m_editor_font,
            ...
            surface.draw_text(
                device_context, *surface.m_editor_font, lstr, tok_x, cy,
                line_unused
                    ? UI::Theme::dim_color(surface.m_palette.text_primary,
                                           surface.m_palette.editor_background)
                    : surface.m_palette.text_primary);
          ...
          if (!r_is_line_inactive) {
            for (const auto &diag : r_diags) {
              ...
              std::size_t end_col = diag.range.end.line == line_index
                                        ? diag.range.end.character
                                        : lstr.size();
              if (end_col > lstr.size())
                end_col = lstr.size();
              if (start_col >= end_col)
                end_col = std::min(start_col + 1, lstr.size());

              float diag_start_x = right_code_x;
              if (start_col > 0 && start_col <= lstr.size()) {
                diag_start_x += static_cast<float>(surface.get_text_width(
                    device_context, *surface.m_editor_font,
                    lstr.substr(0, start_col)));
              }
              float diag_width = 8.0F;
              if (end_col > start_col && start_col < lstr.size()) {
                diag_width = static_cast<float>(surface.get_text_width(
                    device_context, *surface.m_editor_font,
                    lstr.substr(start_col, end_col - start_col)));
              }

              const UI::Theme::Color squiggle_color =
                  diag.severity == Language::Protocol::DiagnosticSeverity::Error
                      ? UI::Theme::Color{247, 84, 100, 255}
                      : (diag.severity ==
                                 Language::Protocol::DiagnosticSeverity::Warning
              ...
              const float wave_end_x =
                  diag_start_x + std::max(diag_width, 6.0F);
          ...
          if (!r_diags.empty()) {
            ...
            const int line_text_w = surface.get_text_width(
                device_context, *surface.m_editor_font, lstr);
            const bool is_collapsed = (m_split_folding.get_marker(line_index) ==
                                       UI::Components::FoldMarker::Collapsed);
            const float lens_start_x =
                right_code_x + static_cast<float>(line_text_w) +
                (is_collapsed ? 42.0F * scale : 20.0F * scale);
            const float avail_w = std::max(
                right_code_limit - lens_start_x - 16.0F * scale, 80.0F * scale);
            ...
            int msg_w =
                surface.get_text_width(device_context, *surface.m_ui_font, msg);
            const float max_msg_w = std::max(
                avail_w - (pad_x * 2.0F + icon_size + gap), 40.0F * scale);
            if (static_cast<float>(msg_w) > max_msg_w && msg.size() > 8) {
              while (msg.size() > 4 &&
                     static_cast<float>(surface.get_text_width(
                         device_context, *surface.m_ui_font, msg + "...")) >
                         max_msg_w) {
                msg.pop_back();
              }
              msg += "...";
              msg_w = surface.get_text_width(device_context, *surface.m_ui_font,
                                             msg);
            }
            ...
            const UI::Rect badge_rect{lens_start_x, cy - badge_h * 0.5F,
                                      badge_w, badge_h};
          ...
          if (m_split_folding.get_marker(line_index) ==
              UI::Components::FoldMarker::Collapsed) {
            const int line_text_w = surface.get_text_width(
                device_context, *surface.m_editor_font, lstr);
            const float badge_x =
                right_code_x + static_cast<float>(line_text_w) + 8.0F * scale;
            const float badge_w = 26.0F * scale;
            const float badge_h = 16.0F * scale;
            const UI::Rect badge_rect{badge_x, cy - badge_h * 0.5F, badge_w,
                                      badge_h};
            surface.fill_rounded_rectangle(device_context, badge_rect,
                                           UI::Theme::Color{45, 50, 65, 230},
                                           3.0F * scale);
          ...
          if (m_focused && m_caret_blink.is_visible() &&
            ...
            for (const auto &cur : split_doc.get_all_cursors()) {
              if (cur.line == line_index) {
                const std::string_view prefix =
                    lstr.substr(0, std::min(cur.column, lstr.size()));
                const int caret_x = round_to_int(
                    right_code_x +
                    static_cast<float>(surface.get_text_width(
                        device_context, *surface.m_editor_font, prefix)));
                const float mono_char_w = static_cast<float>(
                    std::max(surface.get_text_width(
                                 device_context, *surface.m_editor_font, "X"),
                             1));
                const float font_h =
                    surface.m_editor_font
                        ? static_cast<float>(
                        ...
                        : (16.0F * scale);
                const float block_h =
                    std::min(font_h, line_height - 3.0F * scale);
                const float block_y = cy - block_h * 0.5F;
                ...
                if (cur.column < lstr.size()) {
                  std::size_t char_len = 1;
                  const unsigned char lead =
                      static_cast<unsigned char>(lstr[cur.column]);
                  if ((lead & 0xE0) == 0xC0 && cur.column + 1 < lstr.size())
                    char_len = 2;
                  else if ((lead & 0xF0) == 0xE0 &&
                           cur.column + 2 < lstr.size())
                    char_len = 3;
                  else if ((lead & 0xF8) == 0xF0 &&
                           cur.column + 3 < lstr.size())
                    char_len = 4;

                  char_view = lstr.substr(cur.column, char_len);
                  if (char_view != "\t" && char_view != "\r" &&
                      char_view != "\n") {
                    const int measured = surface.get_text_width(
                        device_context, *surface.m_editor_font, char_view);
                    if (measured > 0) {
                ...
                // ....
                if (effective_cursor == "Block") {
                  const UI::Rect block_rect{static_cast<float>(caret_x),
                                            block_y, char_w, block_h};
                  const UI::Theme::Color block_color{174, 175, 173, 240};
                  surface.fill_rectangle(device_context, block_rect,
                                         block_color);
                  if (!char_view.empty() && char_view != "\t" &&
                      char_view != "\r" && char_view != "\n") {
                    surface.draw_text(device_context, *surface.m_editor_font,
                                      char_view, static_cast<float>(caret_x),
                                      cy, surface.m_palette.editor_background);
                  }
                } else if (effective_cursor == "Underline") {
                  const int u_y = round_to_int(cy + block_h * 0.5F);
                  const int u_w = round_to_int(char_w);
                  surface.draw_line(device_context, caret_x, u_y, caret_x + u_w,
                                    u_y, surface.m_palette.text_primary);
                  surface.draw_line(device_context, caret_x, u_y - 1,
                                    caret_x + u_w, u_y - 1,
                                    surface.m_palette.text_primary);
                } else {
                  surface.draw_line(device_context, caret_x,
                                    round_to_int(block_y), caret_x,
                                    round_to_int(block_y + block_h),
                                    surface.m_palette.text_primary);
                }
              }
            }
  ...
  if (m_completion_popup.is_visible() &&
    ...
    const std::ptrdiff_t vis_row =
        static_cast<std::ptrdiff_t>(physical_line_to_visual_row(
            m_folding, document->get_caret_line(), document->get_line_count()));
    const std::ptrdiff_t first_vis =
        static_cast<std::ptrdiff_t>(m_scrollbar.get_first_visible_line());
    const float caret_line_y =
        layout.editor_bounds.y +
        static_cast<float>(vis_row - first_vis + 1) * line_height;
    ...
    const float popup_h = static_cast<float>(visible_count) * item_h +
                          4.0F * surface.m_dpi_scale; // Compact, no footer
    ...
    for (std::size_t i = 0; i < max_visible && (scroll_offset + i) < count;
         ++i) {
      if (const auto *it = m_completion_popup.get_item(scroll_offset + i)) {
        const int w = surface.get_text_width(device_context, *surface.m_ui_font,
                                             it->label);
    ...
    surface.fill_rounded_rectangle(device_context, actual_bounds, completion_bg,
                                   3.0F * surface.m_dpi_scale);
    ...
    for (std::size_t i = 0; i < max_visible && (scroll_offset + i) < count;
      ...
      if (item_idx == selected) {
        surface.fill_rounded_rectangle(device_context, item_rect,
      ...
      case Language::Protocol::CompletionItemKind::Snippet:
        kind_badge = "[]";
        badge_color =
            UI::Theme::Color{79, 193, 255, 255}; // Cyan Snippet Outline
        is_snippet = true;
        break;
      ...
      case Language::Protocol::CompletionItemKind::Field:
        kind_badge = "v";
        badge_color =
            UI::Theme::Color{156, 220, 254, 255}; // Light Blue Variable
        break;
      ...
      case Language::Protocol::CompletionItemKind::File:
        kind_badge = "h";
        badge_color =
            UI::Theme::Color{156, 220, 254, 255}; // Light Blue File Glyph
        break;
      ...
      // Draw Icon Badge
      surface.draw_text(device_context, *surface.m_ui_font, kind_badge,
                        item_rect.x + 4.0F * surface.m_dpi_scale,
                        row_y + item_h * 0.5F, badge_color);

      // Draw Item Label
      const UI::Theme::Color label_color =
          (item_idx == selected) ? UI::Theme::Color{255, 255, 255, 255}
                                 : surface.m_palette.text_primary;
      surface.draw_text(device_context, *surface.m_editor_font, item->label,
                        item_rect.x + 24.0F * surface.m_dpi_scale,
      ...
      if (is_snippet) {
        ...
        const int detail_w = surface.get_text_width(
            device_context, *surface.m_ui_font, item->detail);
        ...
        surface.draw_text(device_context, *surface.m_ui_font, item->detail,
                          detail_x, row_y + item_h * 0.5F,
                          completion_text_muted);
      }
    }
    ...
    if (count > max_visible) {
      ...
      const UI::Rect thumb_rect{track_x, thumb_y, 3.0F * surface.m_dpi_scale,
                                thumb_h};
      surface.fill_rounded_rectangle(device_context, thumb_rect,
                                     UI::Theme::Color{90, 90, 96, 255},
                                     1.5F * surface.m_dpi_scale);
    }
    ...
    const bool has_doc =
        selected_item != nullptr && !selected_item->documentation.empty();
    const bool has_rich_detail =
        selected_item != nullptr && !selected_item->detail.empty() &&
        selected_item->kind != Language::Protocol::CompletionItemKind::File &&
        selected_item->kind != Language::Protocol::CompletionItemKind::Folder &&
        selected_item->kind != Language::Protocol::CompletionItemKind::Text;

    if (selected_item != nullptr && (has_doc || has_rich_detail)) {
      const float detail_pad = 8.0F * surface.m_dpi_scale;
      const float detail_w = 320.0F * surface.m_dpi_scale;

      // Check if there is enough space on the right, otherwise place on the
      // left
      float detail_x = actual_bounds.right() + 4.0F * surface.m_dpi_scale;
      bool can_fit = true;
      if (detail_x + detail_w >
          layout.editor_bounds.right() - 4.0F * surface.m_dpi_scale) {
        detail_x = actual_bounds.x - detail_w - 4.0F * surface.m_dpi_scale;
        if (detail_x < layout.editor_bounds.x + 4.0F * surface.m_dpi_scale) {
          can_fit = false; // Never overlap the completion list
        }
      }
      ...
      if (can_fit) {
        ...
        if (!selected_item->documentation.empty()) {
          ...
          while (std::getline(ss, line)) {
            ...
            while (words_ss >> word) {
              ...
              int text_w = surface.get_text_width(device_context,
                                                  *surface.m_ui_font, test);
        ...
        // Background & Border
        surface.fill_rounded_rectangle(device_context, detail_bounds,
                                       UI::Theme::Color{22, 22, 26, 255},
                                       3.0F * surface.m_dpi_scale);
        surface.draw_rectangle(device_context, detail_bounds,
                               completion_border);
        ...
        surface.draw_text(device_context, *surface.m_editor_font, header_text,
                          cursor_x, cursor_y,
                          UI::Theme::Color{230, 230, 235, 255});

        // Divider line
        const float sep_y = detail_bounds.y + header_h;
        surface.draw_line(device_context, round_to_int(detail_bounds.x),
                          round_to_int(sep_y),
                          round_to_int(detail_bounds.right()),
                          round_to_int(sep_y), completion_border);
  ...
  // Render Parameter Hint / Signature Help tooltip (e.g.
  ...
  if (m_signature_help.is_visible() &&
    ...
    if (open_count <= close_count) {
      ...
      const int text_w = surface.get_text_width(
          device_context, *surface.m_editor_font, sig.label);
      const float hint_w =
      ...
      if (hint_y < layout.editor_bounds.y + 2.0F) {
        if (popup_visible) {
          ...
          // Place signature help either right at the top edge above line or
          // below the whole completion popup
          const std::size_t count = m_completion_popup.get_item_count();
          const std::size_t max_visible =
              m_completion_popup.get_max_visible_items();
          const std::size_t visible_count =
              std::min<std::size_t>(count, max_visible);
          const float popup_h = static_cast<float>(visible_count) * line_h +
                                4.0F * surface.m_dpi_scale;
      ...
      surface.fill_rounded_rectangle(device_context, hint_bounds, hint_bg,
                                     3.0F * surface.m_dpi_scale);
      surface.draw_rectangle(device_context, hint_bounds, hint_border);

      // Parameter hint text in teal/cyan editor font
      surface.draw_text(device_context, *surface.m_editor_font, sig.label,
    ...
    const std::filesystem::path left_path =
        (active_p && !active_p->empty())
            ...
            : (document ? std::filesystem::path(
                        ...
                        : std::filesystem::path{});
    if (!left_path.empty()) {
      draw_media_pane(surface, device_context, layout.workspace_bounds,
                      left_path);
      return;
    }
  }
...
void TextEditor::draw_diagnostic_hover_overlay(
  ...
  const int msg_w =
      surface.get_text_width(device_context, *surface.m_ui_font, msg);
  const int src_w =
      surface.get_text_width(device_context, *surface.m_small_font, source_tag);
  ...
  // Calculate popup position (prefer above the anchor, fall back to below if
  // too close to top)
  ...
  surface.fill_rounded_rectangle(
      ...
      UI::Theme::Color{0, 0, 0, 130}, 6.0F * scale);
  ...
  surface.fill_rounded_rectangle(device_context,
                                 UI::Rect{card_x, card_y + 4.0F * scale,
                                          3.0F * scale, card_h - 8.0F * scale},
                                 accent_color, 1.5F * scale);
  ...
  if (static_cast<float>(msg_w) > max_msg_w && display_msg.size() > 10) {
    while (display_msg.size() > 6 && static_cast<float>(surface.get_text_width(
                                         device_context, *surface.m_ui_font,
                                         display_msg + "...")) > max_msg_w) {
      display_msg.pop_back();
    }
    display_msg += "...";
  }

  surface.draw_text(device_context, *surface.m_ui_font, display_msg,
                    card_x + card_padding, row1_y,
                    UI::Theme::Color{230, 235, 245, 255});

  const float src_x =
      card_x + card_w - static_cast<float>(src_w) - card_padding;
  surface.draw_text(device_context, *surface.m_small_font, source_tag, src_x,
                    row1_y, UI::Theme::Color{80, 155, 245, 255});

  // Divider Line
  const float div_y = card_y + 32.0F * scale;
  surface.draw_line(device_context, round_to_int(card_x + 10.0F * scale),
                    round_to_int(div_y),
                    round_to_int(card_x + card_w - 10.0F * scale),
                    round_to_int(div_y), UI::Theme::Color{46, 50, 64, 255});

  // 4. Middle row (if symbol / include info exists)
  float actions_y = card_y + 48.0F * scale;
  if (has_symbol) {
    const float sym_row_y = card_y + 45.0F * scale;
    const int sym_text_w = surface.get_text_width(
        device_context, *surface.m_small_font, info.symbol_name);
    const float sym_pill_w = static_cast<float>(sym_text_w) + 12.0F * scale;
    const float sym_pill_h = 16.0F * scale;
    const UI::Rect sym_pill_rect{card_x + card_padding,
                                 sym_row_y - sym_pill_h * 0.5F, sym_pill_w,
                                 sym_pill_h};

    surface.fill_rounded_rectangle(device_context, sym_pill_rect,
                                   UI::Theme::Color{42, 45, 58, 255},
                                   3.0F * scale);
    surface.draw_text(device_context, *surface.m_small_font, info.symbol_name,
                      sym_pill_rect.x + 6.0F * scale, sym_row_y,
    ...
    while (!detail_text.empty() &&
           (detail_text.front() == ' ' || detail_text.front() == '\t'))
      detail_text.erase(detail_text.begin());
    const float max_det_w = card_x + card_w - detail_x - card_padding;
    if (static_cast<float>(surface.get_text_width(
            device_context, *surface.m_small_font, detail_text)) > max_det_w &&
        detail_text.size() > 10) {
      while (detail_text.size() > 6 &&
             static_cast<float>(surface.get_text_width(
                 device_context, *surface.m_small_font, detail_text + "...")) >
                 max_det_w) {
        detail_text.pop_back();
      }
      detail_text += "...";
    }
    surface.draw_text(device_context, *surface.m_small_font, detail_text,
                      detail_x, sym_row_y,
  ...
  // 5. Bottom Action Bar: [Explain and Fix (Ctrl+Shift+.)]  View Problem
  // (Alt+F8)  Quick Fix... (Ctrl+.)
  const std::string btn_label = "Explain and Fix (Ctrl+Shift+.)";
  const int btn_text_w =
      surface.get_text_width(device_context, *surface.m_small_font, btn_label);
  const float btn_w = static_cast<float>(btn_text_w) + 16.0F * scale;
  const float btn_h = 20.0F * scale;
  const UI::Rect btn_rect{card_x + card_padding, actions_y - btn_h * 0.5F,
                          btn_w, btn_h};
  ...
  surface.fill_rounded_rectangle(
      ...
      3.0F * scale);
  surface.draw_text(device_context, *surface.m_small_font, btn_label,
                    btn_rect.x + 8.0F * scale, actions_y,
                    UI::Theme::Color{255, 255, 255, 255});

  // Secondary Action Links
  float link_x = btn_rect.right() + 12.0F * scale;
  const std::string link1 = "View Problem (Alt+F8)";
  surface.draw_text(device_context, *surface.m_small_font, link1, link_x,
                    actions_y, UI::Theme::Color{80, 160, 250, 255});

  link_x += static_cast<float>(surface.get_text_width(
                device_context, *surface.m_small_font, link1)) +
            14.0F * scale;
  const std::string link2 = "Quick Fix... (Ctrl+.)";
  surface.draw_text(device_context, *surface.m_small_font, link2, link_x,
                    actions_y, UI::Theme::Color{80, 160, 250, 255});
}
...
void TextEditor::draw_hover_tooltip(
      ...
      if (code_end != std::string::npos) {
        ...
        while (std::getline(stream, line)) {
          ...
          while (!line.empty() && (line.back() == ' ' || line.back() == '\t'))
            line.pop_back();
          if (line.empty() || line == "---" || line == "***" || line == "___")
            continue;
  ...
  if (signature.empty() && doc_lines.empty()) {
    return;
  }

  const float pad_x = 14.0F * scale;
  const float pad_y = 10.0F * scale;
  const float header_line_h = 22.0F * scale;
  const float doc_line_h = 19.0F * scale;

  const float max_card_w = std::min(
      760.0F * scale,
      std::max(300.0F * scale, layout.editor_bounds.width - 32.0F * scale));

  float max_content_w = 260.0F * scale;
  if (!signature.empty()) {
    const int sig_w = surface.get_text_width(device_context,
                                             *surface.m_editor_font, signature);
    max_content_w = std::max(max_content_w, static_cast<float>(sig_w));
  }
  for (const auto &dl : doc_lines) {
    const int dw =
        surface.get_text_width(device_context, *surface.m_ui_font, dl);
    max_content_w = std::max(max_content_w, static_cast<float>(dw));
  }

  const float card_w =
      std::clamp(max_content_w + pad_x * 2.0F, 280.0F * scale, max_card_w);
  const float inner_text_w = card_w - pad_x * 2.0F;

  float card_h = pad_y * 2.0F;
  if (!signature.empty()) {
    card_h += header_line_h;
  }
  if (!doc_lines.empty()) {
    if (!signature.empty())
      card_h += 10.0F * scale;
    card_h += static_cast<float>(std::min<std::size_t>(doc_lines.size(), 8)) *
              doc_line_h;
  }

  const float card_x = std::clamp(
      m_hover_screen_x, layout.editor_bounds.x + 8.0F * scale,
      std::max(layout.editor_bounds.x + 8.0F * scale,
               layout.editor_bounds.right() - card_w - 8.0F * scale));
  float card_y = m_hover_screen_y - card_h - 8.0F * scale;
  if (card_y < layout.editor_bounds.y + 4.0F * scale) {
    card_y = m_hover_screen_y + 24.0F * scale;
  }
  if (card_y + card_h > layout.editor_bounds.bottom() - 4.0F * scale) {
    card_y = std::max(layout.editor_bounds.y + 4.0F * scale,
                      layout.editor_bounds.bottom() - card_h - 4.0F * scale);
  }
  ...
  surface.fill_rounded_rectangle(
      device_context,
      UI::Rect{card_bounds.x + 2.0F * scale, card_bounds.y + 4.0F * scale,
               card_bounds.width, card_bounds.height},
      UI::Theme::Color{0, 0, 0, 90}, 8.0F * scale);
  surface.fill_rounded_rectangle(
      device_context,
      UI::Rect{card_bounds.x + 1.0F * scale, card_bounds.y + 2.0F * scale,
               card_bounds.width, card_bounds.height},
      UI::Theme::Color{0, 0, 0, 140}, 6.0F * scale);

  // 2. Card Background & Border
  const UI::Theme::Color bg{24, 26, 32, 252};
  const UI::Theme::Color border{56, 60, 74, 255};
  surface.fill_rounded_rectangle(device_context, card_bounds, bg, 6.0F * scale);
  surface.draw_rectangle(device_context, card_bounds, border);

  // 3. Strict Clipping Container
  const int save_dc = SaveDC(device_context);
  RECT clip_rc{static_cast<int>(card_bounds.x), static_cast<int>(card_bounds.y),
               static_cast<int>(card_bounds.right()),
               static_cast<int>(card_bounds.bottom())};
  IntersectClipRect(device_context, clip_rc.left, clip_rc.top, clip_rc.right,
                    clip_rc.bottom);

  float cur_y = card_bounds.y + pad_y;

  // 4. Signature / Path Section
  if (!signature.empty()) {
    std::string display_sig = signature;
    int current_sig_w = surface.get_text_width(
        device_context, *surface.m_editor_font, display_sig);
    if (current_sig_w > static_cast<int>(inner_text_w) &&
        display_sig.size() > 20) {
      const std::size_t half = display_sig.size() / 2;
      std::size_t cut_len = 4;
      while (cut_len < display_sig.size() - 8) {
        std::string test = display_sig.substr(0, half - cut_len / 2) + "..." +
                           display_sig.substr(half + cut_len / 2);
        if (surface.get_text_width(device_context, *surface.m_editor_font,
                                   test) <= static_cast<int>(inner_text_w)) {
          display_sig = test;
          break;
        }
        cut_len += 4;
    ...
    surface.draw_text(device_context, *surface.m_editor_font, display_sig,
                      card_bounds.x + pad_x, cur_y + header_line_h * 0.5F,
                      UI::Theme::Color{86, 182, 194, 255});
    cur_y += header_line_h;
  }
  ...
  if (!doc_lines.empty()) {
    if (!signature.empty()) {
      cur_y += 5.0F * scale;
      surface.draw_line(device_context, card_bounds.x + 8.0F * scale, cur_y,
                        card_bounds.right() - 8.0F * scale, cur_y,
                        UI::Theme::Color{46, 50, 62, 255});
      cur_y += 5.0F * scale;
    }

    for (std::size_t i = 0; i < std::min<std::size_t>(doc_lines.size(), 8);
         ++i) {
      surface.draw_text(device_context, *surface.m_ui_font, doc_lines[i],
                        card_bounds.x + pad_x, cur_y + doc_line_h * 0.5F,
                        UI::Theme::Color{214, 218, 228, 255});
      cur_y += doc_line_h;
    }
  }

  RestoreDC(device_context, save_dc);
}
