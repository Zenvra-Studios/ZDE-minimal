...
public:
    ...
    [[nodiscard]] bool initialize(Display* display, int screen, float dpi_scale, const std::filesystem::path& icon_asset_root = {});

    void open(Window parent_window, const std::filesystem::path& target_folder, const std::string& project_name, CreateCallback callback);
    void close();
...
private:
    ...
    void draw_icon(Drawable drawable, const std::string& path, int x, int y, int size,
                   uint8_t bg_r = 27, uint8_t bg_g = 27, uint8_t bg_b = 30);
