...
bool TextEditor::handle_pointer_press(
  ...
  if (is_split_active) {
    ...
    if (right_doc != nullptr && right_bounds.contains(point_x, point_y)) {
      ...
      if (point_x <= splitter_x + 2.0F * scale + right_gutter_w) {
        ...
        if (point_x >= fold_margin_left && m_split_folding.is_fold_start(split_line)) {
        ...
        if (split_line < split_total_lines) {
          right_doc->toggle_breakpoint(split_line);
          return true;
        }
      }
  ...
  // Single view: Handle Folding / Breakpoint Click in Gutter
  if (layout.gutter_bounds.contains(point_x, point_y)) {
    auto *doc = get_focused_document();
    if (doc) {
      m_folding.rebuild(doc->get_lines(), 4);
      ...
      if (line_index < total_lines) {
        doc->toggle_breakpoint(line_index);
        m_reveal_caret_pending = false;
        return true;
      }
    }
...
void TextEditor::draw_tab_strip(
  ...
  auto draw_single_tab = [&](std::size_t tab_index) {
    ...
    if (is_modern) {
      const float v_pad = std::round(4.0F * scale);
      const float h_pad = std::round(2.0F * scale);
      ...
      const float pill_radius =
          std::max(std::round(6.0F * scale), 5.0F);

      if (is_dragging_this) {
        const UI::Rect shadow_rect{pill_bounds.x + 2.0F * scale,
                                   pill_bounds.y + 2.0F * scale,
                                   pill_bounds.width, pill_bounds.height};
        surface.fill_rounded_rectangle(drawable, shadow_rect,
                                       surface.allocate_color(UI::Theme::Color{0, 0, 0, 90}),
                                       pill_radius, surface.m_pixels.workspace_background);
        surface.fill_rounded_rectangle(drawable, pill_bounds,
                                       surface.allocate_color(UI::Theme::Color{45, 48, 56, 255}),
                                       pill_radius, surface.m_pixels.workspace_background);
        surface.draw_rounded_rectangle(drawable, pill_bounds,
                                       surface.allocate_color(UI::Theme::Color{53, 132, 228, 230}),
                                       pill_radius);
      } else if (active) {
        ...
        surface.fill_rounded_rectangle(drawable, pill_bounds,
                                       surface.allocate_color(active_bg), pill_radius,
                                       surface.m_pixels.workspace_background);
        ...
        surface.draw_rounded_rectangle(drawable, pill_bounds,
                                       surface.allocate_color(border_col), pill_radius);
      } else if (tab_hovered) {
        // JetBrains inactive hover pill
        const UI::Theme::Color hover_tint = surface.m_theme.is_dark
            ? UI::Theme::Color{255, 255, 255, 20}
            : UI::Theme::Color{0, 0, 0, 15};
        const uint32_t a = hover_tint.alpha;
        const auto& tbg = surface.m_palette.workspace_background;
        const UI::Theme::Color blended_hover_bg{
            static_cast<uint8_t>((hover_tint.red * a + tbg.red * (255 - a) + 127) / 255),
            static_cast<uint8_t>((hover_tint.green * a + tbg.green * (255 - a) + 127) / 255),
            static_cast<uint8_t>((hover_tint.blue * a + tbg.blue * (255 - a) + 127) / 255),
            255
        };
        surface.fill_rounded_rectangle(drawable, pill_bounds,
                                       surface.allocate_color(blended_hover_bg), pill_radius,
                                       surface.m_pixels.workspace_background);
      }
    } else {
      if (is_dragging_this) {
    ...
    const UI::Theme::Color tab_bg_color = is_modern
        ? (active ? (surface.m_theme.is_dark ? UI::Theme::Color{36, 38, 44, 255} : UI::Theme::Color{255, 255, 255, 255})
                  : surface.m_theme.window_background)
...
void TextEditor::render_pane(const StudioWorkspaceRenderer &surface,
  ...
  const auto token_color =
      [&](UI::Editor::EditorTokenKind kind, bool is_dimmed = false) -> const std::string & {
  ...
  for (std::size_t line_index = first_line;
    ...
    const float bp_lane_width = 20.0F * scale;
    const float bp_x = gutter_rect.x + 10.0F * scale;
    const float bp_r = 4.5F * scale;

    // Line number (right-aligned against gutter_line_x with 10px margin, matching VS Code spacing)
    const std::string line_num = std::to_string(line_index + 1);
    const int num_w = surface.m_editor_font->getTextWidth(line_num);
    const float line_num_right = gutter_line_x - 10.0F * scale;
    const float number_x =
        std::max(line_num_right - static_cast<float>(num_w), gutter_rect.x + bp_lane_width);
    surface.draw_text(
        drawable, *surface.m_editor_font, line_num, number_x, center_y,
        is_active_line ? surface.m_text.primary : surface.m_text.muted);

    // Breakpoint marker in gutter (active red circle or hover preview) centered in dedicated breakpoint lane
    const bool has_bp = doc->has_breakpoint(line_index);
    const bool is_bp_hovered = (!has_bp && m_hovered_gutter_line && *m_hovered_gutter_line == line_index);
    if (has_bp || is_bp_hovered) {
      const unsigned long bp_color = has_bp
          ? surface.allocate_color(UI::Theme::Color{229, 20, 0, 255})
          : surface.allocate_color(UI::Theme::Color{130, 20, 20, 255});
      surface.fill_rounded_rectangle(
          drawable,
  ...
  for (std::size_t line_index = first_line;
    ...
    if (syntax_highlighting) {
      ...
      if (rendered_bytes < line.size()) {
        ...
        surface.draw_text(
            drawable, *surface.m_editor_font, line.substr(rendered_bytes),
            token_x, center_y,
            leftover_unused ? surface.m_text_dimmed.primary : surface.m_text.primary,
            &code_clip);
      }
    } else {
      ...
      surface.draw_text(
          ...
          line_unused ? surface.m_text_dimmed.primary : surface.m_text.primary,
          &code_clip);
    }
