    ...
    const UI::Editor::StudioEditorLayoutResult& layout) const
{
        ...
        const bool close_hovered = m_hovered_tab_close_index &&
                ...
                surface.fill_rectangle(
                    ...
                    active ? surface.m_pixels.tab_active_background
                           : (tab_hovered ? surface.m_pixels.active_line_background : surface.m_pixels.tab_background));
    ...
    if (active_index && *active_index < m_tab_count)
    {
        ...
        surface.draw_line(drawable, tab_bar_left, tab_bar_bottom, active_left, tab_bar_bottom, surface.m_pixels.border);
        surface.draw_line(drawable, active_right, tab_bar_bottom, tab_bar_right, tab_bar_bottom, surface.m_pixels.border);
    }
    else
    {
        surface.draw_line(drawable, tab_bar_left, tab_bar_bottom, tab_bar_right, tab_bar_bottom, surface.m_pixels.border);
    }
}
    ...
    const UI::Editor::StudioEditorLayoutResult& layout) const
{
    ...
    if (document == nullptr)
    {
        ...
        surface.draw_png_icon(drawable, "zenvra_logo.png", center_x, current_y + logo_size / 2, logo_size, surface.m_palette.editor_background);
        ...
        if (surface.m_ui_font)
        {
            ...
            if (surface.m_large_font)
            {
                ...
                surface.draw_text(drawable, *surface.m_large_font, title, static_cast<float>(center_x - title_w / 2), static_cast<float>(current_y), surface.m_text.primary);
            }
            else
            {
                int title_w = surface.m_ui_font->getTextWidth(title);
                surface.draw_text(drawable, *surface.m_ui_font, title, static_cast<float>(center_x - title_w / 2), static_cast<float>(current_y), surface.m_text.primary);
            }
            current_y += gap2;
            ...
            for (int i = 0; i < 4; ++i)
            {
                int sc_w1 = surface.m_ui_font->getTextWidth(shortcuts[i].first);
                int sc_x1 = center_x - round_to_int(20.0F * dpi) - sc_w1;
                int sc_x2 = center_x + round_to_int(20.0F * dpi);
                surface.draw_text(drawable, *surface.m_ui_font, shortcuts[i].first, static_cast<float>(sc_x1), static_cast<float>(current_y), surface.m_text.muted);
                surface.draw_text(drawable, *surface.m_ui_font, shortcuts[i].second, static_cast<float>(sc_x2), static_cast<float>(current_y), surface.m_text.muted);
    ...
    // Collect selection target rects (relative to code_x) and feed the animation model.
    ...
    if (m_selection_animation.has_rects())
    {
        ...
        for (const UI::Rect& anim_rect : m_selection_animation.get_animated_rects())
        {
            ...
            const float screen_y = layout.editor_bounds.y + anim_rect.y -
                static_cast<float>(first_line) * line_height;
            ...
            for (std::size_t token_index = 0; token_index < token_count; ++token_index)
            {
                ...
                if (has_animated_brace)
                {
                    if (brace_offset > 0)
                    {
                        ...
                        surface.draw_text(drawable, *surface.m_editor_font, pre, token_x, center_y,
                            token_color(token.kind));
                        token_x += static_cast<float>(surface.m_editor_font->getTextWidth(pre));
                    }
                    ...
                    if (!m_brace_pulse_color_ready)
                    {
                        ...
                        pulse_color.red = static_cast<std::uint8_t>(
                            std::min(pulse_color.red + 30, 255));
                        pulse_color.green = static_cast<std::uint8_t>(
                            std::min(pulse_color.green + 30, 255));
                        pulse_color.blue = static_cast<std::uint8_t>(
                            std::min(pulse_color.blue + 30, 255));
                    ...
                    // X11 cannot scale glyphs; draw the brace at normal size over the pulse box.
                    surface.draw_text(drawable, *surface.m_editor_font, brace_char, token_x,
                        center_y, surface.m_text.accent);
                    token_x += brace_w;

                    if (brace_offset + 1 < token.text.size())
                    {
                        const std::string post{token.text.substr(brace_offset + 1)};
                        surface.draw_text(drawable, *surface.m_editor_font, post, token_x, center_y,
                            token_color(token.kind));
                        token_x += static_cast<float>(surface.m_editor_font->getTextWidth(post));
                    }
                }
                else
                {
