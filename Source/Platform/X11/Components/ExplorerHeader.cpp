    ...
    bool show_actions) const
{
    ...
    auto draw_icon = [&](ActionIcon icon_type, const char* path, float center_x) {
        ...
        if (is_hovered) {
            ...
            surface.fill_rounded_rectangle(drawable, hover_bg, surface.m_pixels.hover_background, 3.0F * scale, surface.m_pixels.sidebar_background);
        }
        
        surface.draw_svg_icon(drawable, path, round_to_int(center_x), header_center_y, icon_size, color, bg_color);
    };
...
{
    ActionIcon hovered = get_icon_at_point(layout, point_x, point_y, show_actions);
