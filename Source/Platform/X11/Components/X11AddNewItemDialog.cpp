...
bool X11AddNewItemDialog::initialize(Display* display, int screen, float dpi_scale, const std::filesystem::path& icon_asset_root) {
    m_display = display;
    m_screen = screen;
    ...
    m_bold_font = std::make_unique<AntialiasedFont>(
...
void X11AddNewItemDialog::open(Window parent_window, const std::filesystem::path& target_folder, const std::string& project_name, CreateCallback callback) {
    close();
    ...
    int root_y = 0;
    Window child = 0;
    XTranslateCoordinates(m_display, parent_window, RootWindow(m_display, m_screen),
    ...
    m_window = XCreateWindow(
        ...
        m_win_x, m_win_y, static_cast<unsigned int>(m_width), static_cast<unsigned int>(m_height),
    ...
    m_close_btn_rect = {static_cast<float>(m_width) - 46.0F * scale, 0.0F, 46.0F * scale, titlebar_h};

    m_category_pane_rect = {0.0F, pane_top, cat_w, pane_h};
    m_template_pane_rect = {cat_w, pane_top, tpl_w, pane_h};
    m_details_pane_rect = {cat_w + tpl_w, pane_top, static_cast<float>(m_width) - (cat_w + tpl_w), pane_h};

    const float footer_top = static_cast<float>(m_height) - footer_h;
    m_name_input_rect = {76.0F * scale, footer_top + 10.0F * scale, static_cast<float>(m_width) - 260.0F * scale, 26.0F * scale};
    m_add_btn_rect = {static_cast<float>(m_width) - 170.0F * scale, footer_top + 10.0F * scale, 75.0F * scale, 26.0F * scale};
    m_cancel_btn_rect = {static_cast<float>(m_width) - 85.0F * scale, footer_top + 10.0F * scale, 75.0F * scale, 26.0F * scale};
...
void X11AddNewItemDialog::draw_icon(Drawable drawable, const std::string& path, int x, int y, int size,
...
void X11AddNewItemDialog::render() {
    ...
    const float scale = m_dpi_scale;
    const unsigned long bg_col = alloc_rgb(m_display, m_screen, 31, 31, 31);          // Theme window_background #1f1f1f
    const unsigned long titlebar_bg = alloc_rgb(m_display, m_screen, 29, 30, 33);     // Theme titlebar_background #1d1e21
    const unsigned long border_col = alloc_rgb(m_display, m_screen, 48, 50, 55);      // Theme titlebar_border
    const unsigned long sep_col = alloc_rgb(m_display, m_screen, 48, 50, 55);         // Theme titlebar_border
    const unsigned long sel_cat_blue = alloc_rgb(m_display, m_screen, 0, 122, 204);   // Synchronized Menubar Accent Blue #007acc
    const unsigned long sel_tpl_blue = alloc_rgb(m_display, m_screen, 0, 122, 204);   // Synchronized Menubar Accent Blue #007acc
    const unsigned long hov_item_bg = alloc_rgb(m_display, m_screen, 45, 45, 45);     // Theme hover #2d2d2d
    ...
    XFillRectangle(m_display, m_back_buffer, m_gc, 0, 0,
                   static_cast<unsigned int>(m_width), static_cast<unsigned int>(m_titlebar_rect.height));
    XSetForeground(m_display, m_gc, sep_col);
    XDrawLine(m_display, m_back_buffer, m_gc, 0, static_cast<int>(m_titlebar_rect.height - 1),
              m_width, static_cast<int>(m_titlebar_rect.height - 1));
    ...
    if (m_close_hovered) {
        ...
        XFillRectangle(m_display, m_back_buffer, m_gc,
                       static_cast<int>(m_close_btn_rect.x), static_cast<int>(m_close_btn_rect.y),
    ...
    const int pane_bottom_y = static_cast<int>(m_titlebar_rect.height + m_category_pane_rect.height);
    XSetForeground(m_display, m_gc, sep_col);
    ...
    for (std::size_t i = 0; i < m_categories.size(); ++i) {
        ...
        const bool is_hov = (m_hovered_category_index && *m_hovered_category_index == i);

        if (is_sel) {
            XSetForeground(m_display, m_gc, sel_cat_blue);
            XFillRectangle(m_display, m_back_buffer, m_gc,
                           static_cast<int>(item_rect.x), static_cast<int>(item_rect.y),
        ...
        draw_icon(m_back_buffer, cat.icon_path, static_cast<int>(item_rect.x + 12.0F * scale),
                  static_cast<int>(item_rect.y + 6.0F * scale), static_cast<int>(16.0F * scale),
                  bg_r, bg_g, bg_b);
    ...
    if (m_selected_category_index < m_categories.size()) {
        ...
        for (std::size_t i = 0; i < tpls.size(); ++i) {
            ...
            const bool is_hov = (m_hovered_template_index && *m_hovered_template_index == i);
            ...
            draw_icon(m_back_buffer, tpl.icon_path, static_cast<int>(item_rect.x + 12.0F * scale),
                      static_cast<int>(item_rect.y + 6.0F * scale), static_cast<int>(16.0F * scale),
                      bg_r, bg_g, bg_b);
            ...
            if (m_ui_font && m_ui_font->isValid()) {
                ...
                if (m_small_font && m_small_font->isValid()) {
                    ...
                    m_small_font->drawString(m_back_buffer, is_sel ? "#d0e6ff" : "#787884",
    ...
    if (m_selected_category_index < m_categories.size()) {
        ...
        if (m_selected_template_index < tpls.size()) {
            ...
            if (m_ui_font && m_ui_font->isValid()) {
                ...
                std::string word;
                const int max_desc_w = static_cast<int>(m_details_pane_rect.width - 32.0F * scale);

                while (stream >> word) {
                    std::string test_line = current_line.empty() ? word : current_line + " " + word;
                    if (m_ui_font->getTextWidth(test_line) > max_desc_w && !current_line.empty()) {
                ...
                for (const auto& line : lines) {
                    m_ui_font->drawString(m_back_buffer, "#a0a0aa",
                                          static_cast<int>(m_details_pane_rect.x + 16.0F * scale),
                                          dy, line);
    ...
    // Name Label
    if (m_ui_font && m_ui_font->isValid()) {
        m_ui_font->drawString(m_back_buffer, "#b4b4b4",
                              static_cast<int>(18.0F * scale),
                              static_cast<int>(footer_top + 24.0F * scale),
                              "Name:");
    }
    ...
    XFillRectangle(m_display, m_back_buffer, m_gc,
                   static_cast<int>(m_name_input_rect.x), static_cast<int>(m_name_input_rect.y),
    ...
    if (m_ui_font && m_ui_font->isValid()) {
        m_ui_font->drawString(m_back_buffer, "#ffffff", text_x, text_y, m_filename_input);
        ...
        XDrawLine(m_display, m_back_buffer, m_gc,
                  caret_x, static_cast<int>(m_name_input_rect.y + 4.0F * scale),
                  caret_x, static_cast<int>(m_name_input_rect.bottom() - 4.0F * scale));
    }

    // Location label & value
    if (m_small_font && m_small_font->isValid()) {
        m_small_font->drawString(m_back_buffer, "#888890",
                                 static_cast<int>(18.0F * scale),
                                 static_cast<int>(footer_top + 52.0F * scale),
                                 "Location:");

        std::string loc_str = m_target_folder.string();
        const int max_loc_w = static_cast<int>(static_cast<float>(m_width) - (76.0F * scale + 24.0F * scale));
        if (m_small_font->getTextWidth(loc_str) > max_loc_w && loc_str.size() > 10) {
            while (loc_str.size() > 10 && m_small_font->getTextWidth("..." + loc_str) > max_loc_w) {
                loc_str = loc_str.substr(1);
            }
            loc_str = "..." + loc_str;
        }

        m_small_font->drawString(m_back_buffer, "#a0a0b0",
    ...
    const unsigned long add_bg = m_add_hovered ? alloc_rgb(m_display, m_screen, 28, 151, 234)
                                                : sel_tpl_blue;
    XSetForeground(m_display, m_gc, add_bg);
    XFillRectangle(m_display, m_back_buffer, m_gc,
                   static_cast<int>(m_add_btn_rect.x), static_cast<int>(m_add_btn_rect.y),
    ...
    const unsigned long cancel_bg = m_cancel_hovered ? alloc_rgb(m_display, m_screen, 58, 58, 66)
                                                     : alloc_rgb(m_display, m_screen, 45, 45, 50);
    XSetForeground(m_display, m_gc, cancel_bg);
    XFillRectangle(m_display, m_back_buffer, m_gc,
                   static_cast<int>(m_cancel_btn_rect.x), static_cast<int>(m_cancel_btn_rect.y),
                   static_cast<unsigned int>(m_cancel_btn_rect.width),
                   static_cast<unsigned int>(m_cancel_btn_rect.height));
    XSetForeground(m_display, m_gc, alloc_rgb(m_display, m_screen, 60, 64, 75));
    XDrawRectangle(m_display, m_back_buffer, m_gc,
                   static_cast<int>(m_cancel_btn_rect.x), static_cast<int>(m_cancel_btn_rect.y),
                   static_cast<unsigned int>(m_cancel_btn_rect.width),
                   static_cast<unsigned int>(m_cancel_btn_rect.height));
...
bool X11AddNewItemDialog::handle_event(const XEvent& event) {
    ...
    case MotionNotify: {
        ...
        if (new_close_h != m_close_hovered || new_add_h != m_add_hovered ||
            new_cancel_h != m_cancel_hovered || new_cat_h != m_hovered_category_index ||
