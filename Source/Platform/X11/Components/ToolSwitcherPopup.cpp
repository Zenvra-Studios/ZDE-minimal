...
void ToolSwitcherPopup::refresh_items()
{
    ...
    for (const auto& p : pm.get_installed_tool_plugins())
    {
        ...
        {
            item.icon_asset = "Assets/icons/material-icon-theme/docker.svg";
        }
        else if (name_lower.find("qemu") != std::string::npos || item.id.find("qemu") != std::string::npos)
        {
            item.icon_asset = "Assets/icons/vscode-codicons/icons/chip.svg";
        }
        else if (name_lower.find("gdb") != std::string::npos || name_lower.find("debug") != std::string::npos)
        {
            item.icon_asset = "Assets/icons/vscode-codicons/icons/debug-alt.svg";
        }
        else if (name_lower.find("wasm") != std::string::npos)
        {
            item.icon_asset = "Assets/icons/vscode-codicons/icons/server.svg";
        }
        ...
        {
            item.icon_asset = "Assets/icons/vscode-codicons/icons/tools.svg";
        }
    ...
    float dpi_scale) const
{
    ...
    const UI::Theme::Color bg{28, 29, 32, 255};
    surface.fill_rounded_rectangle(
        drawable, bounds, surface.allocate_color(bg), 6.0F * dpi_scale);
    surface.draw_rounded_rectangle(drawable, bounds, surface.m_pixels.border, 6.0F * dpi_scale);
    ...
    else
    {
        for (std::size_t i = 0; i < m_items.size(); ++i)
        {
            ...
            {
                surface.fill_rounded_rectangle(
                    drawable, row_rect, surface.m_pixels.hover_background,
                    4.0F * dpi_scale, surface.m_pixels.sidebar_background);
            }
            ...
            if (surface.m_small_font) {
                surface.draw_text(
                    ...
                    it.is_active ? surface.m_text.primary : surface.m_text.muted);
            }
            ...
            if (it.is_active)
            {
                surface.draw_svg_icon(
                    drawable,
                    "Assets/icons/vscode-codicons/icons/check.svg",
                    round_to_int(row_rect.right() - 14.0F * dpi_scale),
                    round_to_int(row_rect.y + row_rect.height * 0.5F),
                    round_to_int(12.0F * dpi_scale),
                    UI::Theme::Color{59, 130, 246, 255},
                    surface.m_palette.sidebar_background,
                    false);
            }
    ...
    {
        surface.fill_rounded_rectangle(
            drawable, footer_rect, surface.m_pixels.hover_background,
            4.0F * dpi_scale, surface.m_pixels.sidebar_background);
    }

    surface.draw_svg_icon(
        drawable,
        "Assets/icons/vscode-codicons/icons/extensions.svg",
