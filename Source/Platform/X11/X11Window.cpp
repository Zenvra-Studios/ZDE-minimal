...
constexpr long motif_hints_decorations = 1L << 1;
...
Bool event_matches_window(Display* display, XEvent* event, XPointer target_data)
{
...
bool X11Window::initialize()
{
    ...
    m_window_handle = XCreateSimpleWindow(
        ...
        0,
        0,
        static_cast<unsigned int>(m_client_width),
        static_cast<unsigned int>(m_client_height),
        0,
    ...
    XChangeProperty(
        ...
        32,
        PropModeReplace,
        reinterpret_cast<const unsigned char*>(&process_id),
        1);
    ...
    if (!m_chrome_renderer.initialize(m_display, m_screen, m_dpi_scale, m_theme))
    {
        std::cerr << "Fatal error: the X11 chrome renderer could not be initialized.\n";
        release_native_resources();
        return false;
    }
...
bool X11Window::is_focused() const
{
    return m_is_focused;
}
...
void X11Window::set_titlebar_hit_test_callback(TitlebarHitTestCallback callback)
{
...
void X11Window::set_command_state_query_callback(CommandStateQueryCallback callback)
{
...
void X11Window::initialize_atoms()
{
    ...
    m_atoms.net_wm_window_type = XInternAtom(m_display, "_NET_WM_WINDOW_TYPE", False);
    m_atoms.net_wm_window_type_normal = XInternAtom(
        m_display,
    ...
    m_atoms.net_current_desktop = XInternAtom(m_display, "_NET_CURRENT_DESKTOP", False);
    m_atoms.net_wm_state = XInternAtom(m_display, "_NET_WM_STATE", False);
    m_atoms.net_wm_state_maximized_horizontal = XInternAtom(
        m_display,
        "_NET_WM_STATE_MAXIMIZED_HORZ",
        False);
    m_atoms.net_wm_state_maximized_vertical = XInternAtom(
        m_display,
        "_NET_WM_STATE_MAXIMIZED_VERT",
        False);
    m_atoms.net_wm_state_hidden = XInternAtom(m_display, "_NET_WM_STATE_HIDDEN", False);
    m_atoms.net_wm_move_resize = XInternAtom(m_display, "_NET_WM_MOVERESIZE", False);
...
void X11Window::initialize_cursors()
{
    ...
    m_move_resize_cursors[static_cast<std::size_t>(MoveResizeDirection::SizeTopRight)] =
        XCreateFontCursor(m_display, XC_top_right_corner);
    m_move_resize_cursors[static_cast<std::size_t>(MoveResizeDirection::SizeRight)] =
        XCreateFontCursor(m_display, XC_right_side);
    m_move_resize_cursors[static_cast<std::size_t>(MoveResizeDirection::SizeBottomRight)] =
        XCreateFontCursor(m_display, XC_bottom_right_corner);
    m_move_resize_cursors[static_cast<std::size_t>(MoveResizeDirection::SizeBottom)] =
        XCreateFontCursor(m_display, XC_bottom_side);
    m_move_resize_cursors[static_cast<std::size_t>(MoveResizeDirection::SizeBottomLeft)] =
        XCreateFontCursor(m_display, XC_bottom_left_corner);
    m_move_resize_cursors[static_cast<std::size_t>(MoveResizeDirection::SizeLeft)] =
...
{
    if (m_display == nullptr || m_window_handle == 0 || m_atoms.motif_wm_hints == None)
    {
        return;
    }

    MotifWindowManagerHints hints;
    hints.flags = static_cast<unsigned long>(motif_hints_decorations);
    // Linux window controls and their icons remain owned by the active window
    // manager so GNOME, KDE, XFCE, and other desktops keep their native theme.
    hints.decorations = 1UL;
    XChangeProperty(
        m_display,
        m_window_handle,
...
void X11Window::handle_event(XEvent& event)
{
    ...
    case ConfigureNotify:
        if (event.xconfigure.width != m_client_width || event.xconfigure.height != m_client_height)
        {
...
void X11Window::handle_motion(const XMotionEvent& event)
{
    ...
    const UI::Chrome::WindowControl hovered_control = m_chrome_layout.get_window_control(
        static_cast<float>(event.x),
        static_cast<float>(event.y));
    const std::optional<std::size_t> hovered_menu = m_chrome_layout.get_menu_index(
        static_cast<float>(event.x),
        static_cast<float>(event.y));
...
void X11Window::handle_button_press(const XButtonEvent& event)
{
    ...
    {
        const std::optional<std::size_t> popup_item_index = get_popup_item_index(event.x, event.y);
        if (popup_item_index && is_popup_item_enabled(
    ...
    const bool is_double_click = m_last_titlebar_click_time != 0 &&
...
void X11Window::handle_button_release(const XButtonEvent& event)
{
    ...
    if (m_pressed_popup_item_index && m_interaction_state.open_menu_index)
    {
        const std::optional<std::size_t> released_item_index = get_popup_item_index(event.x, event.y);
        const std::size_t menu_index = *m_interaction_state.open_menu_index;
        const std::size_t pressed_item_index = *m_pressed_popup_item_index;
        m_pressed_popup_item_index.reset();
        m_interaction_state.open_menu_index.reset();
        m_interaction_state.hovered_popup_item_index.reset();

        const std::span<const UI::Chrome::WindowMenu> menus = UI::Chrome::get_window_menu_model();
        if (released_item_index == pressed_item_index && menu_index < menus.size() &&
            pressed_item_index < menus[menu_index].items.size() &&
            is_popup_item_enabled(menu_index, pressed_item_index))
        {
            const std::string_view command_id = menus[menu_index].items[pressed_item_index].command_id;
    ...
    const UI::Chrome::WindowControl released_control = m_chrome_layout.get_window_control(
        static_cast<float>(event.x),
        static_cast<float>(event.y));
    const UI::Chrome::WindowControl pressed_control = m_interaction_state.pressed_control;
    m_interaction_state.pressed_control = UI::Chrome::WindowControl::NoControl;
...
void X11Window::handle_key_press(XKeyEvent& event)
{
    ...
    if (accelerator_menu_index && *accelerator_menu_index < m_chrome_layout.visible_menu_count)
    {
    ...
    {
        const std::size_t menu_count = m_chrome_layout.visible_menu_count;
        if (menu_count > 0)
        {
            const std::size_t current_index = *m_interaction_state.open_menu_index;
            const std::size_t next_index = key_symbol == XK_Right
    ...
    if (key_symbol == XK_Return || key_symbol == XK_KP_Enter || key_symbol == XK_space)
    {
...
void X11Window::update_cursor(int point_x, int point_y)
{
    ...
    const std::optional<MoveResizeDirection> direction = get_resize_direction(point_x, point_y);
    if (direction)
    {
        desired_cursor = m_move_resize_cursors[static_cast<std::size_t>(*direction)];
    }
...
void X11Window::begin_move_resize(const XButtonEvent& event, MoveResizeDirection direction)
{
...
void X11Window::update_manual_move_resize(const XMotionEvent& event)
{
    ...
    else
    {
        ...
        const bool resize_top = direction == MoveResizeDirection::SizeTop ||
            direction == MoveResizeDirection::SizeTopLeft ||
            direction == MoveResizeDirection::SizeTopRight;
        const bool resize_bottom = direction == MoveResizeDirection::SizeBottom ||
            direction == MoveResizeDirection::SizeBottomLeft ||
            direction == MoveResizeDirection::SizeBottomRight;

        if (resize_left)
        {
            const int applied_delta = std::min(delta_x, m_manual_start_width - minimum_width);
        ...
        if (resize_top)
        {
            const int applied_delta = std::min(delta_y, m_manual_start_height - minimum_height);
...
void X11Window::send_maximized_state(long operation)
{
    ...
    event.xclient.data.l[1] = static_cast<long>(m_atoms.net_wm_state_maximized_horizontal);
    event.xclient.data.l[2] = static_cast<long>(m_atoms.net_wm_state_maximized_vertical);
...
void X11Window::move_popup_selection(int direction)
{
    ...
    std::ptrdiff_t candidate = m_interaction_state.hovered_popup_item_index
        ? static_cast<std::ptrdiff_t>(*m_interaction_state.hovered_popup_item_index)
    ...
    for (std::size_t attempt = 0; attempt < item_count; ++attempt)
    {
        ...
        if (is_popup_item_enabled(menu_index, static_cast<std::size_t>(candidate)))
        {
            m_interaction_state.hovered_popup_item_index = static_cast<std::size_t>(candidate);
            render();
            return;
        }
    }
...
X11Window::WorkArea X11Window::get_work_area() const
{
    ...
    status = XGetWindowProperty(
        m_display,
        RootWindow(m_display, m_screen),
        m_atoms.net_workarea,
        static_cast<long>(current_desktop * 4),
        4,
        False,
        XA_CARDINAL,
        &actual_type,
...
std::optional<X11Window::MoveResizeDirection> X11Window::get_resize_direction(
    int point_x,
    int point_y) const
{
...
std::optional<std::size_t> X11Window::get_popup_item_index(int point_x, int point_y) const
{
    if (!m_interaction_state.open_menu_index)
    {
        return std::nullopt;
    ...
    const Components::PopupMenuGeometry geometry = m_chrome_renderer.calculate_popup_geometry(
        m_chrome_layout,
        *m_interaction_state.open_menu_index);
    for (std::size_t item_index = 0; item_index < geometry.item_count; ++item_index)
    {
                ...
                static_cast<float>(point_y)))
        {
            ...
            if (*m_interaction_state.open_menu_index < menus.size() &&
                item_index < menus[*m_interaction_state.open_menu_index].items.size() &&
                !menus[*m_interaction_state.open_menu_index].items[item_index].separator)
            {
                return item_index;
            }
        }
...
bool X11Window::is_popup_item_enabled(std::size_t menu_index, std::size_t item_index) const
{
    const std::span<const UI::Chrome::WindowMenu> menus = UI::Chrome::get_window_menu_model();
...
bool X11Window::is_root_atom_supported(Atom atom) const
{
    ...
    const int status = XGetWindowProperty(
        ...
        &bytes_after,
        &property_data);
    if (status != Success || property_data == nullptr || actual_type != XA_ATOM || actual_format != 32)
    {
        if (property_data != nullptr)
        {
            XFree(property_data);
